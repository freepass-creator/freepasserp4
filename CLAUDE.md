...
**하단 홈바 = 찾기 · 검색 · (견적) · 설정**(`lib/tabbar` `appTabsFor` SSOT) — **역할에 따라 셋 또는 넷**.
★**견적은 관리자·공급사에게만 선다**(사장님 2026-09-06 「일단 메뉴 자체를 관리자랑 공급사만 보게 해요. 아직 해당 없어」).
원가·마진·손익이 보이는 화면이라 영업자 폰에는 아예 안 띄운다. 명단 SSOT = `lib/domain/estimate/audience`.
⚠ **메뉴에서 숨기는 것만으로는 막은 게 아니다** — 페이지(`EstimateGate`)와 API(`/api/estimate/cost`)도 같은 명단으로 막는다.
영업자용 «원가 뺀 견적»은 **아직 안 만든다**(사장님 「아직 해당 없어」) — 안 쓰는 분기를 미리 넣으면 다음 사람이 규격으로 안다.
★**폰에서 하는 일은 「차를 찾고 · (값을 내고) · 손님한테 보내기」다**(사장님 2026-08-30 「모바일에서는 그냥 상품 찾고
