...
1. **TopBar 고정** — 모바일은 **네이비 띠 + 상태 표시만**. 누르는 것은 전부 하단 홈바. 웹은 종전대로 흰 바탕·메뉴·세션. (`layout`)
   ★모양은 **전자계약 머리(`components/sign/sign.css` `.c-head`)와 같은 형태** — `[▣✓] freepass │ 이 페이지`
   (사장님 2026-08-30 「상단 바를 그 전자계약 착한거래 형태로」). 보내는 사람(앱)과 받는 사람(손님 계약서)이 한 얼굴로 읽힌다.
   마크 = `BrandMark` 원자(좌표는 `public/icon.svg` 정본 · 흰 네모+남색 체크로 «반전»).
   ⚠ 색은 **원자에 칠하지 않는다** — `.fp-onbar` 안에서 «토큰만» 뒤집으면 `C.*` 쓰는 원자가 전부 따라온다.
   ⚠ 칠하는 주체는 **CSS 미디어쿼리**다(JS `useIsMobile` 아님) — 첫 방문엔 쿠키가 없어 SSR 이 데스크톱으로 잡혀 흰 띠가 번쩍인다.
   ⚠ 상단 제목을 **키우지 않는다** — 한 줄 한 크기(`FS.title`), 위계는 색으로(제목=잉크 굵게 / 대수·보조=회색).
   사장님 2026-08-30 「상단 텍스트 크기만 좀 일괄적으로, **완전 B2C 플랫폼은 아니니까**」(FS.page 로 올렸다가 되돌린 이력).
