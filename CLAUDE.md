...
## ★★손님 화면(가게) 일이면 먼저 읽어라 — `docs/DESIGN_CONFIRMED_SHOP.md`
채널 이름으로 나가는 손님 동(`/shop` 목록 · `/q/[code]` 상세)은 **확정 규격**이 있다.
공유(퍼널) · 대여료 표 · 정책 넷(보험/계약/운전/기타) · 조건의 주소 적재 · 교차 집계 · 반올림 금지.
기계 검사 = `npx tsx scripts/check-design-locked.mts` — 확정된 것이 사라지면 exit 1.
바꾸려면 **사장님께 여쭙고 → 문서를 고치고 → 검사를 고친다.** 검사를 먼저 고쳐 통과시키면 규격을 지운 것이다.
사장님 2026-09-04 「상세페이지를 좀 헤매는 거 같애. 거기를 좀 **확실하게 매듭**을 지어야 될 거 같은데.」
...
### ★★하허호 F86 = «완전 커스텀 레트로» — 확정 규격 (2026-09-16 픽스)
사장님 「86번 시트만 적용해주고, 그 시트는 완전 커스터마이징 하니까 86번만」 · 「이제 픽스해서 규격화해」.
규격 = `docs/영업자시트-매뉴얼.md` §하허호 전용 상품시트 [F86] «완전 커스텀 레트로» · 코드 = `lib/domain/channel-retro-skin.ts`.
탭(공지→종합→회사) · 칸 36(공급사명 맨 앞) · 대여료(단기 칸 없음 · 회사 탭은 쓰는 칸만 · 종합 5칸 · 장기 요금 없는 차 안 실음) · 옛 레트로 겉.
**기계 검사 = `npm run check:f86`** — 발행기가 `--apply` 전에 스스로 돌려 어긋나면 운영 F86 을 안 건드린다 · `check:sync`/`check:release` 에도 들어 있다.
⚠ F86 규격을 F01·공급사시트·다른 채널로 번지게 하지 않는다. F86 을 F01 차례·Roboto 로 되돌리지 않는다.
바꾸려면 **사장님께 여쭙고 → 매뉴얼 절 → 검사** 순서.
...
⚠ 2026-09-04 에 「확인·정정·정정금액」 넉 칸 커밋이 **이력에서 통째로 사라져** 두 칸으로 되돌아간 적이 있다.
  cherry-pick 으로 되살렸고, 그래서 이 검사를 만들었다.
...
그래서 계약진행·재고관리·계약문의·전체메뉴는 **폰 하단에 없다** — **우측 햄버거**로 간다(아직 안 만듦).
⚠ **견적이 탭인 이유** = 재고에 «없는» 차도 센다. 매물 상세 안에만 두면 우리 재고에 없는 차를 물어 온
손님에게 답할 길이 없고, 통화하면서 여는 화면이라 두 번 눌러 들어가면 안 된다.
