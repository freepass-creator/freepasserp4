...
**세대**는 라이브 「차종마스터」 행만. **철자**는 지금 왼쪽 원문. 원문에 없는 `디 올 뉴` 금지(2026-09-02 렌트존 싼타페 MX5).
상품마스터 차번은 이름 입력이 아니다. F03으로 이름을 만들지 않는다. 자동화 = `docs/자동동기-매뉴얼.md` ⓪·①′.
검사 = `npm run audit:raw-ad-prefix`(181허5305) · `npm run audit:live-master-names`.
