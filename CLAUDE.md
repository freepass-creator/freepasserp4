...
## ★★«채널카탈로그» 일이면 먼저 읽어라 — `docs/DESIGN_CONFIRMED_SHOP.md`

**채널카탈로그** = 영업 채널마다 **껍데기만 갈아 끼워** 내주는 손님용(B2C) 카탈로그다
(사장님 2026-09-06 「**이걸 이름을 채널카탈로그라고 하자**」).
채널 도메인으로 들어온 손님은 **로그인 없이** 우리 재고를 그 회사 사이트처럼 본다 —
「손님, 이거 보고 고르세요」가 이 화면의 전부다. 채널이 파는 건 우리 재고고, 손님 눈엔 그 회사다.
...
⚠ **이름은 «채널카탈로그», 코드 경로는 `shop`** 이다(`/shop` · `app/(shop)` · `components/shop` ·
  `ShopCard` · `SHOP`). `/shop` 은 이미 손님에게 나간 링크가 붙은 **살아 있는 주소**라 안 바꾼다.
  주석에 옛 이름 「가게 · 손님 동」이 남아 있으면 **같은 것**이다.
