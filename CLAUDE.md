...
## ★★원자를 «내려보내는» 일이면 먼저 읽어라 — `docs/원자-내려보내기-로직.md`
원자 하나 → 상품리스트 둘(F01·하허호 F86) · ERP · 화이트라벨 **넷에 뿌리는** 규격이다.
정본은 그 문서다 — **코드가 그 문서와 다르면 코드가 틀린 것**이다(사장님 2026-09-08).
★핵심 = §1 **「칸마다 주인은 하나다」** — 정체·제원의 주인은 **직접수집**(원천→원자)이고
 상태는 상태연동+정산원장 · 요금은 가격블록 · 정책은 정책조인. **그 밖은 읽기만 한다.**
 「확정되면 안 바뀐다」(`docs/방향-데이터플랫폼.md` §3-2)가 이 표로 구현된다.
⚠ **§9 를 먼저 보라** — main 은 읽는 쪽(넷이 같은 원자를 본다)은 끝났는데 **쓰는 쪽이 비어 있다.**
 그래서 지금 원자는 원천에서 온 게 아니라 판매시트→RTDB 를 거친 «사본»이고,
 `sheet-merge.ts` 가 정체를 매 회차 덮는다(2026-08-22 의도된 설계 — 고치려면 승인·실측이 먼저).
⚠ 이 문서는 **나흘 동안 main 에 없었다**(닫힌 가지에 갇혀 있었다 — `docs/병합-매뉴얼.md` §6).
