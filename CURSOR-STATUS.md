...
| T9 | _(아래)_ | 1 | n/a | 죽은 원자 실사 표 (코드 변경 0, STATUS만 커밋) |
...
실측: `import { … }` / 심볼 참조. **정의 파일·같은 파일 내부 참조는 사용처에서 제외.**  
배럴(`ui/index.tsx`) 재export만 있는 경우도 사용처 0으로 봄.
...
위 표 **전부**. 문서에 "이걸 써라"로 실려 있으나 앱/페이지에서 import 0.
삭제할지·문서에서 뺄지·실제로 쓰게 할지는 **사장님 판단** (Cursor는 삭제하지 않음).

---

## 현재 상태

`대기중` — 열린 태스크 없음 (T5 🔒 잠금 · T6 ❌ 폐기 · T3/T4는 TASKS 잠금판상 ⬜이나 커밋 완료·STATUS 반영). T5 해제 후 재개.
