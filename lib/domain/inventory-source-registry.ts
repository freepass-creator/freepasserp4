...
/**
 * 차량 재고 원천의 유일한 정본(SSOT).
 *
 * 규칙:
 * - 이 레지스트리만 공급사 재고 원천을 결정한다.
 * - partner.sheet_url, MIRROR_SOURCES, 공급사 허브/정제시트는 재고 원천을 덮어쓸 수 없다.
 * - 아이언은 ironrentcar.com, 오토플러스는 reborncar.co.kr, 손오공은 sokrc.com ERP API가 정본이다.
 * - projection은 정책/보관/표시용이며 재고 정본이 아니다.
 */
