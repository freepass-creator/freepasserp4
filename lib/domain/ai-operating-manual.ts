...
export function buildAiOperatingManual(): ManualSection[] {
  return [
    ...
    { title: '5. 매일 순서(명령) — 이 차례로, 각각 dry-run 먼저', rows: [
      ...
      ['④ 판매시트 발행(탭 4개)', '상품리스트(21곳 − @제외 오플·손오공 구독/픽업 · 출고불가 제외 · 빈 대여료 「-」) → 손오공구독(구독재고=SON_NO_KONG, 반납형·인수형 기간별 금액) → 픽업구독(픽업재고=TCAR_EXTERNAL/T카, 손오공구독과 같은 반납형·인수형 구조, 2026-08-27 추가) → 오플구독(연 2만km 기본, 12개월만 3만km). **오플 12개월 2만km와 폐기된 오플프로모션 탭은 발행·ERP 입력에서 제외**한다. 블록 기본값·표시 이름·별칭은 sales-published-tabs.ts 한 곳. 공급사 하나가 0대로 줄면 멈춘다(맞으면 --force-shrink). 발행된 표 = 네 탭의 합.', 'publish-origin-tab --apply · publish-sonogong-tab --apply · publish-sonogong-tab --tab=픽업구독 --apply · publish-sonogong-tab --tab=오플구독 --apply'],
