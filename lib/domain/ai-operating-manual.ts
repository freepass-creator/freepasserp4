...
import { VEHICLE_MASTER_MATCH_PLAYBOOK, ENCAR_NAME_ABSOLUTE } from './vehicle-master-playbook';
...
export const AI_MANUAL_VERSION = '2026-08-28 v30';
...
export function buildAiOperatingManual(): ManualSection[] {
  return [
    ...
    { title: '0′. ★확정 규칙(2026-08-21) — 오류 없이 굳힌 것. 어기면 버그다', rows: [
      ['순서', '신규 차종은 **엔카 작업 시트에 먼저** → 승인 후 정제시트 정제칸 → 상품시트. 라이브 ERP 원장·mf- 는 확정 전 안 씀. 마스터에 없으면 비슷한 차로 안 붙이고 빈칸.', 'docs/차종마스터-엔카작업시트-매뉴얼.md · fill-supplier-from-encar-sheet'],
      ...
      ['세대 이름', '세부모델 = 엔카 이름 1:1. 예외 둘만: **기아** N세대→개발코드(`K5 3세대`→`K5 DL3`), **우리 시트만** 제네시스 1세대 `G80 DH`(엔카는 `G80`). 그 밖은 엔카 그대로. 괄호 없음(`G80 (RG3)`→`G80 RG3`). NF처럼 연식종료 2019 이전은 넣지 않는다.', 'docs/차종마스터-엔카작업시트-매뉴얼.md'],
    ...
    { title: '1. 시트 지도(요약) — 정본은 원천대장 「시트 지도」 탭', rows: [
      ...
      ['엔카 작업 시트', '이름·제원 작업 정본. 탭: 안내 · 차종마스터(원산지·제조사·모델·세부모델·세부트림·생산시작·생산종료) · 제원마스터(연료·cc·구동) · 전기차배터리마스터. 트림은 배기량 합집합. 정제칸 fill = `fill-supplier-from-encar-sheet`. 라이브 원장과 다른 문서.', url(IDS.엔카마스터)],
    ...
    { title: '2. 정본 서열 — 무엇이 어디의 정본인가', rows: [
      ...
      ['★차명 축은 넷 — 제조사·모델·세부모델·세부트림', '**판매 발행의 정본은 공급사 시트 오른쪽 정제칸**이다. 발행기는 그 값을 판매시트로 «축 그대로» 옮기며 상품마스터를 읽거나 차명을 다시 맞히지 않는다. '
        + '표현의 기본은 **차명 = 세부모델 + 세부트림**(제조사는 제목 줄이 든다 · 사장님 2026-08-22 「표현은 기본이 차명 = 세부모델 + 세부트림」). '
        + '**파워트레인은 폐지**했다 — 엔진 이야기는 「연료」·「배기량」 칸이 든다(「2.2 디젤」을 차명에 붙이지 않는다). '
        + '배기량·연료·색상·차종구분은 차를 설명하는 **부가정보**로 차명 옆에 나란히 둔다.', '아래 0′장 · 3장 · vehicle-refine-flow'],
      ['★ERP 는 정제칸만 본다', '사장님 2026-08-22 「ERP 에는 정제를 쓰는 게 맞고 정제만 잘해 두면 되니까 · 정제시트만 바라보는 거야」. '
        + '차종마스터 참조는 중단했다(재고관리 선택기·매칭근거 제거). 유입 스냅이 정제값을 덮지 못하게 막았고(sheet-import), 「옛 마스터 확정값은 시트가 못 덮는다」 가드도 뺐다(sheet-merge). '
        + '**재고관리 편집 화면은 판매시트와 같은 차례**(제조사·모델·세부모델·세부트림·외장·내장·연식·주행·연료·배기량·차종구분) — 시트와 ERP 를 눈으로 대조할 수 있어야 한다. '
        + '아직 시트가 안 나르는 축: 원산지(보증금 배율에 쓰임 · 정제칸엔 98% 있음) · 인승(정제칸에 없음) — 필요하면 정제칸/판매시트에 열을 세운다.', 'sheet-import · sheet-merge · InventoryEditorPanes'],
      ['정책(보험·연령·주행·분납…)', '공급사 시트 「정책」 탭 한 줄이 정책 하나, 재고 「정책코드」로 조인. 표기 규격 `policy-value-spec.ts`. 비면 「(프리패스 기본)」.', '판매시트 정책 43열은 여기서 나간다'],
      ['열 구성', '판매시트·공급사 상품시트 = 같은 열(현재 코드 예비값 | 66 |열, 실제 정본은 판매시트 「AI 인계」 @매핑·1대=1행) · 공급사 재고 = TEMPLATE_COLUMNS(28)+정제칸(21) · 천이시트 = 7행×13열 카드. 열·셀 규격을 갈래 사이에 복사하지 말고 각 정본 매핑으로 읽는다.', 'sales-sheet-mapping · supplier-preview-parity · channel-card-sheet'],
      ['★정렬 규격', '차명 축 넷(제조사·모델·세부모델·세부트림)은 **왼쪽** — 한 덩어리로 읽는 이름이라 시작 위치가 세로로 맞아야 한다(사장님 2026-08-22 「모델 세부모델 세부트림 정렬은 좌측 정렬」). 옵션·차명·차종구분도 왼쪽. 숫자(금액·Km·배기량·나이·요율)는 **오른쪽**, 날짜와 그 밖은 **가운데**. ⚠ 새 열을 세우면 LEFT/RIGHT_COLUMNS 에 같이 넣어라 — 안 넣으면 조용히 가운데로 떨어진다(실제로 세부모델·세부트림이 그랬다).', 'sales-sheet-format LEFT/RIGHT/CENTER_COLUMNS'],
    ...
    { title: '5. 매일 순서(명령) — 이 차례로, 각각 dry-run 먼저', rows: [
      ...
      ['④ 판매시트 발행(탭 4개)', '상품리스트(21곳 − @제외 오플·손오공 구독/픽업 · 출고불가 제외 · 빈 대여료 「-」) → 손오공구독(구독재고=SON_NO_KONG, 반납형·인수형 기간별 금액) → 픽업구독(픽업재고=TCAR_EXTERNAL/T카, 2026-08-27 추가 — **인수형도 반납형과 같은 12~60 전 기간**. 손오공구독은 SON에 인수형 12·24가 없어 36/48/60만이지만 T카는 상세 estimates에 12·24 인수형이 있다) → 오플구독(연 2만km 기본, 12개월만 3만km). **오플 12개월 2만km와 폐기된 오플프로모션 탭은 발행·ERP 입력에서 제외**한다. ★손오공 구독/픽업 「옵션」·「선택옵션」 = **유상옵션 원문(괄호 안 금액은 뺀다 — 「시트패키지, 베이직플러스」)**, API 기본장비 목록(options[])은 노이즈라 안 쓴다. 가격은 손오공 상세 estimates(creditType LOW)에서 뽑는다(목록보다 완전 — 인수형 12·24 포함). 재고시트 채움은 aiops `손오공.mjs`→`손오공-재고시트.mjs`. 블록 기본값·표시 이름·별칭은 sales-published-tabs.ts 한 곳. 공급사 하나가 0대로 줄면 멈춘다(맞으면 --force-shrink). 발행된 표 = 네 탭의 합.', 'publish-origin-tab --apply · publish-sonogong-tab --apply · publish-sonogong-tab --tab=픽업구독 --apply · publish-sonogong-tab --tab=오플구독 --apply'],
      ...
      ['⑤ ERP 직접 동기', 'ERP는 상품마스터를 거치지 않고 중앙 판매시트 4탭을 직접 읽는다. 공급사 「상품시트」와 천이시트는 ERP 입력이 아니라 같은 정제/제공시트에서 나간 병렬 출력이다. 사전감사·dry-run diff를 확인하고 직전 승인 뒤에만 `--apply`한다.', 'npm run audit:sales-sync-preflight · npm run audit:sales-erp-plan · npx tsx --require ./scripts/lib/server-only-shim.cjs scripts/run-sheet-daily-sync-local.mts [--apply]'],
      ['⑤′ ERP 백업·롤백', '운영 반영은 영향 상품의 RTDB 공개 노드 원본 백업이 성공한 뒤에만 시작한다. 롤백은 같은 실행 ID가 찍힌 행만 대상으로 하며, 후속 수기/동기화 변경이 한 건이라도 있으면 전체 중단한다. 롤백도 미리보기와 별도 직전 승인을 거친다.', 'v4/sheet_sync_backups/{runId} · npm run rollback:sales-erp -- --run={runId} [--apply]'],
      ['⑥ 검수', 'ERP는 현재 판매시트 4탭과 차량번호 집합·상태·제조사·모델·세부모델·세부트림·연료·배기량·모든 기간의 대여료/보증금을 대조한다. 공급사 상품시트·천이시트는 별도 담당 출력이라 ERP 감사와 합쳐 자동수정하지 않는다.', 'audit-sheet-erp-parity · audit-pipeline-destinations'],
      ['⑦ ERP 목록 ↔ 판매시트 대조(ERP 동기 뒤)', '판매시트에만 있는 차량은 오류다. ERP에만 있는 차량도 원칙적으로 오류이며, 계약 엔진 락이 있고 현재 상위 원본이 출고불가임을 확인한 경우만 사유가 있는 예외다. 근거를 못 읽으면 자동 통과시키지 않는다.', 'npm run audit:sales-erp-parity'],
      ['매시간 기본 실행', 'Windows 작업 스케줄러 `freepass-hourly-sync` 기본 모드는 판매시트·ERP를 읽기 전용으로 대조한다. `--apply`는 직전 승인 후 판매시트→ERP를 한 번 반영하고 다시 읽어 검증한다. 상위 시트·공급사 상품시트·천이시트는 쓰지 않는다.', 'scripts/hourly-sync.mts'],
      ['상위 발행 자동화', '중앙 판매시트·공급사 상품시트·천이시트 발행은 별도 담당 작업이다. ERP 정합성 점검자가 임의 재발행하지 않으며 대상·범위·diff 확인과 직전 승인을 받은 별도 실행으로 분리한다.', 'scripts/run-daily.mts'],
    ] },
