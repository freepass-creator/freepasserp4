...
export function cleanTrim(trim: unknown, _maker: unknown, _model: unknown, _subModel: unknown, masterTrims: string[] = [], baseTrim = ''): string {
  ...
  // ② 원문이 «기본»을 «명시»(기본·기본형·기본사양·세부등급없음) → 마스터에 기본형 있으면 복사. 「기본」=엔카 「기본형」의 동의어.
  //    (지어내기 아님 — 마스터에 있는 기본형으로 «복사». 정확일치가 먼저라 「기본」/「기본형」을 각기 가진 모델은 안전.)
  const basic = trims.find((mt) => N(mt) === '기본형');
  if (basic && /^(기본|기본형|기본사양|기본형태|세부등급없음)$/.test(N(t).replace(/[()]/g, ''))) return basic;

  // ③ base-default 모델(마스터가 «엣지 아니면 기본형»이라 선언) → 매칭 안 된 것은 기본형.
  if (baseOn) return base;

  // ④ 마스터가 이 트림을 «정확히» 덮지 못한다 → «공란=미입력»(검수대기). 추측·부분일치 없음.
  //    미입력 = «엔카엔 세부트림이 있는데(base-default 아님) 우리 마스터가 이 원문을 확증 못 함». 지어내지 않고 사람이 마스터를 채운다.
  return '';
}
