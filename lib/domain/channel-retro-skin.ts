 ...
 *   ★장기 요금이 하나도 없는 차(24개월 이후 대여료 없음)도 F86 에 «싣는다», 요금 칸만 빈 채로 둔다
 *     (사장님 2026-09-16 「24개월 이후로 대여료가 없으면 그냥 거기는 대여료 없이 그냥 두자, 그래야 총 상품 숫자를 맞출 수 있다」
 *      · 「안싣는다기보다도 그냥 대여료가 없이 두는거로 하자」 — 9/15 「안 싣는다」에서 정정).
 *     `retroHasLongFee` 는 그 여부(있다/없다)만 판정한다 — 뺄지 말지를 정하지 않는다(발행 계획의 알림에만 쓴다).
 */
export const RETRO_SHORT: readonly string[] = ['단기보증', '1개월', '6개월', '12개월'];
/**
 * 요금 칸에 «값이 있나» — 빈칸·「-」·빈 값 표기(「미입력」·「없음」·「해당없음」, `missing-value-display`)는 값이 아니다.
 *   ★2026-09-16 F86 을 gate 엔진 위로 옮기며 — 그 엔진은 빈 값을 글자로 찍는다(사장님 「F01과 같이 미입력」).
 */
const 빈값표기: readonly string[] = ['-', MISSING_VALUE_LABEL, EXPLICIT_NONE_LABEL, NOT_APPLICABLE_LABEL];
export const retroHasValue = (v: unknown): boolean => { const t = String(v ?? '').trim(); return !!t && !빈값표기.includes(t); };
export function retroHasLongFee(cellOf: (column: string) => unknown, columns: readonly string[]): boolean {
  return columns.some((c) => /\d+개월/.test(c) && !RETRO_SHORT.includes(c) && isMoneyColumn(c) && retroHasValue(cellOf(c)));
}
...
export const RETRO_TAB_ORDER: readonly string[] = [
  // raw supplier ordering. F86 visible front four are assembled separately in channel-f86-plan.
