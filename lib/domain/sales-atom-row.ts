...
// 탭 배정 = 발행기 규칙.
// ★ 손오공상품은 RP012 전체, 픽업구독은 그중 픽업 상품을 다시 보여주는 부분집합이다.
//   따라서 F01의 네 탭은 더 이상 서로 배타적인 partition이 아니다.
export const tabOf = (v: any): string => {
  ...
  if (prov === 'RP023') return '오토플러스';
  return '상품리스트';
};

export const salesTabsForAtom = (v: any): string[] => {
  const prov = S(v.provider_company_code);
  if (prov === 'RP012') {
    return isPickupPhotoAtom(v) ? ['손오공상품', '픽업구독'] : ['손오공상품'];
  }
  if (prov === 'RP023') return ['오토플러스'];
  return ['상품리스트'];
};

export const TAB_ORDER = ['상품리스트', '손오공상품', '픽업구독', '오토플러스'] as const;
