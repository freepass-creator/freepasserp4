...
export function readPolicyTab(rows: string[][]): PolicyBook {
  ...
  if (codeCol >= 0 && nameCol >= 0) {
    ...
    for (const r of rows.slice(1)) {
       ...
       *   그게 진짜 「프리패스 표준」(52칸)을 밀어내 **손오공 343대**(손오공상품 74·픽업구독 269)의
...
export const POLICY_DIRECT: Record<string, string> = {
   ...
   */
  /**
   * ★계좌·비고도 정책에서 온다(2026-08-14). 옛 공급사 시트에는 재고탭 «열»이었는데
   *   문패를 우리 제공시트로 넘기며 사라졌다(전용계좌 71대·비고 70대).
   *   정책마다 하나인 값이라 여기가 제자리다.
   */
  전용계좌: '전용계좌',
  비고: '특이사항',
