...
/**
 * **우리 규격 기간만** 열로 낸다(사장님 2026-08-10). 그 밖은 「기타기간」 한 칸에 모으고,
 * 자세한 건 상세페이지에서 본다 — 6·18개월 같은 비규격 기간, 인수형, 구독.
 *
 * ★열을 늘리지 마라. 공급사마다 파는 기간이 달라 다 펴면 표가 가로로 무너진다.
...
export const HEADERS = [
  ...
  /**
   * ★**판매시트와 같은 축·같은 차례**(사장님 2026-08-23 「우리 재고관리 시트를 개선된 시트를 반영해야 함」).
   *   · 「파워」(파워트레인)는 **폐지 축**이다 — 엔진 이야기는 「연료」가 든다. 되살리지 마라.
   *   · 「트림」 → 「세부트림」 — 판매시트·정제칸·ERP 가 다 그 이름을 쓴다. 여기만 다른 말이면 대조가 안 된다.
   *   · 공급사 원문 두 칸(「2중 보관」)을 뒤에 잇는다 — 정제가 틀렸을 때 되짚는 자리.
   */
  '차량번호', '상태', '상품', '제조사', '모델', '세부모델', '세부트림', '옵션',
...
export function exportRow(p: EntityRecord, providerName: string): (string | number)[] {
  ...
  return [
    ...
    id.maker, id.model, id.sub, id.trim, S(rec.options),
    S(rec.ext_color), S(rec.int_color), N(rec.year), N(rec.mileage), S(rec.fuel_type),
    providerName || S(rec.provider_company_code), cond,
    // 공급사 원문 — 참고용이다(사장님 2026-08-23 「공급사 원문은 기타항목에 참고용으로 넣는 거지」).
    S(rec.supplier_vehicle_name), S(rec.supplier_options),
    ...MONTHS.flatMap((m) => {
 ...
 *   차량번호 상태 상품 제조사 모델 세부모델 세부트림 옵션 외장 내장 연식 주행 연료 공급사 심사 조건 · 기간별
