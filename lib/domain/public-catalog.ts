...
const PUBLIC_POLICY_FIELDS = [
   ...
   *
   * ⚠⚠⚠ **그래도 «부정적인» 칸은 안 올린다**(사장님 같은 날 「기타사항에 **부정적인 거는 넣지마**」).
   *   한때 열둘을 다 올렸다가 사장님이 화면을 보시고 물리셨다. 이 화면은 손님이 차를 «고르는»
   *   자리라, 고르는 손에 벌칙표를 들려 주면 읽기를 멈춘다.
   *   ⇒ 안 올린 것 — 중도해지 위약금 · 연체료율 · 연체 회차 · 자동해지 · **시동제어** ·
   *     보관 · 결격 조건 · GPS.
   *   ★**없앤 게 아니라 «자리»가 아니다.** 그 조건들은 전자계약 약관에 그대로 있고
   *     (`lib/domain/esign-agreement-text`), 손님은 서명 «전»에 읽는다. 상담 중에는 영업자가
   *     관리자 화면에서 본다. 손님 «고르는» 화면과 «약정하는» 문서는 역할이 다르다.
   *
   * ★올린 셋은 **중립이거나 손님에게 이로운 것**이다 —
   *   `deposit_return_days`(돈이 돌아오는 날) · `buyout_notice_days`(인수형을 고를 때 아는 절차) ·
   *   `credit_grade`(「무관」이면 그게 좋은 소식이다).
   *
   * ⚠ **명단과 화면은 한 짝이다.** 여기서 빠지면 값이 화면까지 오지 않고, 화면에서 안 그려도
   *   여기 있으면 응답 JSON 에 실려 나간다 — 되살릴 때는 «양쪽»을 같이 고친다.
   * ⚠⚠ 아래 넷은 처음부터 «안» 올렸다(사장님이 「꽤 넣어줘」 하셨을 때도):
   *   · `commission_clawback_condition` — 우리가 **영업자에게서 환수**하는 조건. 손님 일이 아니고
   *     우리 수수료 구조를 공개 화면에 적는 꼴이다.
   *   · `sales_notes` — **자유 서술 내부 메모.** 무엇이 적혀 있을지 아무도 모른다.
   *   · `provider_company_code` · `source` — 내부 식별자와 파이프라인. 렌트사 «이름»은 나가지만
   *     «어디서 긁어왔는지»는 손님 일이 아니다.
   */
  'deposit_return_days', 'buyout_notice_days', 'credit_grade',
] as const;
