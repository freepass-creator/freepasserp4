...
const PUBLIC_POLICY_FIELDS = [
  ...
  /*
   * ★★★**로그인 뒤에 있던 칸들이 여기로 올라왔다**(사장님 2026-09-16 「**로그인을 안 할 거라서**
   *   기타사항에 **영업자가 보는 걸 꽤 넣어줘야** 함」).
   *
   * ⚠⚠ 2026-09-06 에는 이 칸들을 «영업자만 보는 문»(`/api/shop/inside`)에 두었다 —
   *   「응대에 필요한, 손님에게 안 나가는 정책 칸」이라는 판단이었다. 그 판단의 전제가
   *   **로그인**이었고, 로그인을 안 하기로 하면 그 문은 아무도 못 여는 문이 된다.
   * ★**손님이 계약 전에 알아야 하는 것**만 올렸다 —
   *   위약금 · 연체 · 자동해지 · 시동제어 · 보증금 반환 · 인수 통지 · 보관 · 결격 · GPS · 신용등급.
   *   ★특히 **시동제어**는 숨기면 안 된다 — 차가 멈추는 조건을 계약 뒤에 알게 되면 그건 분쟁이다.
   *   ★**GPS** 도 알려야 한다 — 위치가 기록되는 차라면 그건 손님의 개인정보 문제다.
   *
   * ⚠⚠⚠ **일부러 «안» 올린 것 넷.** 사장님이 「꽤 넣어줘」 하셨지만 이 넷은 성격이 다르다:
   *   · `commission_clawback_condition` — 우리가 **영업자에게서 환수**하는 조건이다. 손님 일이
   *     아니고, 우리 수수료 구조를 공개 화면에 적는 꼴이 된다.
   *   · `sales_notes` — **자유 서술 내부 메모**다. 무엇이 적혀 있을지 아무도 모른다.
   *     「이 손님 조심」 같은 한 줄이라도 있으면 그날 사고다. 공개 화면에 둘 수 없다.
   *   · `provider_company_code` · `source` — 내부 식별자와 파이프라인(구글시트 · 손오공 API).
   *     렌트사 «이름»은 나가지만 «어디서 긁어왔는지»는 손님 일이 아니다.
   *   ⇒ 넷 다 필요하시면 말씀해 주십시오. 제가 임의로 올리지 않았습니다.
   */
  'early_termination_rate_under1y', 'early_termination_rate_over1y',
  'late_fee_rate', 'deposit_overdue_rounds',
  'auto_terminate_overdue_days', 'engine_control_overdue_days',
  'deposit_return_days', 'buyout_notice_days', 'impound_keep_days',
  'disqualification_conditions', 'gps_installed', 'credit_grade',
] as const;
...
const PUBLIC_PRODUCT_FIELDS = [
  ...
  'deposit_note',
  /*
   * ★★★**렌트사 이름 — 손님에게 나간다**(사장님 2026-09-16 「기타사항에 **공급사 하나 넣어줘
   *   렌트사**」 · 「**로그인을 안 할 거라서** 기타사항에 **영업자가 보는 걸 꽤 넣어줘야** 함」).
   *
   * ⚠⚠ **2026-09-06 판단을 뒤집는다.** 그때는 로그인한 영업자만 보는 칸(`/api/shop/inside`)에
   *   두고, 「채널 이름으로 나가는 판에서 뒤에 누가 있는지 꺼내면 답할 필요 없는 질문을 손님
   *   머리에 심는다」·「남의 공급사 이름을 보여 주면 경쟁사에게 매입처를 알려 주는 꼴」이라 적었다.
   *   ⇒ 그 판단의 전제가 **로그인**이었다. 로그인을 안 하기로 하면 그 칸은 «아무도 못 보는 칸»이다.
   *     손님이 실제로 계약하는 상대가 렌트사이므로, 안 보이는 칸에 숨기는 것보다 적는 것이 맞다.
   *   ★여쭙고 받은 답이다(같은 날) — 화이트라벨 페이지에서도 똑같이 드러난다는 것까지 말씀드렸다.
   *     되돌리려면 **이 한 줄을 빼면** 된다.
   * ★**이름만 나간다 — 코드(`provider_company_code`)는 여전히 안 나간다.** 내부 식별자다.
   */
  'provider_name',
] as const;
