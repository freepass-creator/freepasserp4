...
export function importSalesInventoryWorkbook(input: {
  ...
  /** 손오공 픽업(T카) — RP012에 손오공상품과 합쳐 ERP에 반영한다(2026-08-27). */
  ...
  if (wants('RP012')) {
    ...
    // ★손오공 픽업(T카) — 같은 RP012. 손오공상품과 «합쳐» ERP에 함께 반영한다(2026-08-27).
    ...
    fetchedLines.push({
      ...
      message: `✓ ${S(partner.name || partner.partner_name || '손오공')} [손오공상품+픽업구독] — ${sum(result.imported, pickup?.imported)}매물`,
