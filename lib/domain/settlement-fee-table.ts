 ...
 *
 * ★★**프로모션은 «끝난다» — 요율에 섞여 들어오면 다음 달에 틀린다.**
 *   실측 2026-09-08 — 하허호가 161호1543 을 「차량가 8.5%」로 넣었는데,
 *   웰릭스 8월 정산서(PDF)는 **7%**(3,783,000)였다. 사장님 「**송해민은 1.5% 프로모션 끝났음**」 —
 *   8.5% 는 «7% + 1.5% 프로모션»이었고 그 프로모션이 끝난 것이다.
 *   그대로 뒀으면 받는 건 7%, 주는 건 8.5% 라 **우리가 810,563 을 물** 뻔했다.
 *   ⇒ 신차발주 요율을 받아 적을 때는 «그 안에 프로모션이 섞였는지»를 묻는다.
 *     상대 정산서(공급사 쪽)와 맞대면 바로 갈린다.
 *
...
export const FEE_RULES: FeeRule[] = [
  ...
  /**
   * ── 빌린카(LC) = 엘씨렌트 · 구독 ─────────────────────────
   * ★사장님 2026-09-04 「엘시렌트 구독료율이 있을 건데? 한번 찾아볼래?」 — 있었다.
   * ★**실측 두 건이 한 요율로 떨어진다**(2026-08):
   * ```
   * 15다4180  엘씨렌트  60개월  대여료 400,000 → 청구 660,000 · 지급 480,000
   * 333모5593 빌린카   60개월  대여료 450,000 → 청구 742,500 · 지급 540,000
   *           400,000 × 60 × 2.75% = 660,000 ✓   × 2.00% = 480,000 ✓
   *           450,000 × 60 × 2.75% = 742,500 ✓   × 2.00% = 540,000 ✓
   * ```
   * ⚠ **60개월만 넣는다.** 25년 JPK LC 건들은 2.00%/1.60% · 1.89%/1.51% 로 제각각이라
   *   사다리가 있는지 알 수 없다. 다른 기간이 오면 「표에 없다」로 걸려 사람이 확인한다 —
   *   짐작으로 사다리를 지어 놓으면 그 값이 그대로 청구서에 실린다.
   * ⚠ 「엘씨렌트」와 「빌린카」는 같은 회사다(`SUPPLIER_ALIAS`) — 이름을 앞머리로 맞춘다.
   */
  { supplier: '빌린카', kind: '구독', form: '', term: 60, basis: '대여료×기간', claim: 0.0275, pay: 0.02, when: WHEN, auto: true, note: '엘씨렌트 = 빌린카(LC) · 실측 2026-08 두 건' },
