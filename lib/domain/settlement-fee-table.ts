...
export const FEE_RULES: FeeRule[] = [
  ...
  /**
   * ★★**전기차 프로모션 — 기본에 50만을 «그대로 통과»시킨다.**
   *   사장님 2026-09-04 「기본 영업자한테 100 받아서 80 주는데 **프로모션은 그대로 합쳐서 주고 있음**」
   *   「그럼 우리도 오플에다가 **더 청구해야 하는 거지**」 · 「아 오십만원 같다」
   * ```
   * 기본      청구 1,000,000 → 지급   800,000     우리 몫 200,000
   * 프로모션   청구   500,000 → 지급   500,000     우리 몫 0  ← 그대로 통과
   * 합계      청구 1,500,000 → 지급 1,300,000     우리 몫 200,000 (그대로)
   * ```
   *   ★실측 근거 — 하허호 26년 7월 오플정산에 **1,300,000 지급이 세 건**(63주0542·11오1738·63주0980).
   *     기간·대여료와 무관하게 셋 다 같은 값이라 «정액 가산»이다.
   *   ⚠ 우리가 오플에 1,500,000 으로 청구했는지 **7월분을 확인해야 한다** — 안 했으면 건당 50만씩 덜 받았다.
   */
  { supplier: '오토플러스', kind: '전기차', form: '', term: 0, basis: '정액', claim: 1_500_000, pay: 1_300_000, when: WHEN, auto: true, note: '전기차 프로모션 — 기본 100/80 + 프로모션 50만 통과' },
 ...
 */
/**
 * ★★**「HEV」를 전기차로 읽지 않는다.** `EV\b` 는 「K5 HEV」·「그랜저 HEV」의 EV 를 물었다
 *   (실측 2026-09-04 — 오플 전기차 프로모션 50만이 하이브리드 두 대에 붙을 뻔했다).
 *   앞에 «글자가 없어야» 전기차다 — `\bEV`. HEV·PHEV 는 앞이 H 라 안 걸린다.
 * ⚠ 그래도 모델명만으로는 다 못 가린다 — 니로·코나·아이오닉은 하이브리드 판도 있다.
 *   돈이 걸리는 자리(오플 프로모션)는 «재고의 연료 칸»으로 사람이 한 번 확인한다.
 */
...
export const feeKindOf = (product: string, model: string): { kind: FeeRule['kind']; form?: string; fallback?: FeeRule['kind'] } => {
   ...
   *    그래서 최사랑 팀장이 팔십만원을 백삼십만원으로 한 거구나」
   *
   *   여기서 전기차를 안 보고 통째로 「구독」으로 내려보내는 바람에 **오플 전기차 프로모션이
   *   붙을 자리가 없었다.** 그래서 계산기는 늘 80만만 세웠고, 7월에 실제로 130만이 나간 세 건
   *   (63주0542·11오1738·63주0980)을 아무도 못 봤다.
   * ★특약이 없는 공급사는 `fallback` 으로 «그냥 구독»으로 내려간다 — 없으면 «없는 것»이 아니라 일반 규칙이다.
   */
  if (/구독/.test(product)) return ev ? { kind: '전기차', fallback: '구독' } : { kind: '구독' };
