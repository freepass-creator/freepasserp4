/**
 * 프리패스 표준계약서 — **축**으로 정의한다.
 *
 * 사장님 지정(2026-08-08): 표준계약서는 넷이다.
 *   구독 인수형 · 구독 반납형(인수선택) · 렌탈 인수형 · 렌탈 반납형(인수선택)
 *   그리고 **구독은 보험이 둘로 갈린다** — 회사가 넣는 형 / 개인이 직접 넣는 형.
 *
 * ★왜 축으로 두나
 *   문서를 4장(또는 6장) 따로 들면 조항 하나 고칠 때 6군데를 고쳐야 하고, 곧 갈라진다.
 *   **다른 것만 축으로 두고 나머지는 공유한다.** 실제로 다른 건 아래 4가지뿐이다 —
