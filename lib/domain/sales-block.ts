 ...
 *   상품시트로 옮길 때 「▸」만 벗긴다. 탭마다 블록 머리는 그 탭 줄이 실리는 판매 탭의 머리행(상품리스트 / 손오공상품 / 오플구독)과 같다.
...
/** 재고 탭 → 그 줄이 실리는 판매 탭(블록 머리의 정본). 손오공 구독재고 → 손오공상품 · 픽업재고 → 픽업구독 · 오토플러스(RP023) → 오플구독 · 그 밖 → 상품리스트. */
export function salesTabForStockTab(providerCode: string, tabTitle: string): '상품리스트' | '손오공상품' | '픽업구독' | '오플구독' {
  ...
  if (providerCode === 'RP012' && /구독/.test(t)) return '손오공상품';
  return '상품리스트';
}
/** 블록 머리 배경(회청) — 기계 칸 표식. */
