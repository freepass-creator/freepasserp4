 ...
 * ⚠ 「분납중」은 나쁜 상태가 아니라 **아직 안 끝난** 상태다. 분납이 다 들어오면 청구완료로 간다.
 */
import { billingMonth, roundsOf, type SettlementRow } from './settlement-stage';

/**
 * ★★**말은 정산시트에서 가져온다**(사장님 2026-08-26 「우리가 정산시트 기반으로 이런거 통일좀 하고」).
 *   지어낸 말을 쓰면 화면과 시트가 서로 다른 말을 하고, 사람이 매번 번역해야 한다.
 * ```
 * 인도대기   시트 「인도완료」 체크 전            — 청구가 «설» 수 없다
 * 청구예정   시트 「청구월」이 아직 안 왔다
 * 미청구     청구월은 됐는데 청구서가 안 나갔다   ★돈이 새는 자리
 * 분납중     시트 「분납실적」에 앉아 있다        — 부러지면 환수
 * 청구완료   청구서가 나갔고 분납도 끝났다
 * 환수       시트 「환수」 체크
 * 계약취소   시트 「계약취소」 체크
 * ```
 */
...
export const BILL_WHY: Record<BillState, string> = {
  인도대기: '인도가 안 됐습니다. 인도해야 청구월이 섭니다.',
  청구예정: '청구월이 아직 안 왔습니다.',
  미청구: '청구월이 됐는데 청구서가 안 나갔습니다.',
  분납중: '청구는 나갔지만 분납이 아직 안 끝났습니다. 부러지면 돌려줘야 합니다.',
