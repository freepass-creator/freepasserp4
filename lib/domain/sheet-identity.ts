...
const FLOW_OUT_SUPPLIER = (code: string, label: string) => [
  ...
  '① 판매시트 「프리패스 상품리스트」 — 재고 탭 줄이 그대로(출고불가 제외 · 빈 대여료 「-」 · 차명은 정본→정제칸→원문). 탭 4개: 상품리스트 / 손오공 구독재고는 「오공구독」(보증금 반납형·12~60개월 반납형·보증금 인수형·36/48/60개월 인수형) / 손오공 픽업재고(T카)는 「픽업구독」(오공구독과 같은 반납형·인수형, 2026-08-27) / 오토플러스는 「오플구독」(12개월 2만km … 36개월 3만km) — 갈래 탭엔 우리 공통 대여료 블록이 없고 그 공급사 기간별 대여료가 선다. 같은 차는 한 탭에만.',
...
function describe(x: SheetIdentityInput): string {
  switch (x.kind) {
    ...
    case '판매시트': return '영업자가 보는 표 — 탭 4개(「상품리스트」·「오공구독」·「픽업구독」(T카, 2026-08-27)·「오플구독」). 기계가 문패 21곳 재고를 모아 같은 발행기로 찍는 사본 — 손으로 고치지 않는다(다음 발행에 사라진다). ERP 는 네 탭의 합과 정확히 같아야 한다(⑤′).';
...
function reads(x: SheetIdentityInput): [string, string][] {
  switch (x.kind) {
    ...
    case '판매시트': return [['문패 21곳 재고 탭(출고불가 제외) · 차명 정본(상품마스터 코드→규격채택 · 3축 결정) → 정제칸 → 원문 · 정책 43열은 각 시트 「정책」 탭 · 열 구성은 [숨김] AI 인계 @매핑 · 갈래 탭의 원본 요금 블록은 손오공 제공시트 「구독재고」/오플 정제시트 「재고」.', 'publish-origin-tab --apply → --only=RP012:구독 --tab=오공구독 → publish-sonogong-tab → --only=RP023 --tab=오플구독 → publish-sonogong-tab --tab=오플구독']];
    case '원천대장': return [
