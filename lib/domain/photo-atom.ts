...
export function normalizePhotoUrls(value: unknown, baseUrl = ''): string[] {
  ...
  for (const item of source) {
    ...
    if (raw.startsWith('/') && baseUrl) {
      try { url = new URL(raw, baseUrl).toString(); } catch { url = ''; }
    }
...
export function photoAtomFields(value: unknown, collectedAt?: unknown, baseUrl = ''): Record<string, unknown> {
  const image_urls = normalizePhotoUrls(value, baseUrl);
