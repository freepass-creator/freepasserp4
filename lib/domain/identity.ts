...
export function companyAlias(raw: unknown, explicit?: unknown): string {
  ...
  s = s || full;
  /**
   * ★**표시는 한글로 간다** (사장님 2026-09-08 「에스에이 ← 가능한 한글로 가자」 ·
   *   「케이에이치 ← 이런식으로」 · 「제이앤제이 이런식」).
   *   정본은 **우리가 만들어 준 제공시트 이름**이다 — 「[F66] **에스에이** 프리패스 재고」·
   *   「[F59] **제이앤제이**렌트카 프리패스 재고」. 로마자(SA·J&J·KH)는 우리가 줄여 쓰던 말이라
   *   영업자·채널이 보는 표에서는 회사 이름과 안 맞는다.
   * ⚠ 잇는 것은 그대로다 — `SPELL_PAIRS` 가 두 표기를 같은 회사로 묶으므로
   *   발행 가드·정산 조인은 이름표가 바뀌어도 «정체»로 붙는다(2026-09-04 f036a364).
   */
  const kor = SPELL_PAIRS.find(([latin]) => latin.toLowerCase() === s.toLowerCase());
  return kor ? kor[1] : s;
}
