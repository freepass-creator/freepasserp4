...
export const AI_TAIL_COLUMNS: { name: string; note: string; required?: boolean }[] = [
  ...
  { name: '세부트림', note: '★프리패스가 채움 — 트림. 트림까지 알면 박는다. 모르면 빈칸' },
  { name: '배기량(정제)', note: '★프리패스가 채움 — 차종마스터 cc. 정확값이 하나로 모이면 cc(1598), 1591/1598처럼만 갈리면 표시배기량(1.6). 2.0과 1.6이 같이 있으면 차명·왼쪽 칸의 1.6으로 좁힌 뒤 채움' },
  { name: '연료(정제)', note: '★프리패스가 채움 — 가솔린 · 디젤 · 하이브리드 · 전기 · 수소 · LPG. 하나로 모일 때만' },
  { name: '구동방식', note: '★프리패스가 채움 — 2WD · AWD. 트림 안에서도 갈리면 차명(2WD)으로 좁힌 뒤 채움' },
  { name: '차종크기', note: '★프리패스가 채움 — 경형 · 소형 · 준중형 · 중형 · 준대형 · 대형. 하나로 모일 때만' },
  { name: '차종구분', note: '★프리패스가 채움 — 세단 · SUV · MPV · 해치백 · 쿠페 · 왜건 · 픽업 · 화물. 하나로 모일 때만' },
  { name: '차종코드', note: '★프리패스가 채움 — ERP 차종코드(mf-…). 엔카 T가 아님. 상품마스터 확정 코드를 복사한다' },
  ...
  { name: '차종분류', note: '★프리패스가 채움 — 차종크기+차종구분. 준중형 SUV. 판매시트 호환. 둘 다 알 때만' },
];
...
export function columnWidth(name: string): number {
  ...
  if (name === '원산지' || name === '구동방식' || name === '차종크기' || name === '차종구분') return 100;
