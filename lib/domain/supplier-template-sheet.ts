...
export const AI_TAIL_COLUMNS: { name: string; note: string; required?: boolean }[] = [
   ...
   *
   *   차종은 차번마다 **한 번 정하면 안 바뀌는 값**이다. 그런데 코드가 없으면 매번
   *   이름으로 «알아맞혀야» 하고, 알아맞히는 한 언젠가는 틀린다. 실제로 그렇게 틀렸다 —
   *     · 배기량 칸에 배터리 용량 77400 이 박혔다(파워트레인 «글자»에서 숫자를 긁어서)
   *     · 파워트레인 44대가 「하이브리드 1.6 2WD」 → 「하이브리드 2WD 6인승」으로 뒤집혔다
   *     · 확신도 medium 인데 세대를 잘못 잡은 차가 27대였다
   *   코드를 박아 두면 **그 셋이 전부 안 생긴다.** 알아맞히는 단계 자체가 없어지기 때문이다.
   *
   * ★값은 「ERP4 차종마스터 원천대장」의 **트림행키**다 — `mf-002.md-036.sm-ka4::v01::t01`.
   *   `마스터ID::파워트레인순번::트림순번` 꼴이라 제조사부터 세부트림까지 한 줄로 정해진다.
   * ⚠ 뒤따르는 열들(제조사(정제)~차종분류)은 **이 코드에서 나온 «표시값»**이다.
   *   사람이 눈으로 보라고 같이 적어 두는 것이지, 그게 정본이 아니다.
   *   값이 서로 어긋나면 **코드가 이긴다.**
   */
   ...
   *     서로 어긋날 수가 없다. 공급사 입력칸을 그냥 옮겨 담지 마라 —
