...
/**
 * 매번 공급사를 따라간다 — 상태·대여료·보증금·**주행거리**.
 * ★2026-09-04 사장님 「주행거리는 대여료처럼 변동됨 — 다시 올라올 때 변할 수 있어.
 *   주행거리 및 대여료/보증금 이렇게 같이 두면 되겠네, 주행거리에 따라 대여료·보증금 조건이 달라질 테니까」.
 *   → 주행거리를 once 에서 **live 로 옮긴다.** (예전 2026-08-15 「상태·대여료값만」·주행거리 once 결정을 뒤집음 —
 *    세워 둔 차도 재입고 때 주행거리가 갱신되고, 그에 따라 대여료·보증금 조건이 달라진다.)
 * ★보증금(단기·장기)은 대여료와 **한 벌로 움직이는 값**이라 대여료값에 넣었다.
 * ⚠ 차종·색·연식·차대번호는 여전히 once — 한 번 정리하면 우리 것이다(안 바뀐다).
 */
const LIVE_COLUMNS = [
  '상태', '배차상태', '판매상태', '차량상태',
  '단기보증', '장기보증', '보증금',
  '주행거리', 'Km', 'km', '예상주행거리',   // 주행거리 = 변동(2026-09-04) — 대여료·보증금과 한 묶음
