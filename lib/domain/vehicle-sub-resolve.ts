...
import { realMasterTrims } from './vehicle-master-options';

/** 간단 유사도(트림 resolve 의존 없이 스크립트에서도 동작). */
...
export function resolveSubModel(
  ...
  // 견적기 표기 별칭 → 마스터 sub (증거: 웰릭스「포터2」= 마스터「포터 II」)
  const ALIAS: Record<string, string> = {
    '포터2': '포터 II',
    '포터 2': '포터 II',
    porter2: '포터 II',
  };
  const aliasHit = ALIAS[model] || ALIAS[flatSub(model)];
  if (aliasHit && bySub.has(aliasHit)) {
    return { sub: aliasHit, how: `별칭:${model}→${aliasHit}`, cands: [], yearDecided: false };
  }
  ...
  const faceliftRank = (sub: string) => {
    if (/디\s*올\s*뉴|the\s*all\s*new/i.test(sub)) return 3;
    if (/더\s*뉴|the\s*new/i.test(sub)) return 2;
    if (/올\s*뉴|all\s*new/i.test(sub)) return 1;
    return 0;
  };

  cands.sort((a, b) => {
    // 신차(연식=기준연) + 같은 gen_code: 페이스리프트를 이름점수보다 먼저
    const ca = (bySub.get(a.sub)?.codes || [])[0] || '';
    const cb = (bySub.get(b.sub)?.codes || [])[0] || '';
    if (
      ca && ca === cb && y0 && y0 === y1
      && a.yearScore >= 0.85 && b.yearScore >= 0.85
    ) {
      const fa = faceliftRank(a.sub);
      const fb = faceliftRank(b.sub);
