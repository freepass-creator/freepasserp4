...
export type CostSettings = {
  ...
  /** 중고 조달금리 — 렌트사가 그냥 «중고 금리»로 적는다(사장님 2026-09-17
   *  「금리는 어차피 렌트사에서 입력을 하는 거니까 그 금리 입력하는 칸에다가 **신차금리 중고금리**가 들어가겠지」).
   *  ⚠ 위 `interestA/B/CPct` 가 **신차 금리**다. 가산(%p)이 아니라 «두 칸»이라 렌트사가 아는 말로 적는다.
   *  실무 차이는 평균 3%p(사장님) — 기본값이 그 값이다. */
  interestUsedAPct: number; interestUsedBPct: number; interestUsedCPct: number;
...
export const COST_DEFAULTS: CostSettings = {
  ...
  // 중고는 신차보다 평균 3%p 높다(사장님 2026-09-17) → 신차 6 · 중고 9
  interestUsedAPct: pct(D.interestRate.rent) + 3, interestUsedBPct: pct(D.interestRate.rent) + 3, interestUsedCPct: pct(D.interestRate.rent) + 3,
...
export function configFrom(cs: CostSettings, opts: { newCar?: boolean; path?: AcqPath; credit?: string } = {}) {
  ...
  /* ★★**금리 칸은 둘이다** — 신용(A/B/C) × 신차/중고.
       사장님 2026-09-17 「중고차와 신차의 금리는 평균 3% 정도 차이가 난다」 ·
       「금리는 어차피 렌트사에서 입력을 하는 거니까 그 칸에 **신차금리 중고금리**가 들어가겠지」.
     ⇒ 가산(%p)이 아니라 **칸이 둘**이다 — 렌트사는 「신차 6, 중고 9」처럼 그냥 적는다.
     ⚠ 여태는 신용만 보고 같은 금리를 썼다 — 그래서 중고 원가가 그만큼 싸게 섰다. */
  const interestPct = opts.newCar
    ? (band === 'C' ? cs.interestCPct : band === 'B' ? cs.interestBPct : cs.interestAPct)
    : (band === 'C' ? cs.interestUsedCPct : band === 'B' ? cs.interestUsedBPct : cs.interestUsedAPct);
