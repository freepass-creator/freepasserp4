...
export const CHANNEL = {
  rent: {
    ...
    /**
     * 자동차세 — 영업용. **지방세법 제127조**: 1,600cc **이하** 18원 · 2,500cc **이하** 19원 · 초과 24원.
     * ⚠⚠ 여기 상한이 `2499` 였다(엑셀 원식 `IF(cc<2500,19,24)` 를 그대로 옮긴 값).
     *   그러면 **정확히 2,500cc 인 차가 24원**을 문다 — 법은 「2,500 이하」라 19원이다.
     *   2026-09-07 코덱스 대조에서 잡혔다. **엑셀이 법과 달랐다.**
     * ★경계를 법대로 두면 부수 효과가 하나 더 있다 — 신차 표시 리터(2.5)를 2,500cc 로
     *   반올림해도 실제 배기량(2,497)과 **같은 구간**에 떨어진다. 반올림이 세율을 안 바꾼다.
     */
    cartaxBrackets: [
      { maxCc: 1600, perCc: 18 },
      { maxCc: 2500, perCc: 19 },
  ...
  sub: {
    ...
    /**
     * 자동차세 — 비영업용. 지방세법 80/140/200원 + **지방교육세 30%** = 104/182/260.
     * ⚠⚠ 가운데 상한이 `1599` 였다. 그러면 **정확히 1,600cc 인 차가 260원**을 문다 —
     *   법은 「1,600cc 이하」라 182원이다. 1,600 은 아반떼·셀토스·코나가 쓰는 **가장 흔한 배기량**이고,
     *   차이가 **43%**(연 291,000 → 416,000)다. 2026-09-07 코덱스 대조에서 잡혔다.
     */
    cartaxBrackets: [
      { maxCc: 1000, perCc: 104 },
      { maxCc: 1600, perCc: 182 },
...
// acqTaxCredit / bondExempt — **2026-09-07 부터 엔진이 읽는다.** 전기차 «법정» 혜택이다.
//   acqTaxCredit  취득세 감면 한도(원). 전기차 **140만원**(지방세특례제한법 §66③ · 한도까지 전액 감면).
//                 취득세가 한도보다 적으면 그만큼만 깎인다.
//   bondExempt    도시철도·지역개발채권 **면제**. 전기차는 매입 의무가 없다.
//   ⚠ 이 둘은 «법»이라 원가 화면에서 못 고친다. 해마다 바뀌는 **구매보조금**은 원가 화면 칸이다
//     (`CostSettings.evSubsidy` — 국고·지자체가 달라 회사가 정한다).
export const FUEL = {
  ...
  ev:       { label: '전기',   taxYearFlat: null, quoteCc: 1000, acqTaxCredit: 1400000, bondExempt: true },
};
