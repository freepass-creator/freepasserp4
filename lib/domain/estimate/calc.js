...
export function computeTerm(term, input) {
  ...
  /** 원가가 딛는 값 — 우리가 «실제로 치르는» 돈. 보조금·세제감면을 둘 다 뺀다. */
  const netPrice = Math.max(0, price - evSubsidy - saleTaxCredit);
  /**
   * ★★**손님이 딛는 값** — 보증금·선납·만기인수의 밑값. **보조금은 안 뺀다.**
   *
   * 사장님 2026-09-10 판단 — **구매보조금은 «우리»가 받는 돈**이다. 그것까지 빼면
   *   손님 만기인수가 4,592만 → 4,244만으로 **348만을 그냥 깎아 주는** 셈이 된다.
   *   세제감면은 «판매가격»이 실제로 그만큼 싼 것이라 손님 기준에서도 뺀다.
   * ⇒ **원가 기준(netPrice)과 손님 기준(custBase)을 가른다.** 각각 한 값이고, 견적서가 어느 쪽인지 적는다.
   * ⚠ 되돌리려면 이 한 줄에서 `- evSubsidy` 를 더하면 된다.
   */
  const custBase = Math.max(0, price - saleTaxCredit);
  ...
  const depositByRate = custBase * (ch$(input.depositRatio) ?? FINANCE.depositRatio);   // ★손님 기준
  ...
  /**
   * ★★**자체 충당은 «해마다 줄어든 시세»에 건다**(사장님 2026-09-17
   *   「연간 1.5% 충당한다는 거는 **감가된 금액의 시세의 1.5%**야. 3천만 원짜리 차가 첫 해는 3천만 원,
   *    두 번째는 2,500만 원, 세 번째는 2천만 원에 1.5% 이렇게 산출이 된다고」).
   *
   * ⚠ 여태는 «첫해 값 × 연수»였다 — 4년이면 3,000만에 네 번 곱한 셈이라 **뒤로 갈수록 과대**했다.
   *   자차는 그 해에 «차가 얼마짜리인가»에 걸리는 돈이다. 차가 싸지면 충당도 줄어야 맞다.
   * ★밑값은 **시세 그대로**(`netPrice`)다 — VAT 제외가가 아니다. 충당금은 환급과 무관한
   *   우리가 쌓는 돈이고, 사장님이 「차량가의 1.5%」로 셈하신다(2026-09-17).
   * ⚠ 해가 덜 찬 약정(18개월 등)은 **그 해의 몫만큼만** 센다 — 0.5년이면 절반.
   */
  const selfBaseAt = (yearIdx) => {
    if (yearIdx <= 0) return netPrice;
    const r = resolveResidualRate(yearIdx * 12, {
      residualRates, group, accident: 'none', mileage: null, age: null,
      adjust: input.adjust, residualTable: input.residualTable, ageBaked: true,
    });
    return netPrice * (Number.isFinite(r) ? r : 1);
  };
  let selfAccrual = 0;
  for (let y = 0; y < Math.ceil(years); y++) {
    const span = Math.min(1, years - y);             // 마지막 해는 남은 만큼만
    selfAccrual += selfBaseAt(y) * selfRate * span;
  }
  const selfIns = selfAccrual + selfInsuredYear * years;   // 자차 — 자체충당(해마다 시세) + 자차보험(정액)
  ...
  const out = {
    ...
    custBase,       // 손님이 딛는 값 — 보증금·선납·인수의 밑값(보조금은 «안» 뺀다)
