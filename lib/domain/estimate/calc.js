...
export function computeTerm(term, input) {
  ...
  const selfIns = (priceTotal * selfRate + selfInsuredYear) * years;   // 자차 — 자체충당 + 자차보험
  ...
  const directBase = carCost + conditionRisk + interest + acqTax + bond + regFee + cartax + gps + maint + parking + salesFee + insurance + selfIns + ew + initCost;
  /**
