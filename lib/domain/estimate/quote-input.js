...
export function createQuoteInput({ adminCfg, channel, type, form, conditions, residual, residualDefault, credit, defaultGroup, nowYear }) {
  ...
  return {
    ...
    /* ★★선납금은 **화면이 보여 준 값** 그대로다. 예전에는 여기서 «감면 전» `form.price` 로
       다시 셌다 — 손님 문서엔 791만이라 적히고 엔진은 833만을 받은 것으로 계산해
       월납이 9,000원 낮게 나왔다(2026-09-10 · Codex 발견 1).
       ⚠ `netPrice` 는 엔진이 딛는 값과 같아야 한다(price − 보조금 − 세제감면). */
    prepay: Math.round((Number(form.netPrice ?? form.price) || 0) * conditions.prepayPct / 100),
