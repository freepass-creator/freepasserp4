...
export const TURNOVER = {
  ...
  // 회당 «받는» 돈 — 중도해지 위약금. 회계로는 영업외수익이지만, 원가로는 손바뀜 회당 비용을 깎는다.
  //   ★원가 화면이 **평균 보증금 × 신용 구간별 회수율**로 계산해 넘긴다(`cost-settings.configFrom`).
  //     사장님 2026-09-06 「평균 보증금을 입력하면 … 그만큼 상쇄가 되겠지. 근데 사실상 위약금이
  //     발생돼도 **저신용은 거의 못 받거든.**」 — 받아 낼 «자리»와 «받아 내는 정도»는 다른 값이다.
...
export function turnoverCost(credit, opts = {}) {
  ...
  const perTurnover = Math.max(0, gross - (c.penaltyIncome || 0));                // 위약금만큼 상쇄
