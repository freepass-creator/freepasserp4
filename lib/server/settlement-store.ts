 ...
 *
 * ─────────────────────────────────────────────────────────────────────
 * ★★★**2026-09-16 현재 — 한 목소리로 적는다.**
 *   전에 이 자리에 주석이 «둘» 있었고 서로 다른 말을 했다. 하나는 `v4/settlement_rows`(RTDB)를
 *   「정본」이라 했고, 바로 아래 하나는 「운영 정본은 직원이 쓰는 시트」라고 했다.
 *   **정본이 무엇인지 코드가 두 말을 하면 사람도 두 말을 한다.** 그래서 하나로 합친다.
 * ```
 * 운영 정본     구글 정산원장 네 탭              ← 9월 정산은 아직 «시트»에서 한다
 * 원자 저장소   fp4 Firestore `settlement_rows`  ← 시트에서 한 방향으로 부어 넣은 것
 * RTDB          폐기(2026-09-14 사장님 직접 결정) ← 되살리지 않는다
 * ```
 *   ⚠ **`'erp'` 가 가리키는 곳은 RTDB 가 아니라 Firestore 다.** `settlement-erp-store` 의
 *     `.ref('settlement_rows')` 는 `firestore-path-store` 심을 타 Firestore 컬렉션으로 간다.
 *     옛 주석의 「파이어베이스 v4/settlement_rows」는 이관 «전»의 이야기다.
 *   ⚠ **정산은 erp5 로 옮기지 않는다**(2026-09-16 대표 결정) — fp4 Firestore 에 그대로 둔다.
 *     `ERP5-FIRESTORE-ERP4-전환.md`(2026-09-14)가 내부 ERP·계약·정산을 명시적으로 제외한다.
 *
 * ★★**언제 `'erp'` 로 켜나 — 그건 대표가 시점을 정한다. 코드가 저절로 켜지 않는다.**
 *   환경변수를 빼먹었을 때 ERP 직접입력으로 «조용히» 바뀌면 그날로 정본이 둘이 된다.
 *   그래서 기본은 `'sheet'` 이고, `SETTLEMENT_STORE=erp` 를 «명시»한 경우에만 연다.
 *   ERP 반영본은 `settlement-sheet-import` 가 시트에서 **한 방향으로** 올린다.
 *   ★부르는 쪽(화면·API)은 어느 쪽인지 «모른다». 모양이 같아서다.
 *   ⚠ 되돌리려면 환경변수만 지운다. 그 밖은 아무것도 안 건드린다.
