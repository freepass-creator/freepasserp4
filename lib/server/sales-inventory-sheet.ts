...
export async function fetchSalesInventorySheet(input: {
  ...
  const pickup = selected.get('pickup')!;
  ...
  const canonicalOptions = {
    // 상품리스트·손오공상품·픽업구독의 필터/행 숨김은 영업자의 조회 상태일 뿐 삭제 지시가 아니다.
  ...
  const pickupSheet = visibleRowsFromGridResponse(grid, pickup.gid, canonicalOptions);
  ...
  return importSalesInventoryWorkbook({
    ...
    pickup: { ...pickupSheet, gid: pickup.gid },
