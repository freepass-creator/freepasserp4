/**
 * **정산 저장소 — Firestore 쪽.** 시트를 대신한다.
 *
 * ★★★**어디에 쓰나 — 2026-09-16 기준.** `firebaseAdminStore()` 가 돌려주는 것은 «RTDB 가 아니다».
 *   `firestore-path-store` 심이라 `.ref('settlement_rows')` 는 fp4 Firestore 컬렉션
 *   `settlement_rows` 로 간다. RTDB 는 2026-09-14 사장님 직접 결정으로 **영구 폐기**다.
 *   아래 옛 주석의 「파이어베이스」·「RTDB」는 이관 «전»의 이야기라 그대로 두되, 지금 말은 이것이다.
 * ⚠ 이 파일은 `SETTLEMENT_STORE=erp` 를 «명시»했을 때만 탄다. 지금 운영 정본은 직원이 쓰는 시트다
 *   (`settlement-store.ts` 참고). 켜는 시점은 대표가 정한다.
 *
 * ★사장님 2026-08-26
 ...
 * ★★**열쇠는 둘이다.**
 * ```
 * 저장 열쇠   stl_… 대체키   — 문서 id 자리(옛 RTDB 키 자리). 절대 안 바뀐다
 ...
 * ⚠ 저장소가 `undefined`·빈 배열을 조용히 버린다(RTDB 시절부터 그랬다). 읽을 때 `normalizeRecord` 를 꼭 거친다 —
