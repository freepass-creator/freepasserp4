...
/** 공개 화이트라벨의 유일한 재고 원장: ERP5 Firestore. */
export const ERP5_FIREBASE_PROJECT_ID = 'freepasserp5';
const ERP5_APP_NAME = 'freepasserp4-erp5-public-catalog';
...
function erp5Credential(): Credential {
  ...
  if (!projectId || !clientEmail || !privateKey) throw new Error('ERP5 서비스계정 필수값 누락');
...
/** 운영 스위치가 꺼져 있으면 다른 저장소로 우회하지 않고 실패한다. */
