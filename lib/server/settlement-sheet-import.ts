/**
 * **시트에 적힌 정산을 Firestore 원자로 올린다** — 화면 단추와 명령줄이 «같은 코드»를 쓴다.
 *
 * ★★★**어디로 올라가나 — 2026-09-16 기준.** `firebaseAdminStore()` 는 «RTDB 가 아니다».
 *   `firestore-path-store` 심이라 `db.ref(NODE)` 는 fp4 Firestore 컬렉션 `settlement_rows` 로 간다.
 *   ⇒ 「가져오기」 단추는 **원자 저장소(Firestore)** 를 갱신한다. RTDB 는 2026-09-14 영구 폐기다.
 *   ⚠ 이 통로는 **한 방향**이다 — 운영 정본인 «시트»에서 원자로만 흐른다. 거꾸로 쓰지 않는다.
 *   ⚠ 아래 옛 주석의 「파이어베이스」·`v4/…` 표기는 이관 «전»의 이야기다. 지금 말은 이것이다.
 *
