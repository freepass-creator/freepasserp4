...
export const WHITELABELS: Whitelabel[] = [
   ...
   */
  {
     ...
     */
    /*
     * ⚠⚠ **`www` 는 여기 없다 — 거기는 회사 소개 홈페이지가 산다.**
     *   2026-09-07 에 사장님이 「freepassmobility.com 여기에 붙이자 우리거는」 하셔서
     *   **꼭지(apex)만** 이 프로젝트로 옮겼다(`freepasshomepage` → `freepasserp4`).
     *   ⇒ `freepassmobility.com` = 매물 화면 · `www.freepassmobility.com` = 회사 소개.
     *   DNS 를 손대지 않고 그날 바로 가를 수 있는 유일한 짜임이었다(둘 다 이미 Vercel 을 본다).
     * ★다른 주소(`about.` 등)로 소개를 옮기려면 가비아에 CNAME 한 줄이면 된다 — 그때 여기와
