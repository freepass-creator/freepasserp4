...
export type Whitelabel = {
  ...
  self?: boolean;
  /**
   * **라벨이 없는 기본 얼굴인가.** ERP4 MAIN의 `freepasserp.com` 표준 화면이 이 모드다.
   * 회사 간판은 비워도 검색·조건·목록·상세에 필요한 공통 frame은 유지한다.
   * `FREEPASS` 레거시 업무 chrome과는 별도 값으로 유지해 공개 메인과 업무 경계를 섞지 않는다.
   */
  plain?: boolean;
  /**
   ...
   */
  /**
   * **검색창 «위» 배너** — 손님에게 말을 거는 한 자리.
   *
   * ★★★사장님 2026-09-18 「표준라벨에서 배너를 조금 **적극적으로 활용**해 볼 생각이거든 ·
   *   어떤 상품이 좋다 · 무보증 상품이 있다 이런 것들 · **앞으로 이미지 또는 텍스트로 넣을 거야**」.
   *
   * ★**글이냐 그림이냐 — 둘 중 하나다.** `image` 가 있으면 그림이 서고, 없으면 `title`·`body` 가 선다.
   *   둘 다 없으면 배너 자체를 안 그린다(빈 띠가 첫 화면을 먹는 것보다 없는 편이 낫다).
   */
  notice?: {
    /** 글 배너 — 제목·본문. 그림 배너면 비운다. */
    title?: string;
    body?: string;
    /**
     * 그림 배너 — 넣으면 글자 «대신» 그림이 선다.
     *
     * ★★**웹·폰 두 벌이다.** 배너 폭이 웹 1280 · 폰 343 으로 **네 배 가까이** 차이 난다 —
     *   한 장을 양쪽에 늘리면 폰에서 글자가 뭉개지거나 웹에서 여백만 남는다.
     *   폰 것이 없으면 웹 것을 쓴다(없는 것보다 낫다).
     * ★권장 규격 — 웹 **1280 × 160** · 폰 **686 × 320**(2배 해상도 기준). 파일은 `public/banner/` 에 둔다.
     * ⚠ `alt` 는 **필수**다. 그림이 안 뜨거나 화면을 읽어 주는 손님에게는 이 말이 배너 전부다.
     */
    image?: { web: string; mobile?: string; alt: string };
    /**
     * **누르면 가는 곳 — 배너 면 «전체»가 그리로 간다.**
     *
     * ★★조건이 걸린 목록을 그대로 열 수 있다 — 주소가 곧 조건이다(`lib/shop/query` `readQuery`).
     *   예) `/?perk=무보증` → 보증금 없는 차만(운영 실측 687대 중 **341대**) ·
     *       `/?ptype=신차렌트` 처럼 축 이름(`SHOP_AXES`)을 그대로 쓴다.
     *   ⇒ 「무보증 상품이 있다」고 말만 하고 끝내지 않고, **그 차들 앞에 손님을 내려놓는다.**
     * ★`label` 을 주면 글 배너 밑에 꼬리 링크(「… ›」)도 같이 그린다.
     *   그림 배너에는 안 그린다 — 그림 면이 곧 단추라 같은 말을 두 번 하는 셈이다.
     */
...
export const WHITELABELS: Whitelabel[] = [
  /**
   * **ERP4 MAIN 기본 얼굴 — freepasserp.com 공개 상품 조회·검색 화면.**
   *
   * 2026-09-19 현재 `freepasserp.com`의 대문은 이 `plain` 얼굴이다.
   * 로그인 현관이나 과거 업무동의 대체 화면이 아니라, 상품을 찾고 비교하고 상세를 보는 ERP4 메인이다.
   * 라벨은 비우되 검색·조건·목록·상세에 필요한 공통 껍데기는 유지한다.
   * 공개 화면에는 로그인·업무화면·내부 시트 진입을 다시 넣지 않는다.
   */
  {
    key: 'plain',
    plain: true,
    ...
    self: true,
    /* ERP4 MAIN 운영 호스트. 호스트 기반 rewrite가 주소 `/`를 유지한 채 `/shop`을 보여준다. */
    hosts: ['freepasserp.com', 'www.freepasserp.com'],
    /** 2026-09-10 실측 — 꼭지·www 둘 다 이 프로젝트를 가리킨다. */
    domainReady: true,
    /* 탭/메타와 공통으로 쓰는 설명. 로그인/업무동으로 되돌리는 근거로 쓰지 않는다. */
    ...
    notice: {
      /*
       * ★★**2026-09-21 표준라벨 배너는 B2B 영업지원 톤으로 간다.**
       *   소비자에게 혜택을 직접 광고하는 자리보다, 영업자가 고객 조건을 빠르게 맞춰 보고
       *   상담에 바로 활용하는 «상품찾기 도구»라는 인상을 주는 쪽이 이 화면의 역할과 맞다.
       * ★문구는 실제 화면에서 확인 가능한 축만 쓴다 — 무보증 여부·심사조건·보증금·월 대여료.
       * ★표준라벨에만 적용한다 — 채널 간판의 배너는 그 회사 말이라 여기서 안 건드린다.
       */
      title: '신용점수 관계 없이 누구나 가능합니다',
      body: '고객 조건에 맞는 차량을 검색하고, 대여료·보증금·이용조건을 비교해 상담에 활용하세요.',
    },
  },
  {
    key: 'uniplan',
 ...
 *
 * `GUEST_FALLBACK_KEY`는 호스트/채널을 특정하지 못한 공개 손님 화면의 기본 얼굴만 정한다.
 * ERP4 MAIN의 `freepasserp.com`은 `plain.hosts`로 직접 판정되므로 이 fallback과 분리된다.
 * 레거시 업무 기본값(`FREEPASS`)에 공개 브랜드/frame을 섞지 않는다. 기계 검사는 `npm run check:guest`.
/*
 ...
 * ⚠ 업무동 기본값(`FREEPASS`)은 여기서도 false 다. 공개 ERP4 MAIN은 `plain` 호스트가 맡고,
 *   기존 업무 컴포넌트는 별도 레거시 경로에서만 사용한다.
 */
...
/**
 * 표에 등록되지 않은 호스트의 루트 fallback.
 *
 * 운영 `freepasserp.com`은 이미 `plain.hosts`에 등록되어 있어 이 값과 무관하게 상품 메인이다.
 * false는 로컬/임시 호스트에서 옛 업무 경로를 직접 점검할 수 있게 남긴 호환 스위치일 뿐,
 * ERP4 MAIN의 제품 정의를 나타내지 않는다.
 */
 ...
 * 루트 레이아웃은 이 함수를 직접 쓰지 않고 미들웨어의 `x-fp-guest`를 본다.
 * 공개 메인과 레거시 업무 경로의 chrome 판정을 섞지 않기 위한 경계다.
 */
