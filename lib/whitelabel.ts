...
export type Whitelabel = {
  ...
  providerCode?: string;
  /**
   * **이 가게가 ERP 에서 «누구의» 것인가** — 그 회사 코드(`company_code`).
   *
   * ★★쓰는 데는 하나다 — **빠른조건을 «그 회사 사람»이 고칠 수 있게 하는 것**
   *   (사장님 2026-09-10 「퀵필터를 수정할 수 있게」 · 「**영업자·직원이 채널별로**」).
   *   손님 화면에는 아무 영향이 없다 — 이 값이 있든 없든 손님이 보는 것은 같다.
   * ⚠ **채널 key(`uniplan` 등)와 다른 값이다.** key 는 우리 표의 이름이고, 이건 ERP 안에서
   *   그 회사가 달고 다니는 코드다. 둘을 같은 것으로 보면 영업자가 제 가게도 못 고친다.
   * ⚠⚠ **`agent_channel_code` 가 아니라 «회사»다.** 채널 코드는 SP999 계열에서 «사람 코드»로
   *   채워 둔 자리다(`lib/domain/deal.ts` 「agent_channel_code = uid — 채널 관리자 경쟁사 열람
   *   방지」 · `scripts/backfill-agent-channels.mts`). 그걸로 맞대면 **같은 회사 직원인데도
   *   사람마다 열리고 닫힌다.** 「이 가게는 이 회사 것」은 회사 코드로 말한다.
   * ⚠ **비어 있으면 그 가게는 관리자만 고친다** — 「누구 것인지 모르는 가게」를 아무 영업자에게나
   *   열어 줄 수는 없다. 회사 코드를 확인한 «뒤에» 이 한 줄을 적는다(추정해서 적지 않는다 —
   *   틀린 코드를 적으면 남의 회사 사람이 이 가게 첫 줄을 고치게 된다).
   */
