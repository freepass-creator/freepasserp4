...
export type Whitelabel = {
   ...
   *   ⇒ 대신 **두 표기를 가른다** — 지금은 간판이 «BI»(`freepasserp.com`), 동반이 «법인 영문 CI»
   *     (freepassmobility)다. 둘 다 CI 센터 정본이라 지어낸 것이 아니고, 글자가 갈리니
   *     「A ✕ B」 자리가 눈에 보인다.
   *   ⚠ 여기 「간판은 국문(프리패스모빌리티)」이라고 적혀 있었다 — 2026-09-09 에 사장님이
   *     뒤집으셨다(「freepasserp.com 으로 브랜드로고 바꿔주라」). 근거는 그 채널 줄의
   *     `wordmark` 머리말이 정본이다. 옛 말을 여기 남겨 두면 두 곳이 서로 다른 간판을 말한다.
...
export const WHITELABELS: Whitelabel[] = [
   ...
   */
  {
     ...
     */
    /*
     * ★★**번호는 «폰의 문의하기»가 쓴다 — 머리띠는 안 쓴다**(사장님 2026-09-16
     *   「표준라벨은 **날짜 시간 날씨**로 하고 **모바일에서 문의하기 = 박영협 업무폰번호**」).
     *
     * ⚠⚠ 전에는 여기가 `''` 였고, 머리말에 「`tel` 까지 비웠으니 머리띠 오른쪽에는 날짜가 선다」고
     *   적혀 있었다. **번호를 비워서 날짜를 세우는** 방식이었던 것이다. 그래서 폰에서는
     *   하단 문의독이 아예 안 떴다 — 손님이 이 얼굴에서 연락할 길이 없었다.
     * ⇒ 두 일을 가른다. 번호는 채운다(폰 문의하기가 쓴다). 머리띠가 날짜를 세우는 근거는
     *   「번호가 없어서」가 아니라 **「우리 얼굴이라서」**(`self`)다 — `WhitelabelFrame` 참고.
     * ★영업채널은 이 번호를 안 쓴다. 저마다 **제 대표번호**를 든다(아래 채널 줄들의 `tel`).
     */
    tel: CORP.mobile,
    bizLines: [],
    /*
     * ⚠ 「회사 소개」 링크는 **안 단다** — 누르면 프리패스 소개로 나가서 라벨이 드러난다.
     *   머리띠 오른쪽에는 **날짜·시각·날씨**가 선다(근거는 `self` — 위 `tel` 머리말).
     */
    /*
   ...
   */
  {
    ...
    /*
     * ★우리 채널이다 — 갈리는 것은 푸터의 **「양사 협약」 문장 하나**뿐이다(주인이 하나면 「양사」가 없다).
     * ⚠⚠ **여기 「동반 표기(「✕ freepass」)를 «안» 그린다」고 적혀 있었다** — 코드와 정반대다.
     *   실제로는 우리 가게에도 **그린다**(사장님 2026-09-07 「우리거에도 붙여야지 남들한테
     *   보여줄 거야 **여기에 당신들 브랜드 들어가는 자리라고**」). 이 화면은 우리 가게이면서
     *   영업채널에게 보여 주는 **견본**이라, 그 자리가 비면 「여기가 당신들 자리」를 말할 수 없다.
     * ★고친 이유 = 이 한 줄이 **다음 사람에게 「코드가 틀렸다」고 말한다.** 주석을 믿고 코드를
     *   맞추면 사장님이 세우라고 하신 견본 자리가 사라진다. 판정 근거는 `self` 머리말이 정본이다.
     */
    self: true,
   ...
   */
  {
    ...
    name: '차슝',
    /*
     * ★★**이름 자체가 그림인 회사다 — 간판을 «통째로» 가져온다**(하허호와 같은 짜임).
     *
     * ⚠ 처음엔 글자로 세웠다(`wordmark: { main:'차슝', sub:'CHASHOONG' }`). **틀렸다.**
     *   사장님 2026-09-10 「**차슝 그 이미지를 그대로 갖고 와야 되지 않을까? 너무 텍스트로
     *   차슝 쓴 거 같은데?**」 — 맞는 지적이다. 차슝 간판은 **초록 라운드 뱃지(흰 자동차 · S곡선 ·
     *   속도선) + 굵은 한글 로고타이프 + 자간 넓은 영문**이 한 덩어리로 그려진 것이라,
     *   본문 서체로 흉내 내는 순간 그 회사 CI 가 아니다 — 위 `logo.role` 머리말이 말하는 그 경우다.
     * ⇒ `role: 'lockup'` — 그림이 **워드마크를 대신한다.** 그래서 아래 `wordmark` 는 비운다.
     *
     * ★그림은 그 회사 홈페이지 간판 원본 그대로다(344×128). **이미 RGBA 이고 여백이 0**이라
     *   손댈 것이 없었다 — 잉크가 네 변에 닿는다(`check:brand` 규격을 그대로 통과).
     * ★`nameBand` = 「차슝」 글자 **85px** ÷ 그림 세로 **128px** = **0.664**(2026-09-10 실측).
     *   맞추는 것은 그림이 아니라 «글자»다 — 안 그러면 뱃지가 세로를 먹고 이름만 작아진다
     *   (하허호에서 실제로 그랬다).
     *   잰 값: 뱃지 x0~126 · 글자 x146~343 · 「차슝」 y6~90 · 「CHASHOONG」 y107~121.
     */
    logo: { src: '/brand/chashoong-lockup.png', alt: '차슝', role: 'lockup', nameBand: 85 / 128 },
    /** 간판이 그림이라 글자는 안 그린다(위 `role: 'lockup'`). 이름은 `name` 이 갖는다. */
    wordmark: { main: '', sub: '' },
    /*
  ...
  },
  {
     ...
     * ★`nameBand`(2026-09-12 실측) — 오른쪽 글자 블록(x248~621)을 가로줄로 잘라 보면 세 줄이다:
     *   「KR」 y2~86(높이85) · 「Auto Plan」 y109~170(높이62) · 「KOREA ROAD」 y203~229(높이27,
     *   자간 넓은 작은 대문자 — 하허호의 「무심사」 뱃지처럼 «이름»이 아니라 부제).
     *   이름 두 줄(KR + Auto Plan)을 합친 높이 = y2~170 = 169. 169 ÷ 230 = **0.735**.
     */
    logo: { src: '/brand/krautoplan-mark.png', alt: 'KR오토플랜', role: 'lockup', nameBand: 169 / 230 },
    /* 간판이 그림이라 글자는 안 그린다(위 `role: 'lockup'`) — 이름은 `name` 이 갖는다. */
    wordmark: { main: '', sub: '' },
    brandColor: '#17273F',
    tel: '1811-7570',
    ...
    /*
     * ★값은 그 회사 홈페이지(krautoplan.com)가 실제로 하는 말이다(2026-09-12 확인) —
     *   「즉시출고! 상담부터 계약, 인도까지 7일이면 OK」·「전차종 초기비용 없이 무보증 신차장기렌트
     *   가능」·「중간유통과정없는 다이렉트견적」. ⚠ 「무심사」·「신용조회 없이」는 그 사이트에서
     *   확인되지 않아 적지 않는다(지어내지 않는다 — 유니오토·하허호 문구 머리말과 같은 원칙).
     */
    notice: {
      title: '7일이면 즉시출고됩니다',
      body: '초기비용 없이 무보증으로 신차 장기렌트를 이용하실 수 있습니다.',
    },
  },
  {
    key: 'carping',
    ...
    /*
     * ★`nameBand`(2026-09-12 실측) — 「CARPING」 글자만 있는 열(x793~1083, 카트 마크가 안 겹치는
     *   구간)을 가로줄로 자르면 두 줄이다: 「CARPING」 y60~182(높이123) · 태그라인
     *   「신차·장기렌트·리스 카핑」 y227~288(높이62, 작은 글자 — «이름»이 아니라 부제).
     *   123 ÷ 289(그림 전체 세로) = **0.426**.
     */
    logo: { src: '/brand/carping-mark.png', alt: '카핑', role: 'lockup', nameBand: 123 / 289 },
    /* 간판이 그림이라 글자는 안 그린다(위 `role: 'lockup'`) — 이름은 `name` 이 갖는다. */
    wordmark: { main: '', sub: '' },
    brandColor: '#3877E2',
    tel: '1661-1822',
    ...
    /*
     * ⚠⚠ **2026-09-12 검토 중 발견 — 위 사업자 표기가 그 회사 «공식 사이트»(carping1.com)와
     *   다르다.** 그 사이트 바닥글: 「상호명 카핑(carping) · 사업자등록번호 310-10-93045 ·
     *   통신판매업신고 2023-경기부천-0547호 · 대표자 지홍석외 1명 · 경기도 부천시 부일로
     *   205번길 46, 202호(상동, 윌타운) · 상담전화 1688-1362」 — 위 값(165-88-01681 ·
     *   1661-1822 · 2층 203호)과 사업자등록번호·전화·호실이 갈린다.
     *   ★**손대지 않았다.** 법적 표기(사업자등록번호)라 짐작으로 덮어쓰지 않는다 — 어느 쪽이
     *   맞는지(같은 회사의 다른 시기 값인지, 다른 법인인지) 확인이 먼저다. 사장님 확인 전까지
     *   이 값을 그대로 둔다.
     */
    /* 영업채널이라 재고 전체를 판다 — providerCode 를 비워 둔다. */
    /* 빠른조건·축은 안 적는다 = 집 기본. 그 채널 재고를 보고 화면에서 고친다. */
    /*
     * ★값은 그 회사 홈페이지(carping1.com)가 실제로 하는 말이다(2026-09-12 확인) —
     *   「신청하고 5일이면 내 집앞에 딱」·「썬팅 기본장착·블랙박스 기본장착(2채널)」·
     *   「새차에 필요한 차량용품 패키지 제공」. ⚠ 「무심사」·「신용조회 없이」는 그 사이트에서
     *   확인되지 않아 적지 않는다(지어내지 않는다).
     */
    notice: {
      title: '5일이면 새 차가 집 앞에 옵니다',
      body: '썬팅·블랙박스를 기본으로 장착하고, 필요한 차량용품까지 챙겨 드립니다.',
    },
  },
  {
    key: 'siauto',
     ...
     */
    /*
     * ★`nameBand`(2026-09-12 실측) — 심볼 오른쪽(x60~226)을 가로줄로 자르면 두 줄이다:
     *   작은 태그라인 「auto financial service」 y7~16(높이10) · 큰 글자 「빌려탈CAR」
     *   y22~52(높이31). 이 그림에는 「SI오토」라는 이름 글자가 그대로는 없다 — 심볼이 「SI」와
     *   「에스아이」(이름의 줄임)를 이미 들고, 나머지 자리에서 가장 크고 읽히는 말은
     *   「빌려탈CAR」다(그 회사가 자기 서비스를 부르는 말 — 위 `alt` 참고). ⚠ **판단이 갈릴 수
     *   있는 값이다** — 「SI오토」 글자가 그림에 없어 다른 채널(하허호·차슝·KR오토플랜·카핑)처럼
     *   «이름 글자 자체의 비율»로 재지 못했다. 큰 글자만의 비율로 뒀다: 31 ÷ 62 = **0.5**.
     */
    logo: { src: '/brand/siauto-mark.png', alt: 'SI오토 빌려탈CAR', role: 'lockup', nameBand: 31 / 62 },
    /* 간판이 그림이라 글자는 안 그린다(위 `role: 'lockup'`) — 이름은 `name` 이 갖는다. */
    wordmark: { main: '', sub: '' },
    brandColor: '#E60012',
    tel: '1644-9457',
    ...
    /*
     * ★값은 그 회사 홈페이지(siautocar.quv.kr)가 실제로 하는 말이다(2026-09-12 확인) —
     *   「초기비용 Zero」·「세금 절감 효과」·「보험료 할증 No — 사고 처리를 렌트금융사가 대행」·
     *   「저렴한 렌탈료」. ⚠ 「무심사」·「신용조회 없이」는 그 사이트에서 확인되지 않아 적지
     *   않는다(지어내지 않는다).
     */
    notice: {
      title: '초기비용 없이 신차를 이용하실 수 있습니다',
      body: '세금 절감 효과가 있고, 사고 처리를 회사가 대행해 보험료 할증 부담이 없습니다.',
    },
    /* 영업채널이라 재고 전체를 판다 — providerCode 를 비워 둔다. */
    /* 빠른조건·축은 안 적는다 = 집 기본. 그 채널 재고를 보고 화면에서 고친다. */
  },
