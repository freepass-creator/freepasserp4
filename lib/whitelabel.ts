...
export type Whitelabel = {
  ...
  plain?: boolean;
  /**
   * **머리띠 오른쪽에 «오늘 날짜»를 세운다.** 라벨 없는 얼굴의 그 자리다.
   *
   * 사장님 2026-09-09 「**우측 상단에는 날짜 정도는 있어야겠지**」.
   * ★그 자리는 원래 「회사 소개」(우리 가게) 또는 「상담 및 문의 + 번호」(채널)가 든다.
   *   라벨을 안 다는 얼굴은 둘 다 없어 **비어 있었다.** 날짜가 그 자리를 받는다.
   * ★★**날짜를 표에 적지 않는다** — 「오늘」이라 화면이 그때그때 잰다(`lib/format` `todayKo`).
   *   여기 값을 박아 두면 그날 하루만 맞고 그 뒤로는 틀린 날짜가 박혀 있게 된다.
   * ⚠ **폰에는 안 선다** — 거기 오른쪽은 검색·조건 단추 자리다(웹 머리띠만).
   */
  stampToday?: boolean;
  /**
...
export const WHITELABELS: Whitelabel[] = [
   ...
   */
  {
    ...
    domainReady: true,
    /*
     * ★★**임시 문구다 — 이름은 사장님이 고민 중**(2026-09-09 「뭐 일단 아무거나로 해놓고
     *   **이름은 고민 좀 해 볼게**」). 정해지면 **이 한 줄만** 바꾸면 된다.
     * ★지금 값은 검색결과 설명·탭 제목과 같은 말(`BRAND_TAGLINE`)이라, 화면과 메타가 한 말을 한다.
     */
     ...
     */
    /**
     * ★★**우측 상단은 «날짜»가 받는다**(사장님 2026-09-09 「**우측 상단에는 날짜 정도는
     *   있어야겠지**」). 상담 번호가 비운 그 자리다 — 자리를 없앤 게 아니라 **말을 바꾼 것**이다.
     * ★날짜가 서면 이 판은 광고가 아니라 **재고표**로 읽힌다 — 이 얼굴이 노리는 그 느낌이다.
     * ⚠ 값은 표에 없다 — 「오늘」이라 **화면이 그때그때 잰다**(`lib/shop/head-status`).
     *   여기 날짜를 적어 두면 그날 하루만 맞고 그 뒤로는 «틀린 날짜»가 박혀 있게 된다.
     * ★★**두 줄이다**(2026-09-09) — 위는 「오늘 9. 9.(화) · 맑음 27°」, 아래는
     *   「**update 14:23**」(재고를 마지막으로 갱신한 시각. 사장님 「재고 기준이라고 하지 말고 ·
     *   update 언제 시간분까지」). 갱신 시각은 **자동동기가 회차를 닫은 시각**을 읽는다 —
     *   재고 원자에는 「언제 갱신됐나」가 없어서(2026-09-09 실측) 그것이 유일하게 정직한 출처다.
     * ⚠ 그 값이 없으면 **아랫줄을 아예 안 그린다.** 오늘 날짜로 대신 채우면 「방금 갱신됨」처럼
     *   읽혀, 영업자가 그걸 믿고 손님에게 말한다.
     */
    stampToday: true,
    /*
