 ...
 * ★`app/layout.tsx` 가 Exo 2 300·500·600 을 이미 싣고 있다. 여기 이 상수를 쓰면 된다.
 * ★소문자다. `freepass` 는 언제나 소문자로 적는다(명함·로그인 화면과 같다).
 */
 ...
 *
 * ★왜 여기 두나 — 마크를 «색을 입혀» 그려야 하는 자리가 생겼다(손님 홈페이지 머리띠의 동반
 *   표기는 아주 연한 톤이라 남색 원본을 그대로 못 쓴다). `<img src="/icon.svg">` 로는 톤을
 *   못 바꾼다. 그렇다고 화면마다 path 를 베껴 적으면 아이콘과 마크가 갈린다.
