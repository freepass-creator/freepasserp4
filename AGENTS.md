...
> ## 현재 정본 범위 — 2026-09-26 (아래 과거 규칙보다 우선)
> **FreePassERP.com v1은 White Label 공개 상품 조회/검색만 담당한다.** 기본 진입도 White Label 상품 화면이다.
> 데이터는 **FreePass Data의 consumer**로 받는다. 현재 검증된 Firestore reader는 cutover 전 호환 구현일 뿐이며, 화면이 저장소 세대나 RTDB에 직접 결합하면 안 된다.
> **이 저장소의 신규 개발 범위가 아닌 것:** F01/F86 판매시트 개별 운영·발행, 견적기, 내부 ERP 업무화면/로그인, 접수·정산 별도 시스템, 차종마스터·원천수집·ERP5 migration, RTDB 신규 사용/재활성화.
> 아래 문서에 위 항목의 과거 운영 설명이 남아 있어도 **역사 기록이지 작업 지시가 아니다.** 이 범위를 되살리거나 별도 구현선을 만들지 않는다.
> UI/UX는 현재 White Label 정본을 계속 고도화하며, 채널 차이는 `lib/whitelabel.ts`의 허용 필드만 사용한다. 채널별 별도 화면 구현 금지.
> **개발선은 `main` + 고정 U/F/E/I 4개 작업선만 사용한다.** 브랜치와 책임은 `docs/UFEI-OPERATING-MODEL.md`가 정본이다. U/F/E/I는 운영 쓰기를 하지 않고 PR로만 main에 합친다. 임시 5번째 개발선·v2·대체 엔진·대체 어댑터를 만들지 않는다.
> **예외:** `erp5-ssot-refresh.yml`·`erp5-ssot-watchdog.yml`은 FreePass Data production cutover 전까지 운영 연속성을 위해 남은 임시 bridge다. 이 저장소의 제품 기능으로 확장하지 말고, 장애복구·관측·이관/종료 외 변경을 금지한다.
