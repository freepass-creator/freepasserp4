...
function 값맵(c, 분류) {
  ...
  const dep = c.보증금 || {};
  const 중고렌트 = c.원천버킷 === 'LOW_SONOKONG_DAILY';
  const 상품분류 = 중고렌트 ? '중고렌트' : 분류;
  const 상태 = 손오공기본상태(c);
  ...
  return {
    IDENTITY: {
      '차량번호': c.차번, '분류': 상품분류, '제조사': c.제조사, '모델명': c.모델,
      '차명(세부모델+트림)': c.차명, '외부색상': c.외장, '연식': String(c.연식 || '').slice(0, 4),
      '연료': c.연료, '배기량': c.배기량,
      // 중고렌트는 ERP 상세 RENT_* 보증금 원문, 구독/픽업만 연수×대여료 규칙.
      '보증금 반납형': 중고렌트 ? (dep.RENT_RETURN ?? '') : '월 대여료 × 약정연수 (최대 3개월)',
      '보증금 인수형': 중고렌트 ? (dep.RENT_BUYOUT ?? '') : '월 대여료 × 약정연수 (최대 3개월)',
    },
    ALWAYS: {
