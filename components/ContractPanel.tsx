...
import { Check, CircleX, RefreshCw, Send, X } from 'lucide-react';
    ...
    <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 9 }}>
      ...
      {needsFinalize && (
        <div style={{ border: `1px solid ${C.warn}`, borderRadius: R, padding: '9px 10px', background: C.warnBg, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {(role === 'admin' || role === 'provider') && <Btn mobileIcon={<RefreshCw size={18} />} title="완료 처리 재시도" size="sm" onClick={retryFinalize} disabled={busy}>완료 처리 재시도</Btn>}
        </div>
      )}
                      ...
                      {!done && mine && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, paddingLeft: 32 }}>
                          ...
                          <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
                            ...
                            <Btn mobileIcon={<Check size={18} />} title="약정 완료" size="sm" onClick={doAgreement} disabled={busy || !cust.name.trim() || !cust.phone.trim()}>약정완료</Btn>
                          </div>
                        </div>
                      )}
