...
/**
 * ★**카드 하단 뱃지 = 심사조건이 맨 앞**(사장님 2026-08-07 「내가 분명히 하단 뱃지에 심사조건 맨 앞에
 *   넣으라고 했는데」).
 *
 *   **상세 머리**는 「출고상태 + 상품구분」 둘뿐이다(사장님 2026-08-23 「무심사/소득확인 이거는
 *   아래 하단에 조건으로 보내자 — 계약조건이니까. 위에는 출고상태랑 상품구분 뱃지만 두자」).
 *   그 규칙은 `badges(p, …, hideCredit = true, …)` 가 든다 — 머리에서는 심사를 아예 안 만든다.
 *   (전에는 `HEAD_BADGE_KEYS` 상수가 따로 있었는데 아무도 안 읽어 껍데기로 남아 있었다. 지웠다.)
 *
 *   카드 하단은 다르다. 목록을 훑는 사람이 제일 먼저 거르는 건 **살 수 있는 손님인가**,
 *   곧 심사조건이다. 무심사냐 소득확인이냐에 따라 아예 못 파는 손님이 갈린다.
 *   08-23 에 심사를 «상단에서» 뺀 것을 하단에서까지 뺀 것으로 잘못 넓혀 적용해, 카드 어디에도
 *   심사가 안 남았다(하단도 `hideCredit=true` 로 부르고 있었다).
 *   **상단에서 빼는 것과 하단에 세우는 것은 다른 결정이다.**
 *
 *   ⚠ 목록 카드·상세 머리·요약줄 세 곳이 각각 차례를 적어 어긋난 적이 있다(2026-08-23:
 *     상세만 고쳤더니 목록에 그대로 떴다). 그래서 차례는 여기서만 정한다 — 쓰는 쪽은 이 상수를 읽는다.
 *
 * ★**모바일 카드는 심사 하나만**(사장님 같은 날 「상품구분 뱃지와 차량상태 뱃지를 모바일에서는 빼는 게
 *   낫겠어 · 상세페이지에서 뱃지 확인하게끔」). 좁은 화면에 뱃지 셋이 서면 차명이 밀린다.
 *   출고상태·상품구분은 상세에서 확인한다 — 목록에서 거르는 값만 남긴다.
 */
export const LOWER_BADGE_KEYS = ['cd', 'st', 'pt'] as const;
export const LOWER_BADGE_KEYS_MOBILE = ['cd'] as const;
