...
const man = (n: number) => (n ? `${Math.round(n / 10000).toLocaleString('ko-KR')}만` : '');
const km = (n: number) => (n ? `${(n / 10000).toFixed(1)}만` : '');
...
const pickList = (cars: CarLite[], key: 'fuel' | 'cls' | 'product' | 'supplier') => {
...
export default function IntakeStation({ api, preview = false }: { api: BoardApi; preview?: boolean }) {
   ...
   *   지원금은 차번이 «없는» 것이 정상이다.
   */
  const DIRECT = ['차량 직접', '지원금'] as const;
  ...
  const empty = {
    ...
    term: '', rent: '', deposit: '', price: '', payKind: '일시납',
  ...
  }, [board, q, fuel, cls, prod, sup, maxRent, onlyOk]);
  ...
  const toIntake = () => {
    ...
    setF((o) => ({
      ...o,
      term: o.term || picked.term, rent: o.rent || String(picked.rent || ''), deposit: o.deposit || String(picked.deposit || ''),
  ...
  const openDirect = (kind: typeof DIRECT[number]) => {
    ...
    setF({ ...empty, product: kind === '지원금' ? '지원금' : '', channel: last.channel, agent: last.agent });
  };

  const useTerm = (term: string, rent: number, deposit: number) =>
    setF((o) => ({ ...o, term: term.split('_')[0], rent: String(rent || ''), deposit: String(deposit || '') }));

  const isAid = direct === '지원금' || S(f.product) === '지원금';
  ...
  const flip = async (r: Line, key: 'paper' | 'delivered', on: boolean) => {
    ...
    const res = await api.edit(r.id, { [key]: on });
    ...
    <div className="cl">
      ...
      {tab !== '접수' ? (
        ...
        <>
          ...
          <div className="cl-toolbar cl-cond">
            ...
            <label className="cl-cap">월대여료 ≤
              <input type="text" inputMode="numeric" value={maxRent} placeholder="700000"
                onChange={(e) => setMaxRent(e.target.value)} />
            </label>
            <label className="cl-chk">
              <input type="checkbox" checked={onlyOk} onChange={(e) => setOnlyOk(e.target.checked)} /> 출고가능만
            </label>
            <button type="button" className="cl-tb"
              onClick={() => { setQ(''); setFuel(''); setCls(''); setProd(''); setSup(''); setMaxRent(''); setOnlyOk(true); }}>
              <span className="cl-tbi">↺</span>조건 지우기
            </button>
            <span className="cl-tbsep" />
            {DIRECT.map((k) => (
              <button key={k} type="button" className={`cl-tb${direct === k ? ' on' : ''}`} onClick={() => openDirect(k)}>
                <span className="cl-tbi">+</span>{k}
              </button>
            ))}
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: 84 }}>차종</th>
                      <th style={{ width: 78 }}>트림</th>
                      <th className="cl-num" style={{ width: 44 }}>연식</th>
                      <th className="cl-num" style={{ width: 46 }}>주행</th>
                      ...
                      <th className="cl-num" style={{ width: 34 }}>기간</th>
                      <th className="cl-num" style={{ width: 74 }}>월대여료</th>
                      <th className="cl-num" style={{ width: 50 }}>보증금</th>
                    </tr>
                  </thead>
                  <tbody>
                    {hits.map((c) => (
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: 102 }}>공급사</th>
                      <th style={{ width: 90 }}>영업채널</th>
                  ...
                  <tbody>
                    ...
                    {board.intake.length === 0 && (
                      <tr><td colSpan={9} className="cl-note">접수된 줄이 없습니다</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              {(mode === '접수' && (picked || direct)) && (
                <>
                  <div className="cl-tree-head">
                    접수
                    <button type="button" className="cl-back"
                  ...
                  <div className="cl-form">
                      ...
                      <>
                        <div className="cl-fr"><label className="cl-must">※ 차량번호</label>
                          <input value={f.plate} onChange={(e) => set('plate', e.target.value)} placeholder="00가0000"
                            className={S(f.plate) ? '' : 'cl-empty'} /></div>
                    ...
                    <div className="cl-fr">
                      ...
                      {isAid
                        ? <input value={f.customer} onChange={(e) => set('customer', e.target.value)}
                            placeholder="사무실비 · 업무지원비" className={S(f.customer) ? '' : 'cl-empty'} />
                        : <input id="cl-ch-in" value={f.channel} onChange={(e) => set('channel', e.target.value)}
                            list="cl-ch" className={S(f.channel) ? '' : 'cl-empty'} />}
                    </div>
                    {!isAid && (
                    ...
                    <div className="cl-fr2">
                      <div><label>기간</label><input value={f.term} onChange={(e) => set('term', e.target.value)} /></div>
                      <div><label>납입</label>
                    ...
                    {more && (
                      <>
                        ...
                        <div className="cl-fr"><label>접수일</label>
