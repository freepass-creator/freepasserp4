...
import { C, R, Btn, FW, FS, SH, SCRIM } from '@/components/ui';
...
export function Toaster() {
  ...
  return (<>
    <div style={{
      ...
      {toasts.map((t) => (
        <div key={t.id} role="status" style={{ pointerEvents: 'auto', maxWidth: 'min(92vw, 440px)', padding: '10px 16px', borderRadius: R, fontSize: FS.body, fontWeight: FW.strong, color: C.taupeBg, background: BG[t.type], boxShadow: SH.menu, whiteSpace: 'pre-wrap', textAlign: 'center' }}>{t.msg}</div>
      ))}
    </div>
    {confirm && (
      <div
