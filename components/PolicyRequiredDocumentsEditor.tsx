...
import { Btn, ButtonLabel, C, FS, FW, ICON, Input, ListGroup, DetailRow } from '@/components/ui';
import {
...
export function PolicyRequiredDocumentsEditor({
  ...
  return (
    <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
      <div>
        <div style={{ fontSize: FS.sub, fontWeight: FW.title, color: C.ink }}>고객 추가 제출서류</div>
        <div style={{ marginTop: 3, fontSize: FS.cap, lineHeight: 1.55, color: C.mute }}>
          이 정책으로 계약할 때 고객에게 실제로 받을 서류만 정합니다. 운전면허증과 본인 셀카는 별도로 항상 받습니다.
        </div>
      </div>
      ...
      {documents.length ? documents.map((document, index) => disabled ? (
        <ListGroup key={document.key}>
          <DetailRow label={`${index + 1}. ${document.label}`} value={document.required ? '필수' : '선택'} />
          {document.note ? <DetailRow label="안내" value={document.note} stacked /> : null}
        </ListGroup>
      ) : (
        <div key={document.key} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 6, alignItems: 'end' }}>
          <label style={{ display: 'grid', gap: 3, fontSize: FS.cap, color: C.mute }}>
            서류명
            <Input value={document.label} onChange={(next) => { if (next.trim()) update(index, { label: next }); }} full />
          </label>
          <label style={{ display: 'grid', gap: 3, fontSize: FS.cap, color: C.mute }}>
            고객 안내
            <Input value={document.note} onChange={(next) => update(index, { note: next })} full />
          </label>
        ...
        <div style={{ fontSize: FS.cap, color: C.faint }}>추가 제출서류 없음</div>
      )}
