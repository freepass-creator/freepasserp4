...
export function WhitelabelFrame({
   ...
   * ★2026-09-19 안정화 잠금 — 웹·모바일 모두 상세에서는 같은 `headerLead`를 쓴다.
   *   목록은 「상품찾기」, 상세는 「상품 상세」라는 페이지 역할을 같은 위치·같은 타이포로 보여준다.
   *   오른쪽의 날짜·문의·공유 배치는 화면 폭 규칙대로 유지한다.
   */
      ...
      */}
      <header ref={headRef} style={{
        ...
        <div style={{
          ...
          {/* 상세는 웹·폰 모두 «이 화면의 이름»을 든다. 목록일 때만 채널/표준 간판을 쓴다. */}
          {/*
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 24px 32px' }}>
          ...
          */}
          ...
          <div style={{ fontSize: SHOP.fs.sub, color: C.faint, lineHeight: 1.9 }}>
            ...
            */}
            {/*
              ★★★**«협약»으로 적는다 — 우리가 «해 주는» 게 아니라 둘이 «맺은» 것이다**
                (사장님 2026-09-07 「유니오토와 **협약을 통해서 전략적 파트너십 구성으로** 이거
                제공해 주는 거로 해야지, 좀 멋있게 아래쪽에」).

              ⚠ 여기는 두 번 틀렸다.
                ㉠ 「이 홈페이지는 ○○가 **운영합니다**」 — 법적 고지처럼 읽혀 우리 이름이
                   «관리자»로만 섰다. 무엇을 주는지 한 마디도 안 했다.
                ㉡ 그래서 「차량과 계약 시스템을 **제공합니다**」로 고쳤는데, 이번엔 **관계가
                   기울었다** — 우리가 위에서 내려 주는 꼴이라 채널이 «받는 쪽»으로 읽힌다.
                   채널은 제 이름을 걸고 파는 사업자지 우리 대리점이 아니다.
              ⇒ **양사 협약.** 머리띠의 「유니오토모빌 ✕ freepass」와 아래에서 짝을 이룬다 —
                위에서 한 번 나란히 서고, 아래에서 그게 «무슨 사이인지»를 밝힌다.

              ★★**연하게 쓴다**(사장님, 같은 날 「**진하게 쓰지 말고 연하게 써**」).
              ⚠ 바로 앞에 「한 단 진하게」로 올렸다가 되돌린다. 그때 이유는 「안 보인다」였는데,
                안 보이던 진짜 원인은 **굵기가 아니라 말**이었다(고지문이라 눈이 안 머물렀다).
...
function WhitelabelNotice({ wl, mobile }: { wl: Whitelabel; mobile: boolean }) {
   ...
   *   (표 `notice` 머리말 — 「글이냐 그림이냐 둘 중 하나」.)
   */
  const img = notice?.image;
  const hasText = !!(notice?.title || notice?.body);
  if (!notice || closed || (!img && !hasText)) return null;
  ...
  /*
   * ★★**면 «전체»가 누르는 자리다**(사장님 2026-09-18 「어떤 상품이 좋다 · 무보증 상품이 있다 …
   *   제대로 활용할 수 있게끔」). 주소가 곧 조건이라 `/?perk=무보증` 하나면 그 차들 앞에 내려놓는다.
   * ⚠ 닫는 단추는 이 링크 «밖»에 둔다 — 안에 넣으면 닫으려다 목록으로 끌려간다.
   */
  const href = notice.link?.href;
  const 안쪽 = (
    <>
      {img ? (
        /*
         * ★그림 배너 — 웹·폰 두 벌(표 `image` 머리말). 폰 것이 없으면 웹 것을 쓴다.
         * ⚠ `next/image` 를 안 쓴다 — 크기를 미리 모르는 «채널이 넣는 그림»이라 width/height 를
         *   박을 수 없고, fill 로 두면 비율을 여기서 정해야 해서 채널마다 다른 그림이 잘린다.
         *   ⇒ 제 비율대로 폭에 맞춰 눕힌다. 첫 화면이라 `eager` 로 받는다.
         */
        <img
          src={(mobile && img.mobile) || img.web}
          alt={img.alt}
          loading="eager"
          style={{ display: 'block', width: '100%', height: 'auto', borderRadius: SHOP.r.card }}
        />
      ) : (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginTop: 8 }}>
            ...
            {notice.link?.label ? (
              /* ★꼬리 링크는 «글 배너»에만. 그림은 면이 곧 단추라 같은 말을 두 번 하는 셈이다. */
              <span style={{ fontSize: SHOP.fs.body, fontWeight: FW.title, color: C.brand, whiteSpace: 'nowrap' }}>
                {notice.link.label} ›
              </span>
            ) : null}
          </div>
  ...
  return (
    ...
    <div style={{ background: C.brandSoft }}>
      ...
      <div style={{
        ...
        padding: img
          ? (mobile ? '12px 16px' : '16px 24px')
          : (mobile ? '12px 16px 12px' : '24px 24px 24px'),
      }}>
        {href ? (
