            ...
            */}
            {/*
              ...
              ★그렇다고 크게 키우지는 않는다 — 글자 크기는 그대로(`sub`)고 색만 한 단 올린다.
                간판의 주인은 채널이고 우리는 여기서만 이름을 낸다.

              ★★★**「운영합니다」가 아니라 «무엇을 주는가»를 말한다**(사장님 2026-09-07
                「이렇게 하면 뭐 좀 그러니까… **더 좋은 말 없냐? 우리가 이걸 왜 만들어주겠어??**」).
              ⚠ 「이 홈페이지는 ○○가 운영합니다」는 **법적 고지처럼** 읽힌다 — 「누가 책임지나」만
                말하고 우리가 무엇을 주는지는 한 마디도 안 한다. 그러면 우리 이름이 «관리자»로만 선다.
              ★우리가 실제로 주는 것은 셋이다 — **차량(재고 전체)** · **계약·정산 시스템** · 무심사 조건.
                손님이 궁금한 것도 같다: 「이 차들 어디서 오지? 계약은 누가 책임지지?」
              ⇒ 「**차량과 계약 시스템은 프리패스모빌리티가 제공합니다**」 —
                손님의 진짜 물음에 답하면서 우리가 왜 이 판을 깔았는지도 같이 말한다.
              ⚠ 「함께합니다」류로 흐리지 않는다 — 누가 책임지는지가 안 보이면 안 쓰느니만 못하다.
            */}
            <div style={{ marginTop: SHOP.sp.cozy, color: C.mute }}>
              이 홈페이지의 <strong style={{ fontWeight: FW.title, color: C.ink }}>차량과 계약 시스템</strong>은{' '}
              <strong style={{ fontWeight: FW.title, color: C.ink }}>{CORP.name}</strong>가 제공합니다.
            </div>
          </div>
          {/*
