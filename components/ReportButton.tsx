  ...
  if (!open) {
    return (
      <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        ...
        <Btn title="상품 검수 요청" variant="ghost" size="sm" onClick={() => setOpen(true)}>⚑ 검수 요청</Btn>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
        ...
        <Btn title={busy ? '검수 요청 접수 중' : '검수 요청 보내기'} size="sm" onClick={submit} disabled={busy}>{busy ? '접수 중…' : '요청 보내기'}</Btn>
      </div>
    </div>
  );
}
