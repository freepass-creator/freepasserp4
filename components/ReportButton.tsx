...
export function ReportButton({ p, compact }: { p: EntityRecord; compact?: boolean }) {
  ...
  const submit = async () => {
    setBusy(true);
    try { await submitReport(p, reason, memo.trim()); toast('수정 요청 접수됨 — 관리자·공급사에 전달됩니다', 'ok'); setOpen(false); setMemo(''); }
  ...
  return (
    <div style={{ border: `1px solid ${C.line}`, borderRadius: 6, background: '#fff7ed', padding: 10, display: 'flex', flexDirection: 'column', gap: 6, maxWidth: 340 }}>
      <div style={{ fontSize: 12.5, fontWeight: 700, color: '#9a3412' }}>매물 수정 요청 <span style={{ fontWeight: 400, color: C.faint }}>· 공급사·관리자에게 전달</span></div>
