...
export const SHOP = {
  /**
   * 상단바 타이포 — 웹/모바일 공통.
   * 페이지 폭이 달라도 서비스 이름·상세 머리의 위계는 바뀌지 않는다.
   */
  top: { title: 18, cobrand: 12 },
   ...
   *   조건 아홉 개가 한 줄에 서는 자리라, 한 칸이 4px 낮아지면 그 줄 전체가 눈에 띄게 가벼워진다.
...
export const PHOTO_SIZES = {
  /**
   * 목록 카드 — 폰은 화면 폭, **웹은 «px 로 못 박는다»**.
   *
   * ⚠⚠ 2026-09-17 운영 실측 — 웹 1440 에서 카드 칸은 **297px** 인데 `33vw`(=475px)라 적혀 있었다.
   * ★`33vw` 가 틀린 이유 — 웹 목록은 «화면 폭의 1/3»이 아니다. 본문이 1280 에서 멈추고
   *   왼쪽 조건칸(260)과 칸사이(32)를 뺀 나머지를 셋으로 나눈다:
   *   **(1280 − 48 − 260 − 32) ÷ 3 ≈ 313px** 이 «제일 넓을 때»이고, 창이 좁아지면 더 준다.
   *   그래서 화면이 아무리 넓어도 320 을 넘지 않는다 — 폭이 아니라 **상한**을 적는 자리다.
   * ★폰은 그대로 `100vw` 다(칸 343 / 화면 375 — 맞다).
   *
   * ⚠⚠⚠ **얼마나 줄어드나 — 「384 를 받는다」가 아니다. 배포 뒤 운영에서 다시 쟀다.**
   *   Next 는 `sizes` 안에 **vw 가 하나라도 있으면** 후보에서 `deviceSizes[0]`(=640) **밑을 전부
   *   버린다**(`getWidths` 의 `smallestRatio` 거르개). 우리 글줄은 폰 쪽에 `100vw` 가 있어
   *   비율이 1 이라, 바닥이 640 으로 잡힌다 — 실제 srcset 후보는 `640·750·828·1080…` 뿐이고
   *   **384 는 애초에 없다.**
   *   ⇒ 그래서 이 고침이 실제로 버는 곳은 **망막 화면(DPR 2)** 이다:
   *     · 고치기 전 — 475 × 2 = 950 → **1080** 을 받았다
   *     · 고친 뒤   — 320 × 2 = 640 → **640** 을 받는다 (픽셀로 -45%)
   *     · 보통 화면(DPR 1)은 320 → 640 으로 **전과 같다**(더 작은 후보가 없어서다)
   *   ★폰은 손대지 않았다 — 343 × 2 = 686 → 750 으로 전과 같고, 그게 맞는 값이다.
