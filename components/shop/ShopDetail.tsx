...
export function ShopDetail({ p, agentName, agentPhone, listHref = '/shop' }: {
  ...
  const specs: FactRow[] = grouped(
    [
       ...
       */
      /*
       * ★★**차급이 여기 있다 — 「위아래 4개씩」을 맞추려고**(사장님 2026-09-08 「야 **최초등록을
       *   빼 기타사항으로 보내고 차급을 넣어야겠네. 그럼 4개 4개**니까」).
       * ★뜻으로도 맞다 — 색상·차급은 「이 차가 무엇인가」고, 연식·주행거리는 「얼마나 탔나」다.
       *   넷이 「이 차의 겉과 이력」 한 묶음이 된다.
       * ⚠ **연식과 주행거리는 붙여 둔다**(사장님 2026-09-05 「연식, 주행거리 — 이게 «연식에 따른
       *   주행거리»를 보고」). 그래서 차급은 색상 «뒤»지 연식·주행 사이가 아니다.
       */
  ...
  const etcRows: FactRow[] = [
    /*
     * ★★**최초등록은 여기다**(사장님 2026-09-08 「최초등록을 빼 **기타사항으로 보내고**」).
     * ★차량 정보에 두면 「색상·차급·연식·주행거리」 넉 줄이 다섯이 되어 4/4 가 깨진다.
     *   그리고 손님이 «먼저» 보는 값도 아니다 — 연식을 보고 나서 더 알고 싶을 때 찾는 값이다.
     * ★덤 — 정책이 안 붙은 차(62%)는 이 구역이 통째로 비었는데, 96% 채워지는 이 한 줄이
    ...
    /*
     * ⚠⚠ **「대차」를 걷었다**(사장님 2026-09-16 「**다 빼**」 — 부정적인 값을 걷자는 그 줄기).
     *
     * ★실제 값이 거의 「불가」다(집 기본값도 `policy-defaults` 에서 「불가」 — 사장님 2026-08-19 표기).
     *   고르는 화면에서 「대차 불가」는 **읽는 사람의 손을 멈추게 하는 말**이고, 정작 그 값이
     *   필요한 순간은 «사고 난 뒤»다 — 그때는 계약서와 담당자가 답한다.
     * ★★**규격에도 맞다.** 이 칸은 `policy-tier` 에서 `exposure: 'contract'` 로 분류돼 있다 —
     *   계약서(제5조·제20조)에 나갈 값이지 카탈로그 값이 아니다. 여기 있던 것이 오히려 층을 넘은 것이다.
     * ⚠ 2026-08-20 에 「사고 나면 손님이 가장 먼저 묻는 값」으로 들어온 줄이다. 되살리려면
     *   여기 한 줄 **더하고 명단(`PUBLIC_POLICY_FIELDS`)에도 같이** 넣어야 한다 — 명단에서
     *   빠지면 값이 화면까지 오지 않는다. 전자계약·관리자 정책 화면은 그 명단을 안 타므로
     *   거기서는 그대로 보인다.
     */
    /*
      ...
      */}
      ...
      {hasPolicy ? (
        <p style={{ margin: `${SHOP.sp.part}px 0 0`, fontSize: SHOP.fs.cap, color: C.faint, lineHeight: 1.7 }}>
          {/*
            ★★**우리끼리 쓰는 말을 손님 화면에 내지 않는다**(사장님 2026-09-08 「그리고 **운영은
              뭐지??**」 — 「공급사가 제공한 **운영정책**」을 보시고 물으셨다).
            ⚠ 두 낱말이 다 우리 사정이다.
              · **운영정책** — 업계 안에서 쓰는 말이다. 손님은 「운영이 뭔데?」에서 멈춘다.
              · **공급사**   — 채널 이름으로 나가는 판에서 «뒤에 누가 있는지»를 굳이 꺼내면,
                답할 필요 없는 질문(그럼 파는 건 누구냐)을 손님 머리에 심는다.
            ⇒ 손님이 알아야 하는 것만 남긴다: **「지금 적힌 것은 계약 때 확정된다」**와
              **「더 궁금하면 담당자에게」**. 나머지는 우리 사정이다.
          */}
...
function Facts({ rows, cols, mobile, dense }: {
   ...
   * ⇒ **고정 격자.** 무리가 바뀌어도 칸의 x 자리는 그대로다.
   *
   * ★★★**넷으로 나눈다**(사장님 2026-09-08 「최초등록을 빼 기타사항으로 보내고 차급을 넣어야겠네.
   *   **그럼 4개 4개**니까」 · 「**위아래 4개씩이니까 맞추는 개념**으로 보는 건 좋은데」).
   * ⚠ 셋으로 나눴을 때는 두 무리가 3+1 · 3+2 로 갈려 **줄이 들쭉날쭉했다**(2026-09-08 실측 —
   *   「색상 연식 최초등록 / 주행거리 / 배기량 연료 구동방식 / 승차정원 차급」).
   *   혼자 남은 칸이 두 줄이나 생기면 격자가 «표»로 안 읽히고 흘리다 만 것으로 보인다.
   * ⇒ 무리를 넷씩으로 맞추고(색상·차급·연식·주행거리 / 배기량·연료·구동방식·승차정원)
   *   칸도 넷으로 나눈다. **두 줄이 정확히 한 줄씩 앉는다.**
   * ⚠ 09-07 에 「3분할」로 정한 것은 그때 무리가 «셋»이었기 때문이다(색상·연식·주행거리).
   *   숫자가 규격이 아니라 **「무리와 칸 수를 맞춘다」**가 규격이다 — 무리가 바뀌면 칸도 바뀐다.
 ...
 */
/*
 * ⚠⚠ **여기 「영업자 전용 칸」(`ShopInside` + `InsideBody`)이 있었다 — 걷었다**
 *   (사장님 2026-09-17 「화이트라벨에서 erp4 아예 분리해서 **로그인 기능 아예 없는 거야**」 ·
 *    공급사 칸은 「없앤다」 확정).
 *
 * ★그 칸은 2026-09-06 에 「영업자들만 보는 섹션 — 공급사가 어딘지」로 만든 것이고,
 *   **로그인한 사람에게만** 보이도록 문(`/api/shop/inside`)까지 따로 두고 있었다.
 *   손님 동에서 로그인을 통째로 걷었으므로 이 칸은 설 자리가 없다 — 로그인 없이 공급사를
 *   보여 주면 손님·경쟁사·영업채널이 다 본다(그 라우트가 403 으로 막던 바로 그것이다).
 * ★**공급사는 업무동에서 본다** — `/m/[code]` 상세가 그 자리다(거기는 로그인이 있다).
 * ⚠ 문(`app/api/shop/inside`)은 **그대로 둔다** — 업무동이 쓰고, 토큰 없이는 401 이다.
 */
...
function Missing() {
  return (
    <div style={{ fontSize: SHOP.fs.sub, color: C.faint, lineHeight: 1.7 }}>
      {/* ★같은 이유로 「공급사」를 안 쓴다(위 안내문 머리말). 「아직 안내 전」이 손님의 말이다. */}
      아직 안내가 준비되지 않았습니다. 담당자에게 확인해 주세요.
    </div>
  );
}
