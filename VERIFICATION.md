...
## 2026-08-31 전자계약 1~8 기능·오류 검증 (Codex)

- 정본: `docs/ESIGN-MANUAL.md`(기능)과 `docs/ESIGN-UIUX-SPEC.md`(읽기 전용 UI) 기준. 화면 컴포넌트는 수정하지 않았다.
- 수정: 완료본 스냅샷은 개인 `customer_birth`가 따로 없더라도 승인 직전 복호화된 주민등록번호에서 생년월일만 파생한다. 원문 주민등록번호는 공개 계약 데이터에 복사하지 않는다.
- 검증 보정: UI 문구 변경으로 오래된 정확 문구를 찾던 부속서류/A4-웹 대응 검사를, 기능 불변조건(필수·선택 서류, 약관 끝까지 읽기, 명시 최종 동의)으로 바꿨다. 화면 문구·배치는 바꾸지 않았다.
- PASS: 발송 전 차량·서식·정책 gate, 개인 7단계 키·43개 입력 검증, 법인 서명자/위임서류 승격, 계약·약관 62/62, 부속서류, 반려·만료·해지·진행 44/44, 서명 PNG, 승인·봉인 PDF, 공개 링크 no-store/noindex·만료/해지 차단·서버 암호화, TypeScript, UIUX drift, template drift, fonts drift.
- PDF: 테스트 봉인본은 A4 8장으로 생성됐고 144dpi 렌더 8장을 확인했다. 표지와 최종 동의·서명 페이지에 잘림은 없었다. 이 검증용 문서는 17장 운영 계약서가 아닌 8장 fixture이므로 운영 계약의 장수 보증으로 확대하지 않는다.
- 범위 제한: 실제 고객 개인정보 제출·실운영 관리자 승인/봉인은 권한 및 고위험 외부 변경이어서 수행하지 않았다. 이 경로는 관리자 계정으로 별도 테스트 계약을 만든 뒤 확인해야 한다.
- 교차검증: Claude Code는 사용량 제한, Gemini CLI는 이 저장소 비대화형 접근 제한, Cursor CLI는 읽기 전용 에이전트 실행 경로 부재로 독립 결과를 얻지 못했다. 따라서 재현 가능한 로컬 sim·타입·PDF 증거로만 이번 판정을 냈으며 다중 AI 게이트는 미충족으로 기록한다.

## 2026-08-31 상품 목록 뱃지 규격 재검증 (Codex)

- 기준: 상품 카드·목록에서는 대여료와 대여기간 외 상자형 뱃지를 쓰지 않는다. 차량상태·상품구분·심사·우대조건·프로모와 옵션은 아이콘+텍스트 또는 일반 텍스트로 표시한다. 필터·오류·건수·업무 상태 배지는 범위 밖이다.
- 수정: `ProductCard`/`ProductRowCard`/동기 미리보기 썸네일에서 남아 있던 사진 위 박스형 신호·프로모 API를 제거했다. 간단보기는 사진 우하에 출고상태→상품구분 `SignalMarks`, 심사는 대여료 아래 우대조건 줄의 첫 항목으로 유지한다. 엑셀형 목록의 상태·상품·심사·우대조건도 아이콘+텍스트로 통일했고, 옵션의 배경·둥근 모서리를 제거했다.
- 회귀 방지: `scripts/check-design-locked.mts`에 썸네일 레거시 API, 엑셀형 목록의 `Badge`, 텍스트만인 우대조건, 옵션 배경/반경 재도입을 실패시키는 규칙을 추가했다.
- PASS: `npm run typecheck`, `npm run check:fonts`, `npm run check:ui`, `npm run check:design`, `git diff --check`. 로컬 `/finder`는 HTTP 200으로 응답했고 상단 남색 헤더를 캡처 확인했다.
- 제한: 캡처 세션은 상품목록 권한이 없어 실제 카드 행을 렌더하지 못했다. Cursor 읽기 검토는 옵션 칩·엑셀 조건 텍스트 두 건을 지적했고 모두 반영 후 위 게이트를 재통과했다. Claude Code는 주간 제한, Gemini CLI는 이 환경의 읽기 도구 오류로 이번 독립 판정을 완료하지 못했다. 따라서 4역할 교차검증은 미충족이며, 배포 전 권한 있는 브라우저 세션의 카드 시각 확인이 남는다.

## 2026-08-31 웹 상세 차량스펙 공간 활용 (Codex)

- 기준: 넓은 웹 본문의 차량스펙은 라벨·값 두 쌍을 한 행에 놓고, 모바일은 한 쌍 한 행을 유지한다. 선택옵션은 첫 항목쌍 다음 전체 행이며 계약·보험 표는 바꾸지 않는다.
- 수정: 이미 `pair: true`로 선언돼 있던 차량스펙을 `ProductDetail` 렌더러가 실제로 소비하도록 했다. 홀수 마지막 항목은 값 칸이 남은 폭을 자연스럽게 차지한다.
- 회귀 방지: 확정 디자인 문서와 `check-design-locked`에 웹 4열/모바일 2열, 옵션 전체 행·위치를 고정했다.
- PASS: `npx tsc --noEmit`, `npm run check:design`, `npm run check:fonts`, `npm run check:ui`, 변경 대상 `git diff --check`. Next production build는 컴파일·타입검사·페이지 데이터 수집 단계까지 오류 없이 진행했다.
- 독립 검토: 별도 읽기 검토는 PASS(차량스펙만 4열, 모바일·계약·보험 회귀 없음). Claude Code는 주간 제한, Gemini CLI는 이 환경의 비대화형 실행이 결과를 반환하지 않아 4역할 교차검증은 미충족이다. 실제 로그인 데이터로 된 웹·모바일 시각 확인과 운영 배포는 아직 하지 않았다.

## 2026-08-31 간단보기 중고렌트 상품구분 아이콘 복구 (Codex)

- 원인: `SignalMarks`는 사진 위에서 상품명만 흰색으로 반전하고 아이콘은 상품 톤을 유지했다. 중고렌트의 회색 태그가 어두운 썸네일 그라디언트에 묻어 아이콘이 없는 것처럼 보였다.
- 수정: 사진 위 신호는 상태·상품구분 모두 흰 아이콘과 흰 글자로 반전한다. 본문·엑셀 목록의 상품구분 아이콘은 기존 상품 톤을 유지한다.
- 회귀 방지: 디자인 잠금 검사에 `onPhoto` 아이콘 반전 규칙을 추가했다.

## 2026-08-31 상세 사진 최근·관심 오버레이 제거 (Codex)

- 수정: 상세 큰 사진과 사진 없음 자리에서 최근 상태·관심(하트) 오버레이를 제거했다. 상세의 상태·상품구분은 제목 아래 정보 줄에 남아 사진 비교를 방해하지 않는다.
- 회귀 방지: 디자인 잠금 검사가 상세 컴포넌트의 사진 위 `ProductStateMarks`·`FavHeart` 재도입을 실패시킨다.

## 2026-08-31 상품찾기 도구줄 단순화 (Codex)

- 수정: 상품찾기에서 최근·관심 레일·버튼을 제거하고 해당 목록을 더 이상 읽거나 필터에 사용하지 않는다. 기존 로컬 저장 데이터와 다른 화면의 호환 경로는 삭제하지 않았다.
- 배치: 웹 검색줄은 `세부필터 → 검색창 → 간단/상세/엑셀`로 정리했다. 정렬의 인기차량순은 검색줄 Select를 없애고 퀵필터 맨 앞 `인기` 팝오버로 옮겼다.
- 검증: 비로그인 웹 캡처에서 위 배치·최근/관심 부재를 확인했다. 로그인 권한이 없어 실제 결과 행과 세부필터 선택은 시각 검증하지 못했다.

## 2026-08-31 상품찾기 단순화 운영 배포 (Codex)

- 운영 배포: `dpl_GhrZkYwLqr9AuA38vfGpYHLRMRML`이 Ready가 되었고 `https://freepasserp.com` 및 `https://www.freepasserp.com`에 연결됐다.
- 포함 범위: 상품찾기 도구줄 단순화, 카드/상세의 확정 뱃지·아이콘 규격, 웹 차량스펙 4열 활용만 반영했다. 작업 트리에 있던 전자계약·계약·정산의 별도 변경은 배포 원본에 포함하지 않았다.
- 배포 원본 게이트: `npm run typecheck`, `npm run check:design`, `npm run check:fonts`, `npm run check:ui` 모두 통과했다. 배포 후 `/finder`는 HTTP 200, Vercel 최근 오류 로그는 0건이다.
- 독립 검토: 별도 읽기 검토는 상품찾기 제거 범위·웹 도구줄 순서·`인기` 퀵필터 우선순위를 PASS로 판정했다. Claude Code는 사용량 제한, Gemini CLI는 결과를 반환하지 않아 네 역할 교차검증은 완료되지 않았다.
- 남은 범위: 계약·정산·재고의 실제 데이터·관리 기능은 보존했다. 영업자 전용 공유 플랫폼으로 더 줄이는 작업은 직접 URL 권한 차단과 메뉴/라우트 정책을 먼저 고정한 뒤 별도 검증으로 진행한다.

## 2026-08-31 상품찾기 퀵필터 인라인 배치 (Codex)

- 수정: 웹 툴바를 `세부필터 → 폭을 줄인 검색창 → 인기·모델·색상·기간 등 퀵필터 → 보기 전환`으로 배치했다. 모바일 흐름은 바꾸지 않았다.
- 좁은 웹: 퀵필터는 두 줄로 밀려 툴바가 깨지지 않고, 검색창 오른쪽 영역 안에서 가로 스크롤한다. 팝오버는 뷰포트 기준으로 열리고 스크롤 시 닫혀 잘림·위치 이탈을 막는다.
- 회귀 보정: 인기 정렬만 변경해도 초기화가 활성화되며, 퀵필터를 열면 세부필터를 먼저 닫아 두 팝오버가 동시에 남지 않는다.
- PASS: `npx tsc --noEmit`, `npm run check:design`, `npm run check:ui`, `npm run check:fonts`, 대상 `git diff --check`.
- 독립 검토: 별도 읽기 검토가 초기에는 줄바꿈·초기화·동시 팝오버 3건을 FAIL로 지적했고, 보정 후 PASS로 재판정했다. 로그인 세션 부재로 실제 결과가 있는 운영 화면의 시각 확인은 아직 하지 않았다.

## 2026-08-31 상품찾기 인라인 퀵필터·옵션칩 운영 배포 (Codex)

- 운영 배포: `dpl_BNWk7aCMtapjvRAjk5CbHSiuUTw4` Ready. `https://freepasserp.com` 및 `https://www.freepasserp.com` 별칭 전환을 확인했다.
- 포함 범위: 검색창 축소와 검색창 오른쪽 인라인 퀵필터, 우측 보기모드 유지, 선택옵션의 옵션칩 통일만 반영했다. 계약·정산·전자계약의 별도 작업 트리 변경은 포함하지 않았다.
- 배포 원본 게이트: `npm run typecheck`, `npm run check:design`, `npm run check:ui`, `npm run check:fonts` 모두 통과했다. 배포 후 `/finder` HTTP 200, 최근 Vercel error 로그 0건이다.
- 제한: 로그인 데이터가 있는 운영 화면의 시각 확인은 권한 세션이 없어 수행하지 못했다. Claude Code·Gemini CLI의 독립 결과도 이번 배포에서는 확보하지 못했다.

## 2026-08-31 상품찾기 초기화 위치·컨트롤 규격 운영 배포 (Codex)

- 운영 배포: `dpl_J2Jrgz7arDQ3REw7Ydi9jVhTu86p` Ready. `https://freepasserp.com` 및 `https://www.freepasserp.com` 별칭 전환을 확인했다.
- 포함 범위: 초기화를 심사조건 바로 뒤로 이동하고, 검색·필터·초기화·보기모드의 웹 공통 32px 컨트롤 토큰 적용만 반영했다.
- 검증: 배포 원본에서 `npm run typecheck`, `npm run check:design`, `npm run check:ui`, `npm run check:fonts` 통과. 배포 후 `/finder` HTTP 200, 최근 Vercel error 로그 0건.

## 2026-08-31 상품찾기 운영 배포 회귀 복구 (Codex)

- 원인: 앞선 CLI 배포 뒤 별도 production deployment `dpl_4A24Xd6EP2r6YJqDeV1XbEooWfQa`가 운영 별칭을 덮어, 이전의 2줄 툴바(독립 정렬 Select·최근 버튼)가 다시 노출됐다.
- 복구 배포: `dpl_8kywTmnqSKSp8uc6cYe4EyQDA1dq` Ready. 최신 Finder UI 8개 파일을 배포 원본과 SHA-256으로 대조한 뒤 운영 별칭을 전환했다.
- 실제 운영 검증: 새 인증 탭에서 `세부필터 → 검색창 → 상품 많은 순·퀵필터 → 우측 보기모드` 한 줄, 독립 정렬 Select 0개, 최근 버튼 0개를 확인했다. 684대 중 화면 카드 72개가 렌더됐고 브라우저 error 로그 0건이다.

## 2026-08-31 상품찾기 상품 선표시 운영 배포 (Codex)

- 운영 배포: `dpl_HnWBqqEmx21KZWsS7YXMeed3pag2` Ready, `https://freepasserp.com` 별칭 전환 확인.
- 수정: 상품과 공급사명을 같이 기다리던 조회를 분리했다. 상품과 실시간 차량상태는 먼저 표시하고, 공급사명만 후속 보정한다. 공급사 조회 실패도 이미 보이는 상품을 비우지 않는다.
- 검증: 배포 원본 typecheck·design·UI·fonts 통과, 배포 후 `/finder` HTTP 200·Vercel error 로그 0건. 새 인증 탭에서 10초 안에 카드 72개·브라우저 error 0건을 확인했다.
- 잔여: 완전히 빈 새 세션의 첫 product read는 여전히 약 10초가 걸렸다. 이번 변경은 그 뒤에 추가로 막히던 공급사명 조회를 제거한 것이며, 첫 RTDB 읽기 자체의 추가 최적화가 필요하다.
