...
- `components/shop/ShopDetail.tsx`
  - 선택 옵션을 차량명·사실줄 바로 아래로 옮기고 한 번만 표시했다. 옵션이 없는 차량은 해당 줄만 생략한다.
  - 대표 월 대여료·보증금·약정기간과 실제 전체 가격표를 한 `기간별 대여료` 섹션으로 합쳤다.
  - 차량 정보의 첫 식별 줄은 제조사·세부모델·세부트림으로 보강했다.
  - 실제 공급사 `price` 행만 사용하며, `vehicle_price`나 공식 근거 없는 신차가를 고객 화면에 추가하지 않았다.
- `docs/DESIGN_CONFIRMED_SHOP.md`와 `scripts/check-design-locked.mts`를 같은 기준으로 갱신했다. 이후 옵션 위치·중복과 요금 섹션 분리를 자동으로 차단한다.

### 실행 증거

- `npx tsc --noEmit` — PASS
- `npm run check:design` — PASS
- `npm run check:building` — PASS
- 실제 로컬 웹 `/q/9k7shqdjac-U0060?wl=uniplan` — PASS
  - 12·24·36·48·60개월 전체 행 표시 확인
  - 36개월 선택 시 대표 금액과 약정기간이 즉시 37만 2,000원·36개월로 갱신되는 것 확인
  - 옵션 없는 실제 차량 화면도 차량정보·기간표가 정상 표시되는 것 확인
- Gemini CLI 읽기 전용 독립 검토 — PASS
- Cursor Agent — **미검수**: 월 사용 한도 도달로 실행 거절됨.
- Claude Code — **미검수**: 읽기 전용 CLI 세션이 응답 없이 종료되어 결과를 얻지 못함.

### 남은 항목

- 폰트/토큰 전역 게이트는 기존 드리프트로 실패한다. 이번 변경 파일에는 기존 `ShopDetail.tsx`의 토큰 이탈 3건이 남아 있으나, 이번 작업에서 새로 추가한 항목은 아니다. 별도 규격 정리 범위로 분리한다.
- 운영 배포·외부 데이터 변경은 하지 않았다.
