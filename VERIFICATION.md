...
## 2026-08-10 기존 렌탈계약서 대체 보완·섹션 중복 감사

결과: **구조·코드 CONDITIONAL PASS / 법적·금전 문구 Claude·사람 승인 전 게시·발송·머지 NO-GO**

- 기존 계약서를 갈아엎지 않고 현재 프리패스 표준계약서의 제4·7·14·16·17·18·22조에 대체에 필요한 절차를 보완했다. 인쇄/PDF 정본과 착한거래 전송 약관은 전문 일치 검사를 추가했고 전송 약관 버전은 `rental-v3-2026-08-10`이다.
- 불필요한 값 중복 4종을 정리했다. 보험 가입 주체·보험료 포함 여부는 `04 자동차 보험`, 자차 자기부담률과 보험사 사고접수 번호는 보험 상세 한 곳, 자동이체일은 원본 필드 한 곳만 소유한다. 특약은 예외·추가 합의 전용으로 표시했다.
- 표지 요약과 차량 인수증·대여사실확인서·운전자격확인서·CMS·연대보증 등 독립 확인/서명 부속서의 식별값 반복은 의도된 반복으로 유지했다.
- PDF QA에서 정비·결제 페이지의 푸터만 다음 종이로 밀리던 빈 페이지를 제거하고, 약관 조문 제목과 첫 문단을 같이 배치해 고아 제목을 막았다. 최종 A4 PDF는 18쪽이며 전 페이지에서 잘림·겹침·빈 페이지·고아 제목이 없음을 확인했다.
- 회귀: `sim-esign-agreement` 49/49, `sim-esign-contract-kind` 55/55, `sim-chakhandeal-esign` 29/29 PASS. `npx tsc --noEmit`, `check:fonts`, production build 33/33 pages PASS.
- 계약 필드 감사는 합집합 174칸 중 기존 미연결 66칸을 계속 보고한다. 사용자가 이번 구조 비교에서 값/빈칸은 제외했으므로 새로 만들거나 채우지 않았고 총 미연결 수는 종전과 동일하다. UTF-8 BOM 때문에 중단되던 계약 원자 감사는 BOM을 허용하도록 보강했다.
- 검토 요청과 중복 판정표는 `docs/CONTRACT_REPLACEMENT_REVIEW_2026-08-10.md`에 남겼다. 특히 제14조 제5항의 신차 등록 후 취소 산정과 제4·7·16·17·18·22조 문구는 법적·금전 게이트 대상이다. 운영 데이터·전자계약 발송·배포는 변경하지 않았다.
