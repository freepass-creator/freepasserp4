...
## 2026-09-03 RTDB 비용 방어·운영 배포 (Codex)

- 운영 근거: Firebase Console에서 `freepasserp3` RTDB 다운로드가 최근 7일 47.4GB였고, live profiler에서 `v4/settlement_rows` 전체 읽기 3회·약 1.19MB를 관측했다. 사진 바이트는 RTDB가 아닌 별도 이미지 전송이다.
- 수정: 상품찾기·공유 상세에서는 TopBar/AppTabBar의 30초 메뉴 뱃지 폴링을 중지하고, 관리자 정산 뱃지를 위해 정산 원장 전체를 읽던 경로를 제거했다. 공유 상세의 브라우저 상태 폴링도 제거했으며, `/api/sheet/live-status`는 브라우저 요청으로 시트 동기화·상품 전체 transaction을 실행하지 않고 마지막 서버 상태만 반환한다.
