...
**Post-repin ERP5 validation은 HOLD다.** Run `35572350744`은 source recollection과 settlement→Atom contract lock까지 성공했지만 `원천에서 ERP5 현재 원자 계산`이 cancelled됐고 fixed snapshot, public catalog, F01/F86 publish, freshness, Atom↔F01↔F86 cross-audit, photo/evidence는 실행되지 않았다. AI Core shadow receipt 단계는 완료됐다. 이 run을 data corruption으로 확대 해석하지도, full PASS로 취급하지도 않는다. 새 pin의 운영 확정은 production-equivalent full pipeline green 뒤에만 한다.
...
**Dedicated SSOT Source Contract gate도 계속 HOLD다.** Re-pin 이후 run `35571946592`, PR #458 head `35572954687`, `35572965499`, later main `35573667738`가 pre-job failure로 끝났다. Generic CI는 이 전용 fail-closed gate를 대체하지 않는다. `.github/workflows/ssot-source-contract.yml`의 workflow/YAML governance 문제를 구현 세션에서 복구하고 실제 source-contract job green을 확보한다.
...
초기 F01 `35572968635`와 F86 `35573151608`은 delegated auth `unauthorized_client`로 실패했고 `writeAttempted=false`였다. **PR #459 merge `9a5a21e7fa5b71984f9761868722125c963e7054`가 Drive scope를 existing publisher의 DWD scope와 맞춘 뒤, fresh F01 widths dry-run `35573509757`은 `SHEET_FORMAT_APPLY=false` 상태에서 credential preparation, contract regression, snapshot/preflight/readback, evidence까지 `success`로 완료했다.** 따라서 F01 width-only dry-run의 delegated-auth/preflight HOLD는 해소됐다. 그러나 live apply/write는 검증되지 않았고 F86 fresh validation도 없으며, `full` mode는 reference audit가 `VERIFIED`되기 전까지 계속 HOLD다.
