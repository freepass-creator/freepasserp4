...
/**
 * **정산확인 — 영업자·공급사가 «가지고 가는» 화면.**
 *
 * ★2026-08-26 에 「아직 준비중으로 해주고」로 잠가 뒀다. 이유는 영업자 계정이 정비 안 돼서였다 —
 *   실측 당시 8월 영업담당자 9명 중 1명은 계정이 없고, 코드가 안 박힌 줄도 40개였다.
 *   그 상태로 열면 「내 실적이 왜 이것뿐이냐」는 문의부터 받는다.
 *
 * ★★**2026-09-06 에 열었다.** 사장님 「정산확인은 말 그대로 영업 채널이나 공급사들이 들어와서
 *   **우리 얼마나 팔았지, 우리 얼마나 실적했지** 보는 거야 … 그냥 영업자가 보는 건 상품찾기, 정산확인」.
 *   ⇒ 메뉴에 이름만 있고 화면이 잠겨 있으면 메뉴가 거짓말을 한다.
 *   ⚠ 계정·코드가 덜 박힌 줄은 **여전히 안 보인다**(`scopeRows` 가 못 알아보면 0줄이다 — 그게 안전한 쪽이다).
 *     「내 실적이 이것뿐이냐」가 들어오면 그건 화면 문제가 아니라 **원장에 코드가 안 박힌 것**이다.
 *     정산관리 4패널에서 그 줄의 영업채널·코드를 채우면 여기서 보인다.
 *
 * ⚠ 관리자는 여기서 기존 «계약 책상»(RTDB 계약·서류·전자계약)을 그대로 쓴다.
 *   원장(정산관리)과 축이 다른 화면이라 섞지 않았다. 메뉴에는 관리자에게 이 항목을 안 보인다 —
 *   관리자가 보는 정산은 «정산관리»다(사장님 2026-09-06).
 */
const READY = true;
