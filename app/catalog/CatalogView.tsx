...
/**
 * 「많이 찾는 조건」 맨 앞에 서는 **성격 칩** — 금액이 아니라 «어떤 조건의 차인가».
 *
 * 참고한 피드형 시안이 요금 칩 앞에 「무보증 0원」·「즉시출고」 같은 성격 칩을 세워 뒀고,
 * 그게 맞다 — 저신용·무심사 손님이 제일 먼저 재는 것은 월요금보다 **초기에 얼마 드는가**다.
 * ★다만 시안에 있던 「지원금」·「승계」 칩은 «우리 상품이 아니라» 안 만든다(2026-09-04 실측):
 *   지원금 필드가 없고, 잔여개월·만기인수 승계는 다른 장사의 개념이다. 없는 것을 칩으로 세우면
 *   눌렀을 때 0대가 나오고, 그건 손님에게 「이 사이트는 비었다」로 읽힌다.
 * ★즉시출고도 뺐다 — 실측 20대뿐인데 바로 위 안내가 이미 「지금 바로 출고 가능」이라 말한다.
 *   같은 화면이 「전부 즉시」라 해 놓고 칩은 20대만 주면 그게 앞뒤가 안 맞는 것이다.
 * ⇒ 남는 것은 보증금 0 하나(실측 57대). **새 상태를 만들지 않고** 조건칸의 보증금 축
 *   `d0` 을 그대로 켠다 — 두 자리가 다른 값을 들면 그게 곧 「숨은 필터」다.
 */
const QUICK_DEP_ZERO = 'd0';
        ...
        <div style={{ display: mobile ? 'block' : 'flex', gap: 40, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginTop: 14 }}>
              ...
              <Btn size="sm" variant={dep.has(QUICK_DEP_ZERO) ? 'solid' : 'ghost'}
                onClick={() => setDep((p) => toggleInSet(p, QUICK_DEP_ZERO))}>보증금 0원</Btn>
