...
export async function POST(request: Request) {
  ...
  try {
    if (body.action === 'release_cancelled') {
      const result = await releaseCancelledVehicleClaim(firebaseAdminStore(), actor, contractCode);
    ...
    const result = await transitionVehicleClaim(firebaseAdminStore(), actor, { contractCode, key, value });
    return json({ ok: true, result });
  } catch (error) {
