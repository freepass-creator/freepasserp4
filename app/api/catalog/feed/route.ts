...
export async function GET(request: Request) {
  ...
  const requestedWl = S(url.searchParams.get('wl'));
  const whiteLabelRequest = hasBrand(resolveWhitelabel(request.headers.get('host'), requestedWl));
  ...
  try {
     ...
     */
    const useErp5 = whiteLabelRequest && erp5WhitelabelCutoverRequested();
