...
import { readFreepassCatalogFreshness } from '@/lib/server/freepass-catalog';
...
/**
 * 재고 갱신 시각도 상품과 같은 FreePass Catalog Consumer Boundary에서만 읽는다.
 * 별도 ERP4 상태 기록으로 fallback하지 않는다. 다른 원장의 더 최근 시각이 묵은 재고를
 * 최신처럼 보이게 만드는 사고를 막고, 향후 FreePass Data cutover도 한 경계에서 수행한다.
 */
async function loadUpdated(): Promise<{ ms: number; at: string } | null> {
  const ms = await readFreepassCatalogFreshness().catch(() => 0);
