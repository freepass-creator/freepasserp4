...
export default function DataCheck() {
  ...
  return (
    <main style={{ maxWidth: 820, margin: '0 auto', padding: '18px 16px 44px' }}>
      ...
      <div style={{ fontSize: 13, color: C.mute, marginBottom: 16 }}>{rows.length}매물 자동 스캔 · 이상 {groups.length}종 · 표시 {totalHits}건{openReports.length ? ` · 검수 요청 ${openReports.length}` : ''}</div>

      {openReports.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: '#9a3412', marginBottom: 6 }}>⚑ 영업자 검수 요청 {openReports.length}건</div>
