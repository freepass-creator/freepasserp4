...
export default function Erp5ProductDetail() {
  ...
  const facts = [
    ['제조사', 'maker', product.maker], ['세부모델', 'sub_model', product.sub_model], ['파워트레인', 'variant', product.variant || product.powertrain],
    ['세부트림', 'trim_name', product.trim_name], ['차종분류', 'vehicle_class', product.vehicle_class], ['외부색상', 'ext_color', product.ext_color], ['내부색상', 'int_color', product.int_color],
    ['연식', 'year', product.year], ['주행거리', 'mileage', String(product.mileage ?? '').trim() ? mileage(product.mileage) : ''],
  ];
  ...
  return <main className={styles.productDetailPage}>
    ...
    <div className={styles.productDetailGrid}>
      ...
      <section className={styles.detailFacts}><h2>차량설명</h2><dl>{facts.map(([label, field, value]) => <div key={String(label)}><dt>{String(label)}</dt><dd>{atomDisplayText(String(field), value, product)}</dd></div>)}</dl><h2>조건설명</h2><div className={styles.detailConditionChips}>{conditions.map((condition) => <span key={String(condition)}>{String(condition)}</span>)}</div></section>
