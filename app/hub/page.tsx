      ...
      {!loaded ? <Loading /> : error ? <CenterNote>{error}</CenterNote> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '4px 0 24px' }}>
          ...
          <div style={{
            ...
            <span style={{
              width: 12, aspectRatio: '1 / 1', borderRadius: '50%', background: TONE[health], flex: '0 0 auto',
              boxShadow: health === 'running' ? `0 0 0 4px ${C.selected}` : undefined,
            }} />
            <span style={{ fontSize: FS.page, fontWeight: FW.head, color: C.ink, letterSpacing: '-0.02em' }}>
            ...
            <div style={{ border: `1px solid ${C.line}`, borderRadius: R_CARD, padding: '16px 20px', display: 'flex', gap: 34, flexWrap: 'wrap' }}>
              {([
                ...
                <div key={k} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  <span style={{ fontSize: FS.page, fontWeight: FW.head, color: C.ink, fontFamily: NUM }}>{v}</span>
                </div>
              ))}
            </div>
          ) : null}
          ...
          <div style={{ border: `1px solid ${C.line}`, borderRadius: R_CARD, overflow: 'hidden' }}>
            <div style={{
              ...
              <span style={{ fontSize: FS.page, fontWeight: FW.head, color: C.ink, fontFamily: NUM }}>{inv ? inv.length : '—'}</span>
