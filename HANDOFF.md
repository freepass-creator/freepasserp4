...
> **Codex 기존 회원 약관 재동의 게이트(2026-08-04):** `NEXT_PUBLIC_REQUIRE_LEGAL_RECONSENT=true`일 때만 현재 `LEGAL_VERSION` 동의 기록이 없는 활성 로그인 회원에게 이용약관·개인정보 처리방침 필수 재동의를 띄우고, 본인 `users/{uid}`의 세 동의 증적만 갱신한다. 게스트·공개 경로·로그인·차단 계정과 캐시 복원 흐름은 제외했다. 순수 17/17·권한 44/44·phase12 69/69·후보 Rules Emulator 40/40·type/UI/fonts·플래그 ON build 30/30 PASS. 기본값은 OFF이며 Vercel/운영 데이터/Rules/Production 변경 0이다. 다음 작업자는 확정 운영자 6필드와 약관 본문을 먼저 확인하고 Preview 전용 기존 회원으로 1회 표시→저장→재로그인 미표시를 검수한 뒤에만 Production flag를 켠다. `database.rules.json`은 사람/Claude 실데이터 게이트 전 게시 금지.
