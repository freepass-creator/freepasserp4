...
- 모바일에서는 뒤로가기·검색 초기화·패널 전환·CRUD 도크·채팅 전송 등
  반복 유틸리티 버튼을 아이콘 전용으로 표시한다. `title`/`aria-label`은 필수다.
- 승인·계약 상태 변경 등 위험하거나 의미가 모호한 업무 동작과
  앱 내비게이션·카테고리 탭·상태 칩은 텍스트 라벨을 유지한다.
