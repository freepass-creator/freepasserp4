...
> **2026-08-10 Codex → Claude 계약서 대체 보완 검토 요청:**
>
> [`docs/CONTRACT_REPLACEMENT_REVIEW_2026-08-10.md`](docs/CONTRACT_REPLACEMENT_REVIEW_2026-08-10.md)
>
> 기존 계약서를 갈아엎지 않고 누락 절차를 보완했으며, 보험료 포함 등 불필요한 섹션 중복을 단일 소유로 정리했다. 법적·금전 조항이므로 게시·발송·머지 전 Claude/사람 go/no-go가 필요하다.

> **2026-08-10 Cursor → Claude 오픈 전체 검수 요청:**  
> [`docs/CLAUDE_OPEN_FULL_REVIEW_REQUEST_2026-08-10.md`](docs/CLAUDE_OPEN_FULL_REVIEW_REQUEST_2026-08-10.md)  
> 임시 판정 NO-GO · `check:b2b-release` 50 PASS / 5 FAIL · Rules 게시 순서·도메인(fp3)·작업 브랜치 분리 포함. 최종 go/no-go는 Claude.

> **2026-08-06 공급사 상품 연동 재검증:** 운영 roster 15곳은 최신 원본 392행·반영 349대 전체 PASS다. 누락 이안카 `RP031`은 가상 2탭 설정으로 원본 243행·반영 161대 PASS, 비활성 렌트존 `PT-0001`은 가상 활성화로 원본 6행·반영 5대 PASS다. 둘 다 운영 설정/write는 하지 않았다. 감사 도구에 `--override-virtual`과 오토플러스 비정상 행 열구조 마스킹 진단을 추가했다. 다음 게이트는 두 공급사 설정 승인 → 관리자 상품 검증 미리보기 → 충돌 0 확인 → 명시적 반영이다. 자동 동기화는 계속 OFF다.
> **공급사별 독립 판정:** 관리자 표와 감사 CLI에 `반영 가능 / 확인 필요 / 차단` 공통 판정을 추가했다. 무효·중복·집계붕괴는 해당 공급사를 차단하고, 가격 누락·차종 저신뢰는 행을 제외하거나 검수 대상으로 명시한다. 최신 원본에 지속 차단 공급사는 없지만 RP021 가격 2행, PT-0001 가격 1행과 공급사별 차종 검수 36건은 원본/관리자 확인이 남는다. 회귀 142/142 PASS.
