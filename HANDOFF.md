...
## 2026-07-27 실제 남은 오픈 게이트

1. RTDB 전체 JSON 백업과 복구 위치 확정
2. 상품 private `5,641/4,867대 · 4,867쓰기 · 15,781삭제 · 20,648경로/52배치` 승인·실제 적용
3. 정산 private `15/12건 · 12/0/0쓰기 · 24삭제 · 36경로/1배치` 승인·실제 적용
4. 관리자 외 영업 관리자/직원·공급사 관리자/직원 실계정 격리 테스트
5. 전자서명 반려·해지·만료의 실브라우저 테스트
6. Vercel Production 배포와 도메인·로그인·모바일 스모크

Rules 게시, 관리자 운영 E2E, 계약완료·차량잠금·건별/월별/VAT 정산·엑셀,
26개 라우트 build는 완료됐다. 위 게이트가 끝날 때까지 추가 파일 분리와 UI 리팩터링은 동결한다.

## 2026-07-27 상품 private 운영 dry-run

- 실제 운영 미리보기: 검사 `5,641대`, 민감필드 상품 `4,867대`, 안전제외 `0`.
- 이동 계획: private 쓰기 `4,867건`, public 삭제 `15,781경로`, 총 `20,648경로/52배치`.
...
- 운영 데이터는 아직 변경하지 않았다. 52배치 대량 변경이므로 전체 RTDB 백업과 사용자 명시 승인 후 실행한다.
...
- 운영 데이터는 아직 변경하지 않았다. 공개 필드 삭제가 포함되므로 사용자 명시 승인 후 `/dev`의 `정산 private 이동 실행`을 사용한다.
