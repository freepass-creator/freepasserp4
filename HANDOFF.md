...
> **2026-08-10 전자계약 발송센터 후속 설계:** 사용자의 최신 결정은 `ERP 계약 선행 없는 프리패스 자체 발송센터`다. Excel 불러오기·직접 작성·ERP 불러오기가 동일한 Contract Atom/검증/Snapshot 파이프라인으로 합류해야 한다. Claude는 구현 설계 전 반드시 `docs/CLAUDE_ESIGN_SEND_CENTER_BUILD_MEMO_2026-08-10.md`를 읽고 `PLAN.md`에 저장경계·단계별 오더·보안 게이트를 확정한다. 현재 `/esign` 4패널과 착한거래 연동 화면을 최종 UX로 간주하지 않는다.
