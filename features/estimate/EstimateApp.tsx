...
/**
 * 견적기 «본체» — 두 자리에서 같은 화면이 선다.
 *   `/estimate` (업무 · 관리자·공급사 · 원가 보임) · `/quote` (손님 · 신차 · 원가 없음)
...
export default function EstimateApp({ surface = "work", kind }: { surface?: EstimateSurface; kind?: EstimateKind }) {
  ...
  const secCarinfo = (
    <section id="sec-carinfo">
      ...
      <div className="vfields">
        ...
        {/* ★「매입 할인」은 **우리가 싸게 샀다**는 말이다 — 원가 칸이라 손님에게 안 준다(2026-09-17).
            ⚠ 감추기(`hidden`)로 막지 않는다 — 이 줄도 CSS 가 display 를 덮는다. 안 그린다. */}
        {canCost ? (
          <div className="cs-field">
            <label>매입 할인</label>
  ...
  return (
    <div className="wx-root est-root">
      {/* ★★**머리 띠를 «다시» 세운다**(사장님 2026-09-17 「일단 웰릭스 거 UI UX에 **그대로 맞추고
             색깔만 우리 걸로 남색으로** 맞추면 될 것 같아」). 원본 웰릭스 테이블의 `.global-topbar` 짜임 그대로다 —
             높이 48 · 아래 선 하나 · 왼쪽 이름 · 오른쪽 동작들(`.gt-btn`).
          ⚠ 2026-09-08 에는 반대였다(「상단바 없고 그냥 이거 자체가 별도 페이지야」). **뒤엣것이 이긴다.**
          ★★**로고 자리는 비운다** — 노브랜드(CLAUDE.md). 웰릭스는 여기 CI 를 세우지만 우리는 «화면 이름»만 쓴다.
             공급사·영업자가 같이 쓰는 판이라 우리 이름이 서면 안 된다.
          ★색은 우리 남색 토큰(`--brand`)이다 — 값을 여기 적지 않는다(색은 토큰·색상마스터에서만 온다). */}
      {guest ? null : (
        <div className="global-topbar">
          <div className="global-topbar__brand"><span className="dot" />{isNew ? '신차 견적' : '중고 견적'}</div>
          <span className="global-topbar__hint">{isNew ? '제조사 공표가 · 옵션 · 색상' : '시세 · 연식 · 주행으로 센다'}</span>
          <span className="spacer" />
          <div className="global-topbar__actions">
            {/* 갈래를 바꾸는 길 — 웰릭스는 한 갈래뿐이라 없던 자리다. 우리는 화면이 둘이라 오갈 문이 있어야 한다. */}
            <Link className="gt-btn outline" href={isNew ? '/estimate/used' : '/estimate/new'}>
              {isNew ? '중고 견적으로' : '신차 견적으로'}
            </Link>
            {canCost ? <Link className="gt-btn outline" href="/estimate/cost">원가설정</Link> : null}
            <button type="button" className="gt-btn primary" disabled={!priceKnown || !quoteDoc.lines.length}
              onClick={() => setDocOpen(true)}>견적서</button>
          </div>
        </div>
      )}
      ...
      <section className="quote-panel">
        ...
        {/* ★★**손님·담당자가 «맨 위»다** — 웰릭스 테이블이 그렇게 둔다(사장님 2026-09-17
             「웰릭스 거 UI UX 에 그대로 맞추고」). 먼저 적고 시작하는 칸이라 아래에 있으면
             견적을 다 낸 뒤에야 보인다 — 그러면 견적서를 누를 때서야 「이름을 안 적었네」가 된다. */}
        ...
        {guest ? null : (
          ...
          {/* ★「견적서」는 **머리 띠에만** 둔다 — 웰릭스 테이블이 그렇다(2026-09-17).
              아래에 또 두면 같은 단추가 두 번 서서 어느 것이 진짜인지 헷갈린다(실측에서 둘이 보였다). */}
          </>
        )}
