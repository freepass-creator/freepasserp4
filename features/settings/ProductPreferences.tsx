        ...
        <>
          {rows.map((row) => (
            <ListRow
              ...
              right={<Btn title={actionLabel} size="sm" variant="ghost" onClick={() => { haptic.select(); action(row.code); }}>{actionLabel}</Btn>}
            />
          ))}
          <div style={{ paddingTop: 8 }}>
            <Btn title={clearLabel} size="sm" variant="ghost" onClick={() => { haptic.impact(); clear(); }}>{clearLabel}</Btn>
          </div>
        </>
      )}
    </div>
    ...
    <>
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
          <Btn title="최근 본 비우기" size="sm" variant="ghost" disabled={!recentCount} onClick={() => { haptic.impact(); clearRecent(); toast('최근 본을 비웠습니다', 'info'); }}>최근 비우기</Btn>
          <Btn title="찜 비우기" size="sm" variant="ghost" disabled={!favoriteCount} onClick={() => { haptic.impact(); clearFavs(); toast('찜을 비웠습니다', 'info'); }}>찜 비우기</Btn>
        </div>
      </div>
