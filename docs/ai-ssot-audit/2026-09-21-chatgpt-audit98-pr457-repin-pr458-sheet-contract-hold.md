...
**MATERIAL IMPLEMENTATION CHANGE / Audit (97) current-production summary STALE / HOLD(post-repin ERP5 full validation + dedicated Source Contract) / NEW MANUAL DISPLAY-WRITER TOPOLOGY / Audit 90–91 semantic skew OPEN.**
...
4. **Dedicated SSOT Source Contract is still not an effective fail-closed gate.** After the re-pin, its push runs continued to fail before a source-contract job was created, including `35571946592`, PR #458 head run `35572954687`, and current-main run `35572965499`. Generic CI success does not substitute for this dedicated gate.
...
6. **The first F01 and F86 formatting runs both failed before any sheet write.** Run `35572968635` (F01) and run `35573151608` (F86) both passed the local contract regression and credential preparation, then failed in `Snapshot, preflight, optional narrow apply, readback`. Their uploaded failure receipts report `writeAttempted=false` and `unauthorized_client: Client is unauthorized to retrieve access tokens using this method, or client not authorized for any of the scopes requested.` Therefore no mutation from these two runs is established; the new writer is operationally HOLD on Workspace delegated authentication at this audit cutoff.

7. **Legacy boundaries remain unchanged.** `mirror-sync.yml` and `sales-erp-hourly.yml` remain manual/dry-run retired paths, and RTDB/mirror remains non-canonical. No evidence in the audited delta changes the 24-source canonical authority boundary or promotes a legacy path back to SSOT.
...
- Treat `sheet-formatting-only.yml` as a separate, opt-in formatting writer. Do not promote it to inventory authority. Keep `full` title/wrapping apply fail-closed until reference dependencies are VERIFIED, and resolve the delegated Workspace authentication failure before any apply attempt.
