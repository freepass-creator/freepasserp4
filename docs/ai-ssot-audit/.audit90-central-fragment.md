...
- Native ERP5 `event=schedule` `35562733391`도 current head에서 13:56:57 KST에 다시 나타났다. 감사 시점에는 source→Atom→fixed snapshot→public catalog까지 성공 후 F01/F86 발행을 진행 중이었다. native ERP5 delivery 재출현은 확인하지만 settlement native cadence/timeliness HOLD 전체를 닫지는 않는다.
