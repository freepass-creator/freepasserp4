...
| ④ 상품시트로 가져온다 | 판매시트 「상품리스트」 차명(제조사·모델·세부모델·세부트림)은 ②정본(코드→규격채택 이름 / 결정) → ③공급사 칸 → 공급사 원문 순으로 싣고, 원문은 「공급사표기」에 남긴다. 출고불가는 안 싣는다. 대여료 없는 칸은 「-」. 「손오공구독」·「오플구독」 탭은 같은 발행기가 갈래만 따로 찍는다. | publish-origin-tab --apply → publish-sonogong-tab --apply · sync-product-master-live --apply |
