...
| 항목 | 내용 | 어디서/명령 |
|---|---|---|
| 무엇 | 프리패스 재고·차종·판매시트 운영을 «어떤 AI가 와도 같은 방식으로» 하기 위한 운영 매뉴얼. 정본은 리포 `lib/domain/ai-operating-manual.ts`, 이 탭은 그 사본(모든 시트에 같은 글). | 버전 2026-08-21 v15 |
...
| 판매시트 「프리패스 상품리스트」 | 영업자가 보는 표 — 기계가 찍는 사본, 손으로 고치지 않는다. ★기본 세팅(사장님 2026-08-19 확정 「이제 이게 기본 세팅이야」) = 탭 4개: 「상품리스트 …」(21곳 − 오플 − 손오공 구독, 출고불가 제외) · 「손오공상품 …」·「픽업구독 …」·「오플구독 …」 — 같은 발행기·같은 정본 차명이지만 우리 공통 대여료 블록(단기보증·1개월·12개월·장기보증·24~60개월) 대신 **그 공급사의 기간별 대여료**가 그 자리에 선다(손오공: 보증금 반납형(연수×대여료)·12~60개월 반납형·보증금 인수형·36/48/60개월 인수형 / 오플: 12개월 2만km·12개월 3만km·18…36개월 3만km). 같은 차는 한 탭에만. 표준 칸은 별칭으로 되찾는다(sales-published-tabs.ts). [숨김] AI 인계(@매핑=열 구성 정본·@제외) · AI 정제(치환 사전) · 이 시트는. | https://docs.google.com/spreadsheets/d/1Y1Mx1EcEpAuNer0y50Dq4eK92CpVjThO_suZLmo2vVs/edit |
| 원천대장 「ERP4 차종마스터 원천대장」 | 차종마스터 탭 = ERP 차종코드(mf-) 보관 · 차종마스터_규격검토/규격채택 · 상품마스터(차량번호→차종코드) · 시트 지도. 차명·제원 사전은 아래 엔카 차종마스터. 라이브 원장 「차종마스터」 탭에 엔카 줄을 쓰지 않는다. | https://docs.google.com/spreadsheets/d/1T_RrErmGoj_yG9S1u7n--2NDolTOw8wA8ROQjPWuAlg/edit |
| 엔카 차종마스터 | 중고차 원자 시트(정본). 탭 「안내」·「차종마스터」·「세부모델」. 키 M/SM/T/U. 공급사 기본스펙·사본 탭의 사전. 기존 ERP 원장 mf- 와 다른 문서. | https://docs.google.com/spreadsheets/d/1oMB9eoNnQFxUyRK4CSxYh_hKrtCf7s_79xLs-GYwXCE/edit |
...
| 차명(제조사·모델·세부모델·세부트림) | 엔카 차종마스터(M/SM/T) → 공급사 행키+기본스펙(아는 층만) → 판매시트. ERP 차종코드(mf-)는 상품마스터 정본. 글자 스냅으로 매번 다시 맞히지 않는다. | 아래 3장 · 3′장 |
...
| 항목 | 내용 | 어디서/명령 |
|---|---|---|
| 한 줄 요약 | 판매시트(영업자 표)와 ERP 는 같은 표를 본다. ERP 는 판매시트 4탭을 그대로 읽고(상품마스터 안 거침), 양쪽 모두 «출고불가만» 빼며 상품화중·출고협의·계약중은 보여 준다. 대여료·보증금이 없는 차도 목록에는 나온다. 차량번호가 없는 줄만 판매시트에서 빠진다 — 번호가 열쇠라 그것이 없으면 어디에도 못 붙는다. | 버전 2026-08-20 v1 |
| ① ERP 가 읽는 원본 | **영업자 상품리스트(판매시트) 4탭**(상품리스트·손오공상품·픽업구독·오플구독)을 그대로 읽는다. 상품마스터는 안 거친다(취급 이력 원장으로만 남는다). 되돌리려면 환경변수 SHEET_DAILY_SOURCE=product_master. | lib/server/sheet-daily-sync.ts · lib/server/sales-inventory-sheet.ts |
...
| ④ 시트 「계약중」의 뜻 | ERP 도 **계약중 그대로** 둔다(예전엔 출고불가로 투영해 목록에서 사라졌다 — 실측 손오공상품 2대). ERP 계약 엔진의 락은 이와 별개이고 시트가 만들지도 풀지도 못한다. | lib/domain/sheet-daily-sync.ts projectSheetContractStatus |
...
## 3. 차명 정제 흐름 — 엔카 차종마스터 → 공급사 기본스펙 → 상품시트
...
| ① 사전 = 엔카 차종마스터 | 한 줄 = 원자(U). 계층 키는 세 층: 모델행키 M(제조사×1차모델) · 세부모델행키 SM(+세부모델) · 세부트림행키 T(+세부트림). 원자 U 는 그 T 가 연료×배기량×인승×구동으로 갈라진 줄. 기존 ERP 차종마스터(mf-…)는 보지 않는다. 색은 색상마스터. | https://docs.google.com/spreadsheets/d/1oMB9eoNnQFxUyRK4CSxYh_hKrtCf7s_79xLs-GYwXCE (연동) · 라이브 원장 「차종마스터」 탭에 쓰지 않음 |
| ② 차량번호 정본 = 상품마스터 + 3축 결정 | 차량번호마다 ERP 차종코드(mf-…) 하나(검증상태 확정). 엔카 T 와는 결이 다르다 — 공급사 「차종코드」는 ERP 키, 「세부트림행키」는 엔카 T. 코드가 없는 차는 결정 파일(CODE/TRIPLE/PARTIAL/HOLD)이 정본. 글자 스냅으로 매번 다시 맞히지 않는다. | 원천대장 「상품마스터」 차종코드 열 · 리포 data/product-vehicle-review-decisions.json |
| ③ 공급사에 박는다 = 행키 + 기본스펙 | 정책코드 다음: 모델행키 · 세부모델행키 · 세부트림행키 · 원산지 · 제조사(정제) · 모델 · 세부모델 · 세부트림 · 배기량(정제) · 연료(정제) · 구동방식 · 차종크기 · 차종구분. 엔카 원자와 맞춰 **아는 층만** 넣는다(모델만 알면 M+원산지+제조사+모델, 세대까지면 SM+세부모델, 트림까지면 T+세부트림). T 안에서도 배기량·구동이 갈리면 차명·왼쪽 칸의 1.6·2WD·연료로 좁힌다. 정확 cc 가 1591/1598처럼만 갈리면 표시배기량(1.6). 모르면 빈칸, 점검사항에 엔카 후보. 이미 있는 유효 T 는 안 지운다. ERP 차종코드(mf-)·선택옵션·외장/내장색상은 다른 도구. | stamp-encar-codes-on-supplier --apply · 열 자리는 insert-encar-code-columns --apply |
| ④ 상품시트로 가져온다 | 판매시트 「상품리스트」 차명(제조사·모델·세부모델·세부트림)은 ②정본(코드→규격채택 이름 / 결정) → ③공급사 칸 → 공급사 원문 순으로 싣고, 원문은 「공급사표기」에 남긴다. 출고불가는 안 싣는다. 대여료 없는 칸은 「-」. 「손오공상품」·「픽업구독」·「오플구독」 탭은 같은 발행기가 갈래만 따로 찍는다. | publish-origin-tab --apply → publish-sonogong-tab --apply · sync-product-master-live --apply |
| ⑤ 새 차가 들어오면 | stamp 를 다시 돌린다(아는 층만 채움·유효 T 유지) → 못 정한 차는 점검사항 → 엔카 마스터에 없는 차는 마스터 보강 후 stamp. fill-supplier-ai-columns 는 선택옵션·외장/내장·ERP 차종코드를 맡는다. 그다음 ④ 발행. | stamp-encar-codes-on-supplier --apply · fill-supplier-ai-columns --apply · run-daily ④ |
...
| 기본스펙(원산지부터) | 원산지 · 제조사(정제) · 모델 · 세부모델 · 세부트림 · 배기량(정제) · 연료(정제) · 구동방식 · 차종크기 · 차종구분. 연식·주행거리·외부색상·내부색상은 왼쪽 렌트사 칸(원문). 색 규격은 외장/내장색상. |  |
| 아는 층만 | 후보가 하나로 모이면 넣는다. 모델만 알면 원산지·제조사·모델(+M). 세대까지면 세부모델(+SM). 트림까지면 세부트림(+T). T 가 있어도 배기량·구동이 갈리면 차명·왼쪽 칸(1.6 · 2WD · 연료)으로 좁힌다. 정확 cc 가 1591/1598처럼만 갈리면 표시배기량(1.6). 틀린 값보다 빈 칸. 유효 T 는 안 지운다. |  |
...
| 채우기 명령 | 마스터 시트를 고치면 stamp 가 그 파일을 읽는다. stamp 시작 때 제원 가드가 깨지면 시트를 안 쓴다. 미리보기 기본, 반영은 --apply. 공급사 「차종마스터」 탭은 IMPORTRANGE 연동. | npx tsx scripts/stamp-encar-codes-on-supplier.mts --apply · npx tsx scripts/publish-vehicle-master-tab.mts --apply |
| 하지 말 것 | 라이브 차종마스터에 쓰기 · 공급사에 원자ID/마스터표기 두기 · 후보가 여럿인데 배기량·트림을 찍기 · ERP 차종코드(mf-)를 T 칸에 넣기 · 옵션 글자로 트림 맞추기. |  |

## 4. AI(자동화)가 적고 만지는 칸 — 이 밖의 칸을 기계가 건드리면 버그다

| 항목 | 내용 | 어디서/명령 |
|---|---|---|
| ★공급사 시트가 읽히는 방식(2026-08-19 굳힘) | 차량번호가 열쇠: 프리패스는 줄 번호가 아니라 **차량번호**로 차를 알아봅니다. 차량번호가 비면 그 줄은 안 읽습니다(번호 전 신차는 「미정」·차대번호). / 열은 이름으로: 열 **이름**으로 읽습니다(차례 아님). 열 이름을 바꾸거나 지우지 마세요. 필요한 칸이 없으면 프리패스가 넣습니다. / 표(Table)는 끝 열까지: 표가 첫 열부터 마지막 정제칸까지 한 덩어리입니다(2026-08-19). 표에서 정렬·필터해도 **줄 전체가 같이** 움직입니다. 표 밖에 값을 적거나 표를 지우지 마세요. / 왼쪽 = 렌트사 칸(남색 머리): 차량번호·입고일자·점검사항(프리패스 요청) ／ 상태·분류·제조사·차명(세부모델+트림)·옵션·색상·연식·주행거리·연료·배기량·차량가격 ／ 보증금·기간별 대여료 ／ 최초등록일·사진링크. 렌트사가 적고, 프리패스는 빈 칸만 채웁니다(입고일자·연식 등 파생값). / 오른쪽 = 프리패스 칸(보라 머리, 구분선 「│」 뒤): 정책코드 다음: 모델행키(M) · 세부모델행키(SM) · 세부트림행키(T) · 원산지 · 제조사(정제) · 모델 · 세부모델 · 세부트림 · 배기량(정제) · 연료(정제) · 구동방식 · 차종크기 · 차종구분. 그 뒤 ERP 차종코드(mf-) · 선택옵션 · 외장/내장색상. 엔카 키·기본스펙은 아는 층만 넣고, 옵션·색상은 그 줄 원문에서 다시 계산합니다. 손대지 않으셔도 됩니다. / 「상품시트」 탭 = 올라간 값: 재고 탭 옆 「상품시트」 탭은 프리패스 판매시트에 올라간 이 공급사 줄 그대로입니다(발행마다 갱신, 손으로 고치지 않음). 재고 탭 원문과 다르면 차량번호만 프리패스에 알려 주세요 — 5단계 추적으로 바로 잡습니다. / 「점검사항」 칸: 상태 앞 「점검사항」은 프리패스가 적는 요청입니다(값이 있으면 노란 바탕). 처리하시면 답을 적거나 지워 주세요. / 입고일자: 차량번호가 이 시트에 처음 올라온 날입니다. 비워 두면 프리패스가 자동으로 적습니다(최초등록일이 아님). / 값 규격: 상태 6값 · 분류 4값 · 제조사 드롭다운 이름(르노·KGM…) · 연료 6값 · 날짜 YYYY-MM-DD · 돈은 숫자만 · 색상은 규격 12색 밖이면 「기타」(별칭은 색상마스터). / 틀리면 어디를 고치나: 대여료·보증금·상태·차명·옵션·색상 원문 = 이 시트 재고 탭(정제시트 공급사는 원본) · 정책 = 정책 탭 · 차종 이름 = 프리패스(차종마스터·결정) · 판매시트/상품마스터/ERP 는 사본이라 거기서 고쳐도 다음 반영에 사라집니다. | lib/domain/sheet-reading-rules.ts · 작성 안내·정제시트 안내 0장 |
...
| ★차명 정제 흐름(한 줄) | 엔카 차종마스터(M/SM/T/U) → 공급사 행키+기본스펙에 아는 층만 박음(stamp) → ERP 차종코드·옵션·색상은 fill. 상품리스트는 정본→공급사 칸→원문 순. 자세한 단계는 아래 「차명 정제 흐름」 표. | 새 차가 들어올 때마다 · 엔카 차종마스터가 바뀌면 |
...
| 엔카 행키 + 기본스펙(원산지~차종구분) | 엔카 원자와 맞춰 **아는 층만** 넣는다. 모델만 알면 M·원산지·제조사(정제)·모델. 세대까지면 SM·세부모델. 트림까지면 T·세부트림. T 가 박혀도 배기량·구동은 같은 트림 안 갈래(1.6/2.0 · 2WD/AWD)가 있으면, 차명·왼쪽 칸(1.6 · 2WD · 연료)으로 좁힌 뒤에 채운다. 정확 cc 가 1591/1598처럼만 갈리면 표시배기량(1.6). 모르면 빈칸. 이미 있는 유효 T 는 안 지운다. 틀린 값보다 빈 칸. | stamp-encar-codes-on-supplier --apply |
| ERP 차종코드 · 선택옵션 · 외장/내장색상 | 차종코드(mf-…)는 상품마스터 정본이 있으면 그걸 믿는다. 선택옵션·외장/내장색상은 그 줄 왼쪽 원문(옵션·외부색상·내부색상)에서 규격으로 정제해 넣는다. 빈 칸은 채우고, 원문과 어긋나면 다시 맞춘다. 엔카 M/SM/T·기본스펙 칸은 이 도구가 안 건드린다. | fill-supplier-ai-columns --apply · realign-derived-cells |
...
| ② 엔카 기본스펙 채움 | 공급사 행키+원산지~차종구분을 엔카 원자와 맞춰 아는 층만 넣는다(21곳). | npx tsx scripts/stamp-encar-codes-on-supplier.mts --apply |
| ②′ 정제칸(옵션·색·ERP코드) | 선택옵션·외장/내장색상·ERP 차종코드. 엔카 M/SM/T·기본스펙은 안 건드린다. | npx tsx scripts/fill-supplier-ai-columns.mts --apply --include-mirror |
...
| ④ 판매시트 발행(탭 4개) | 검증된 동일 snapshot을 상품리스트 → 손오공상품(RP012 중고렌트+오공구독) → 픽업구독(RP012 T카 외부재고) → 오플구독(RP023)으로 발행한다. F86은 12개월 이하 단기 대여료·단기보증만 숨기고 24개월 이상 장기 대여료·장기보증은 표시한다. 별도 오공구독 탭은 만들지 않는다. | `ERP5 SSOT 원천 최신화(매시간)` 단일 운영 workflow |
...
| 정제시트 차명·옵션이 틀림 | 공급사 원본 시트/홈페이지에서. 정제시트에서 고쳐도 다음 미러가 되돌린다. 정제칸(모델·세부모델·세부트림)은 정제시트에서. | sync-mirror-all → stamp |
