...
| 열 구성 | 판매시트·공급사 상품시트 = 같은 현재 61열(1대=1행) · 공급사 재고 = TEMPLATE_COLUMNS(28)+정제칸(11) · 천이시트 = 7행×13열 카드. 열·셀 규격을 갈래 사이에 복사하지 말고 각 정본 매핑으로 읽는다. | sales-sheet-mapping · supplier-preview-parity · channel-card-sheet |
...
| ③ 정제시트에서 세 갈래로 내보낸다 | ① ERP: 정제/제공시트 → 중앙 판매시트 3탭 → ERP가 판매시트를 직접 읽는다. ② 공급사 「상품시트」: 중앙 판매시트에서 그 공급사 행을 현재 61열·1대=1행 그대로 복제한다. ③ 천이시트: 손오공·웰릭스 정제/제공시트를 1대=7행×13열 카드로 별도 변환하며 J:L에는 최저가 한 벌만 넣는다. 세 출력은 서로의 입력이 아닌 병렬 사본이므로 셀 주소를 서로 복사하지 않는다. 출고불가는 제외하고, 대여료 없는 판매시트 칸은 「-」. | publish-origin-tab / publish-sonogong-tab → run-sheet-daily-sync-local(ERP) · publish-supplier-preview-tabs(상품시트) · publish-channel-cards(천이) · audit-sheet-erp-parity + audit-pipeline-destinations |
...
| ④′ 공급사 「상품시트」 | 중앙 판매시트에서 해당 공급사 행을 현재 61열·1대=1행으로 그대로 복제한다. 대상 문서는 「프리패스 거래처 정리」 공급사 탭에서 `거래중`이고 정제/제공 링크가 있는 행만 쓴다. Drive 파일명 검색 금지. 링크가 `[구버전·폐기]` 문서를 가리키면 쓰기 전 전체 발행을 막는다. 머리글 이름·순서·행 전체가 모두 같아야 하며 공급사 전용 열을 빼거나 더하지 않는다. | npx tsx scripts/publish-supplier-preview-tabs.mts --apply |
...
| ⑥ 검수 | 정제칸·돈·빈칸·차종마스터 잠금을 확인한 뒤 세 갈래를 따로 검사한다. ERP=판매시트 레코드, 공급사 상품시트=61열 행 전체, 천이시트=7×13 카드 91셀과 차량 대수. | audit-sheet-erp-parity · audit-pipeline-destinations · audit-vehicle-refine · check-vehicle-master-lock · verify-master-pass |
