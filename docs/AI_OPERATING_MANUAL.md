...
| 항목 | 내용 | 어디서/명령 |
|---|---|---|
| 무엇 | 프리패스 재고·차종·판매시트 운영을 «어떤 AI가 와도 같은 방식으로» 하기 위한 운영 매뉴얼. 정본은 리포 `lib/domain/ai-operating-manual.ts`, 이 탭은 그 사본(모든 시트에 같은 글). | 버전 2026-08-21 v19 |
...
| ★**사전은 두 층이다**(2026-08-21 실측) | **이름**(제조사·모델·세부모델) = `vehicle-master.json` **1,813종**. **차종크기·차종구분·연료·배기량·구동·엔카 행키(M/SM/T/U)** = 「차종마스터 신규」 시트 **428종**. 시트는 스스로 「이 시트가 정본」이라 적지만 **범위가 「엔카 중고차만」**이라 우리가 파는 차의 절반이 안 잡힌다(481대 중 이름이 붙는 것 시트 203 · JSON 394). **둘 중 하나만 정본이라 쓰면 안 된다.** 어느 쪽을 볼지 먼저 정하고 도구를 돌린다. | 시트 「안내」 탭 · audit-refine-vs-master.mts |
| ★**제조사 표기는 공급사 쪽이다**(시트 「안내」 명시) | 시트(엔카) = 르노코리아 · KG모빌리티. **정제칸 「제조사(정제)」에는 공급사 표기 「르노」·「KGM」을 넣는다.** 시트 표기로 되돌리지 마라 — 맞는 값을 어긋남으로 세게 된다. | `canonMakerDisplay` · maker-display.ts |
...
## 3. 차명 정제 흐름 — 우리 차종마스터화 → 정제칸에 그 내용 → 상품시트
...
| ③ 상품시트로 가져온다 | 판매시트 「상품리스트」 차명은 ②정제칸(차종마스터 글자) → 공급사 원문 순으로 싣고, 원문은 「공급사표기」에 남긴다. 출고불가는 안 싣는다. 대여료 없는 칸은 「-」. 「손오공구독」·「오플구독」 탭은 같은 발행기가 갈래만 따로 찍는다. | publish-origin-tab --apply → publish-sonogong-tab --apply · sync-product-master-live --apply |
...
| ★도구가 무엇을 정본으로 읽는지 먼저 본다(2026-08-21) | 감사 도구 `audit-vehicle-refine` 이 **옛 원장 「차종마스터」 탭**을 사전으로 읽는 줄 모르고 결과를 그대로 보고했다 — 「마스터에 없다」 16건 중 7건이 헛발(스파크 M300·RAV4 5세대 등은 멀쩡히 있었다). 그 뒤 신규 시트를 안 보고 JSON 으로만 견줘 **「제조사 어긋남 55건」을 지어냈다**(르노·KGM 은 안내대로 맞는 값). 숫자를 옮기기 전에 그 도구의 사전이 무엇인지 연다. | audit-refine-vs-master.mts |
| ★열을 하나 끼우면 그 탭 읽는 곳을 전부 훑는다(2026-08-21) | 정책 탭 맨 앞에 「정책UID」를 넣자 「정책코드」가 둘째 칸으로 밀렸다. 자리로 읽던 `readPolicyTab` 을 고치고 **거기서 멈췄다** — `sync-mirror-policies.mts` 도 `hdr[0] !== '정책코드'` 였다. 아이카·이안카 정책 미러가 13:37부터 30분마다 죽었다. 같은 실수를 같은 날 두 번 했다. | `grep -rn "hdr\[0\]" scripts/ lib/` 로 자리 읽기를 먼저 턴다 |
| ★새 칸을 만들기 전에 같은 뜻의 옛 칸을 찾는다(2026-08-21) | 판매시트 「차종구분」을 차종크기+차종구분으로 만들면서 이미 있던 「차종분류」와 대조하지 않았다. 76대에서 두 값이 갈렸고 12대는 판매시트 쪽이 틀렸다 — A6 가 「중형 SUV」, 팰리세이드가 「대형 MPV」로 영업자에게 나갔다. 둘 다 남기면 언젠가 반드시 갈린다. | publish-origin-tab.mts:552 |
