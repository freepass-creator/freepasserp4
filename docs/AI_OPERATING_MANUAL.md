...
| 열 구성 | 판매시트·공급사 상품시트 = 같은 열(현재 코드 예비값 ／ 63 ／열, 실제 정본은 판매시트 「AI 인계」 @매핑·1대=1행) · 공급사 재고 = TEMPLATE_COLUMNS(28)+정제칸(11) · 천이시트 = 7행×13열 카드. 열·셀 규격을 갈래 사이에 복사하지 말고 각 정본 매핑으로 읽는다. | sales-sheet-mapping · supplier-preview-parity · channel-card-sheet |
...
| 구분선 「│」 오른쪽 전부(정책코드 · 모델행키 · 세부모델행키 · 세부트림행키 · 원산지 · 제조사(정제) · 모델 · 세부모델 · 세부트림 · 배기량(정제) · 연료(정제) · 구동방식 · 차종크기 · 차종구분 · 차종코드 · 선택옵션 · 외장색상 · 내장색상 · 차종분류 · 차종분류코드 · 차명(정제)) | 프리패스/AI 칸(머리 보라). 렌트사는 여기에 적지 않는다. 왼쪽(남색 머리)은 렌트사 칸 — 기계가 값을 안 덮는다. | 늘 |
