...
| 항목 | 내용 | 어디서/명령 |
|---|---|---|
| 무엇 | 프리패스 재고·차종·판매시트 운영을 «어떤 AI가 와도 같은 방식으로» 하기 위한 운영 매뉴얼. 정본은 리포 `lib/domain/ai-operating-manual.ts`, 이 탭은 그 사본(모든 시트에 같은 글). | 버전 2026-08-23 v23 |
...
| 항목 | 내용 | 어디서/명령 |
|---|---|---|
| 담당 | 쓰기(json 이름·aliases·mf- 키 의미·「AI 정제」 정본 매핑·새 키 발급) = **커서만**. 라이브 원장 「차종마스터」 탭은 누구든 쓰지 않는다. 다른 AI는 이슈만 남기고 키·이름을 바꾸지 않는다. | vehicle-master-lock · VEHICLE_MASTER_WRITE_ONLY |
...
| 구분선 「│」 오른쪽 전부(정책코드 · 모델행키 · 세부모델행키 · 세부트림행키 · 원산지 · 제조사(정제) · 모델 · 세부모델 · 세부트림 · 외장색상 · 내장색상 · 연료(정제) · 배기량(정제) · 구동방식 · 인승 · 차종구분 · 배터리용량(정제) · 차종크기 · 차종코드 · 선택옵션 · 차종분류 · 차종분류코드 · 차명(정제)) | 프리패스/AI 칸(머리 보라). 렌트사는 여기에 적지 않는다. 왼쪽(남색 머리)은 렌트사 칸 — 기계가 값을 안 덮는다. 정제시트는 내장색상 뒤에 연식·주행거리. | 늘 |
