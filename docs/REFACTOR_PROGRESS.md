...
- 웹·모바일의 정보 순서·패널 의미·상태·기능을 동일하게 유지하고 배치만 반응형으로 전환하는
  UI/UX 원칙을 설계·인수인계 문서에 고정
- 모바일 전용 기능·요약·업무 흐름은 별도 지시 없이 추가하지 않는 규칙 명시
...
## 완료: 모바일 표면 액션 아이콘 전용 2차 통일

- 공통 `Btn.mobileIcon`을 추가해 모바일 아이콘/웹 텍스트 분기를 SSOT로 통합
- 계약·전자서명·정산·재고·OCR·시트 연동·사진·채팅·회원·설정·관심함·목록 액션 적용
- 위험 액션은 아이콘화하되 기존 확인 절차 유지
- 선택 칩·탭·확인 대화상자는 텍스트 유지
- 모바일 상품·계약 엑셀, 월정산 정산서, 재고 종합표 내보내기 미노출
- Phase 12 회귀 가드 33/33 PASS
