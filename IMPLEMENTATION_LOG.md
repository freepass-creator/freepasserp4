...
## 2026-08-19 — 파트너 상세 = 기본정보 · 운영정책 · 수수료정책 (Cursor)

사장님: 「기본정보에 회사·계좌·실무자·대표자·상품시트 주소. 그다음 운영정책 / 수수료정책」.

- 파트너 패널: `기본정보` / `운영정책` / `수수료정책` (계정 패널은 그대로)
- 기본정보 = 회사정보 · 대표자 · 실무자 · 계좌 · 상품시트 URL. 주소·계좌·시트는 한 줄 폭
- 운영정책 = 보증금 규칙 · 어댑터 · gid · 헤더 · 매핑 초기화
- 수수료정책 = 공급사 수수료율(R1)
- 원자 = 기존 `FormCard`/`FormGrid`/`FormReadList`/`ListGroup` 배열만

---

## 2026-08-19 — 상단바 건수 = 페이지명 N단위 중 M단위 (Cursor)

사장님: 「상단헤더 타이틀을 전부 같게. 몇 건 중 몇 건」.

- `PageStatus` = `[아이콘] 라벨 N단위` + 목록이 줄면 `중 M단위`. 미회신·처리대기 같은 KPI는 상단에서 뺌
- `WorkPage` 헤더 라벨 = `title`(NAV_LABEL). `statusCount`=전체, `listCount`=지금 목록
- 상품찾기·재고·문의·계약·정산·계약서·회원·정책·감사 동일 DNA
- 상담 미확인/미회신은 목록 칩에만 유지
