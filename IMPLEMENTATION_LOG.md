...
## 2026-08-19 — 파트너사 관리는 `/policy` (Cursor)

사장님: 「정책관리는 이제 파트너관리. 회원·파트너 탭에 넣지 마라」.

- 메뉴 `정책관리` 표기 제거. 같은 자리(`/policy`)는 `파트너사 관리`만 남김. 화면 안내 문구도 같은 이름.
- 4패널: 목록 · 기본정보(회사·대표·실무) · 운영정책(시트+심사/조건/보험/전자계약) · 수수료정책(율·계좌)
- 회원·파트너(`/members`) 파트너 패널은 원래대로 되돌림
