...
## 2026-08-29 — 모바일 컨트롤을 문서 규격(40·56)으로 되돌림

사장님 「통상적인 규격으로 배포」. tokens가 36으로 내려가 검색·필터·메뉴가 작아 보였다. CLAUDE.md대로 md 모바일 40 · 바 56 · 검색/메뉴/탭 아이콘 ICON.xl(20). sm은 표 안 36 유지.

---

## 2026-08-29 — 클로드 오더: fill 분할 적용 U1·U2

클로드 계획 `memoized-splashing-robin`(전량 apply 금지). `--only-blank`로만 씀.

- U1 퍼시픽 1칸(구동 AWD) 반영됨.
- U2 나머지 빈 칸 83 반영. 재-dry-run **빈 칸 0**. 표기·바로잡음 0. 스냅샷 `tmp/encar-fill-snap/`.
- 칸: 배기량 39 · 구동 36 · 세부트림 7 · 연료 1. 큰 곳 오플 38 · 아이카 21 · 이안카 9.
- G1 르노코리아·KG모빌리티 → 제조사(정제) 보류.
- U3 표기는 이전 전량 fill에 이미 반영. **U4~U6 덮기·U5 4WD→AWD·U7 연카 구조·U8 매칭불가·판매시트 발행은 안 함**(사람 판정).

---

## 2026-08-28 — 상품찾기 모바일 검색줄: 검색칸=필터칸 높이

사장님 「아이콘·검색창·필터 버튼 크기가 안 맞는 느낌」·메인만. 필터가 아이콘(+숫자) 내용만큼만 줄어 검색칸(ctrlH 36)과 안 맞았다. trailing 슬롯을 입력 높이 정사각으로, 조건 개수는 CountPill. 돋보기·필터 글리프는 ICON.md. iOS 44/머티리얼 48은 토큰 36이라 메인만 안 올림.

---

## 2026-08-28 — 차종 붙이는 요령을 매뉴얼 0″에 박음

커서 실무: 풀 중첩 · uniqueLongest(오타 안 맞춤) · FL 합침(같은 트림 삭제+기간 이어붙임, 배터리 팩은 남김) · fill은 왼쪽 원문(제보 트림도 되돌림) · 검증은 시트 원본. playbook + 차종마스터 매뉴얼 + vehicle-master.mdc. AI 운영 매뉴얼 v30.

---

## 2026-08-28 — 제네시스 FL 합침 + 정제칸 fill

엔카에 없는 제네시스 FL 세부모델 6종 7행 삭제. 같은 트림 기존 행 생산종료를 FL 종료(현재)로 이어 붙임. 배터리 2행은 이름만 떼고 kWh 유지(G80 87.2/94.5, GV70 77.4/84.0). 재조회 FL 0 · 차종 1668행. 이어서 공급사 정제칸 `--apply` 20곳(연카 정제칸 없음) 1153대 · 빈칸 329 · 바로잡음 880. 라이브 mf-·판매시트 안 씀.

---

## 2026-08-28 — fill: 세부모델 잠근 뒤에만 세부트림

사장님 「세부모델 찾은 다음에는 그 밑에서 세부트림」·「제조사 모델도 그 안에서만」. `attachFromEncarSheet`는 제조사 풀→모델 풀→세부모델 풀→트림 풀. 한 칸이 안 모이면 아래는 안 붙임.

---

## 2026-08-28 — 차종마스터: 엔카 절대 경고를 모든 AI 문서에 박음

사장님 「절대적으로 따르라고 모든 AI한테 경고해라」. 한 줄 `ENCAR_NAME_ABSOLUTE` — 엔카 1:1, 예외 둘만, 세 번째 예외 금지. AGENTS·CLAUDE·CURSOR·.cursorrules·vehicle-master.mdc·운영매뉴얼 0장·안내. `check-vehicle-master-lock` 이 글자 유무를 본다.

---

## 2026-08-28 — 손님 서명: 법인 「서명자 정보」 단계

클로드 오더. `docs/ESIGN-MANUAL.md` §8. 법인 계약만 `identity` 다음에 선다.
성명·관계(대표이사 / 위임받은 임직원 고르기)·주민번호 필수, 면허번호 선택. 제출 payload 에 네 칸을 싣는다.

---

## 2026-08-28 — 손님 서명: 여백·페이지 구조 통일

사장님 「위아래 여백이랑 페이지 구조 다 통일」.
- 만료·완료·불러오는 중도 같은 껍데기(`SignShell` = compact 머리 · 본문 · 하단).
- 좌우 22px, 입력 칸 사이 18px, 카드 사이 14px. ERP `FormCard`/`Dropzone`/`WorkInput` 을 손님 칸(`SignField` · `.scard` · `.auth-opt` · `.sig`)으로 맞춤.
- `.fp-main-pad:has(.sign-root)` 이중 스크롤 잠금.

---

## 2026-08-28 — 손님 서명: 하이픈 자동 · 개인사업자 추가입력

사장님 「주민번호랑 전화번호 하이픈 자동, 개인사업자면 사업자정보 추가로」.
- 입력 중 하이픈: 주민·법인등록 `fmtRrn`, 전화 `fmtPhone`, 사업자 `fmtBizNo`, 면허 `fmtLicense` (`formatters.ts`).
- 개인 계약에서 「개인사업자입니다」를 고르면 세금계산서 단계가 열린다. 발행 때 이미 개인사업자면 처음부터 열린다.
- 손님이 사업자정보를 채우면 발행 유형이 개인이어도 제출본에 저장한다.

---

## 2026-08-28 — 오플 정제시트 정제칸 fill (엔카 작업 시트)

사장님 「오플 정제시트 채우자」. `fill-supplier-from-encar-sheet --who=오토플러스 --apply`. 정제시트 `1Tvd5IioF5y_yu3L1BQMRP4J1R8hcZHwkgl3vl-TsgY0`. 판매시트·ERP·라이브 mf- 안 씀.

- 133대 · 빈 칸 84 · 바로잡음 67 · 같음 604. 세부모델 42(EV6 CV1→EV6 · 니로 EV SG2 떼기 · GV70/GV80 개발코드 떼기 등) · 구동 19(FWD/RWD→2WD · XDRIVE/콰트로→AWD) · 트림 4 · 연료 2(볼보 B4/B5 가솔린→하이브리드).
- 안 덮음 3칸: 배기량 1969→2000 같은 리터 2(볼보 XC40·V60) · 투싼 `N Line 인스퍼레이션`→`인스퍼레이션` 1. fill 스크립트에 이 두 가드만 넣음(검수 sameLiter · LATIN_BRAND_TRIM_CANON).
- 모델 못 붙인 20대는 그대로(제조사 빈칸 EV6 6대 · G80/GV70 제조사 없음 · Q5·3시리즈·트랙스 등). 추측으로 안 채움.
- 판매 재발행·ERP 동기 이번 요청 없음.

---

## 2026-08-28 — 전자계약 면: 표준페이지 없음, 공용 원자만 붙임

표준 페이지 템플릿을 씌우지 않는다. `/sign`·미리보기는 `WorkPage` 없음. 표=`WorkTable`, 표 아닌 블록=`FormCard`, 안내=`Message`, 동의=`Checkbox`. `EsignStageCard`는 표가 없는 단계(플래그·버튼)만. `WorkTable`을 FormCard로 한 번 더 감싸던 자리 해제.

---

테스트하다가 멈춘 화면. **폐기 예정. 지금은 보류.** 코드는 그대로 두고 고치지 않음. UI 규격 정본 아님. 메모=`docs/MEMO-ERP5-보류-2026-08-28.md`.
