...
## 2026-09-02 — 정제칸은 빈 칸에 한 번만

오더: 정제칸은 한 번 채워 두면 된다.

손오공 재고시트가 이름칸을 매시간 덮고, 폐쇄기가 이미 있는 칸을 비웠다. 둘 다 빈 칸만 쓰게 바꿈. 예외는 원문 없는 디올뉴 되돌림뿐. 새 차·빈 칸만 다음 회차에 채움.

---

## 2026-09-02 — 손오공 정제칸 채움 (구독·픽업)

오더: 정제칸을 비워 둔 채 상품리스트/손오공 차종이 안 나옴. 채우는 쪽=커서(hourly ⓪ `손오공-정제` → 재고시트). fill-supplier는 손오공 스킵.

원인: 트림/연식 실패 시 모델·세부모델을 안 씀. 경유≠디젤·HEV 세부모델 글자 필수·배기 2200≠2199로 연식 맞는 세대가 통째로 비워짐. NAMES가 빈 세부모델로 기존 칸을 지움.

고침: 연식+연료로 세부모델이 유일이면 채움(트림 몰라도). 모델만 유일이면 모델만. 생산기간 한쪽 빈 행은 연식 매칭에서 뺌. 재고시트는 값 있는 이름축만 씀.
실측 구독 판매가능 74: 모델 공란 55→11, 세부모델 공란 65→28. 픽업 268: 모델 공란 offer 34→1.
남은 11대(아이오닉·포터·볼트 등)는 라이브 모델명이 안 갈림 → 검수대기. 마스터 안 바꿈.
판매: 손오공구독 74대 · 픽업구독 268대 (09.02 15:10). 구독은 상품리스트 @제외.

---

## 2026-09-02 — 손오공 68로3386 차명(정제) 적용 (마스터 안 바꿈)

검수: 라이브 세대는 `디 올 뉴 니로 EV SG2`(기존 행, 동결). 원문은 `The all new NIRO EV / 에어(Air)` — 한국어 디올뉴 없음. 마스터 철자를 바꾸지 않음. 정제칸 철자=`니로 EV SG2`.
원인: 폐쇄기가 세부모델이 이미 맞으면 차명(정제)를 안 덮음.
고침: closer가 원문 없는 디올뉴 차명(정제)를 조합칸으로 덮음. 손오공 재고시트 NAMES도 세부모델 있을 때 차명(정제)를 같이 씀. `--who=손오공 --apply` 1대. 구독재고 차명(정제)=`니로 EV SG2`. 세부트림 에어는 라이브 행 재조회 전에 안 넣음.

---

## 2026-09-02 — 클로드·코덱스·제미나이 CLI 연동

오더: 세 CLI를 커서가 부르게 연동.

`scripts/ai-cli.mts` — `claude -p` / `codex exec` / `gemini -p`. 기본 읽기 전용. 확인 `npm run ai:ping`.

---

## 2026-09-02 — 원문 철자 사수 (매뉴얼 + 게이트)

오더: 과거값·틀린 디올뉴가 다시 못 나오게 매뉴얼 강화하고 사수.

정본: `docs/차종명명-정제-매뉴얼.md` §1-0 — 세대=라이브 행, 철자=지금 원문, 상품마스터 차번 금지, 원문 없는 디올뉴 금지.
게이트: `npm run audit:raw-ad-prefix` (181허5305=`싼타페 MX5`) · hourly ①‴ · `check-vehicle-master-lock` (fill import·손오공 원문디올뉴·폐쇄기 철자).
손오공-정제도 원문 없는 디올뉴 접두를 안 붙임. 시트 「AI 운영 매뉴얼」 탭 재게시는 검토 후 `publish-ai-manual-tab --apply`.

---

## 2026-09-02 — 렌트존 싼타페 MX5 (원문 없는 디올뉴 덮음)

원인: 라이브 탭 철자가 `디 올 뉴 싼타페 MX5`라 정제칸·차명(정제)를 그걸로 박고, 빈칸만 규칙이 이미 적힌 칸을 안 되돌림. 폐쇄기가 `싼타페 MX5`를 라이브에 없다고 비우거나, 시트 값을 다시 디올뉴로 복구.

고침: 원문에 디올뉴가 없으면 스냅 `싼타페 MX5`로 덮음. 라이브 `디 올 뉴 싼타페 MX5`는 같은 세대로 인정하되 접두를 안 붙임. 원문에 디올뉴가 있으면 라이브 철자 유지(아이카). 렌트존 181허5305·5296 확인=`싼타페 MX5` / `싼타페 MX5 익스클루시브`. 폐쇄 미리보기 렌트존 0대(되돌리지 않음). 판매·ERP 재발행 없음.

---

## 2026-09-02 — 정제칸 빈 칸만 · 차명(정제) 과거데이터 차단

오더: 정제시트·제공시트 정제칸은 **빈 칸만** 채운다. 렌트존에서 차명(정제)가 지금 왼쪽 차명이 아닌 과거 값으로 박혀 있음.

원인: fill이 상품마스터 차량번호→코드(옛 매물)와 라이브 행 이름을 덮어 씀. 렌트존 싼타페 원문=`싼타페 MX5 26MY … 익스클루시브` 인데 세부모델·차명(정제)=`디 올 뉴 싼타페 MX5 …`(09-01 라이브 복사). 라이브 탭에는 `싼타페 MX5` 행이 없고 `디 올 뉴 싼타페 MX5`만 있음.

고침: `fill-supplier-ai-columns` 입력=지금 시트 왼쪽 차명 스냅만. 상품마스터/결정/규격채택 안 씀. 이미 글자 있는 칸은 안 덮음. 차명(정제)는 빈칸이거나 모델명만일 때만 조합. `--include-mirror --apply` 760대 · 채움 127칸 · 그대로 11387칸. 렌트존 이번 라운드 0칸(싼타페 `디 올 뉴`는 이미 채워져 빈칸 규칙으로 안 지움). 셀토스 세부모델은 라이브에 `셀토스 SP3` 없어 빈칸 유지.

판매·ERP 재발행 없음.

---

## 2026-09-01 — 라이브 이름 전수 0 (판매 접두깎기 + 손오공 시트 미반영)

게이트 `npm run audit:live-master-names` 픽업 0 · 구독 0 · 판매 0. 375어8085 = `G80`/`G80 RG3`/`기본형`. 아이카 109호5391 판매 = `디 올 뉴 싼타페 MX5`/`익스클루시브`(라이브 행).

원인 둘: ① 정제칸은 이미 라이브 이름인데 발행기 「AI 정제」가 `디 올 뉴 싼타페 MX5`→`싼타페 MX5` 로 깎음. ② 손오공 정제 JSON은 트림을 비웠는데 closer가 want(JSON)만 비교해 시트 옛 트림(블랙·트렌디)을 안 덮음.

고침: `ai-refine-guard` 접두떨기 거부 · 발행기 모델/세부모델/세부트림 치환 스킵 · closer는 시트 vs 닫힌 값을 비교. 손오공 35대 덮음. 판매 4탭 재발행(상품리스트 298 · 구독 72 · 픽업 141 · 오플 66). ERP는 일일 동기 ⑦만.

---

## 2026-09-01 — 라이브 행 복사 규칙을 자동화·클로드·코덱스 문서에 박음

375어8085 픽업 정제칸 = `G80` · `G80 RG3` · `기본형` · 라이브 행 일치(맞게 정제됨). ERP는 아침 동기 값이라 아직 일렉트리파이드일 수 있음 — 판매·ERP만 안 덮음.

클로드 참고: `docs/차종명명-정제-매뉴얼.md` §1-0 · `docs/자동동기-매뉴얼.md` ⓪·①′ · `lib/domain/ai-operating-manual.ts` 0‴장 · `CLAUDE.md` · `AGENTS.md` · `docs/AI_COLLABORATION.md`.
코덱스 검사: `npm run audit:live-master-names` (`scripts/audit-live-master-name-copy.mts`). 375어8085 게이트. 전수 0건은 아직 보고용.
자동화: hourly-sync ⓪ 이미 `손오공-정제.mjs`(라이브 `1T_RrE`). 시트 「AI 운영 매뉴얼」 탭 재게시는 `publish-ai-manual-tab --apply`(이번엔 리포만).

---

## 2026-09-01 — 불변 규칙: 라이브 차종마스터 행만 복사

오더: 모델·세부모델·세부트림은 라이브 「차종마스터」에 있는 행 값을 그대로. 없는 이름 생성·FL 붙이거나 떼기 금지. 판매·ERP만 고쳐 맞추기 금지.

손오공-정제가 F03(`1oMB9e`)을 읽고 있어 픽업 G80이 F03 전용 이름 `일렉트리파이드 G80 RG3`+`기본형`을 씀. 그 조합은 라이브 마스터에 없음(라이브 EV는 `G80 EV RG3` / `e 87.2kWh`).

라이브 375어8085 해당 행 = `G80` · `G80 RG3` · `기본형` (가솔린 3470 AWD, 2020-03~2023-11). 픽업 정제칸은 이미 이 세 값.

경로: `손오공-정제.mjs` 정본을 라이브 `1T_RrE` 「차종마스터」로 바꿈. 연료·연식 겹침에서 세부모델이 둘이면 안 고름. 픽업 G80 중 행이 하나로 모인 7대만 그 행 이름을 복사(2024→`G80 RG3 FL` 기본형, F03 `블랙`→라이브 `기본형`). 판매·ERP는 안 덮음.

검수대기(라이브 튜플에 없음, 비우지 않고 보고): 픽업 122 · 판매 236 · ERP 470. 2023년 G80은 RG3/FL 생산기간이 한 해에 겹쳐 월 없으면 행을 못 고름.

---

## 2026-09-01 — 픽업 G80 가솔린이 일렉트리파이드 정제칸을 쓰던 사고


원인: F03 「차종마스터」 G80 RG3 트림은 **블랙 한 줄**뿐이고, **기본형**은 `일렉트리파이드 G80 RG3`에만 있다. 손오공 정제 엔진(`sonokong/lib/vehicle-refine.mjs`)이 연식·트림만 보고 연료를 안 걸러, 롯데 트림 「기본형」인 가솔린 G80이 EV 행으로 붙었다. `fuelClash`는 `일렉트릭|EV`만 봐서 **일렉트리파이드**를 전기로 인정하지 못했다. fill-supplier-ai-columns 는 손오공 시트를 건너뛴다.

375어8085 원문=`제네시스_더 올뉴G80_가솔린 3.5 AWD_기본형` · 연료 가솔린 · 3470cc. 차명(정제)는 사장님이 먼저 `G80 RG3`로 고침. 세부모델 칸만 EV로 남아 있었음.

고침: 일렉트리파이드=전기 게이트 + 본매칭에도 연료 필터. 원문 트림이 그 세부모델 F03 행에 없으면 세부트림은 안 박음(G80 RG3에 기본형 없음). 픽업재고 세부모델 17칸 `일렉트리파이드 G80 RG3`→`G80 RG3`(가솔린만). 차명(정제)·세부트림은 안 덮음. ERP는 아직 아침 동기 값(`일렉트리파이드`) — 판매 픽업구독은 이미 `G80 RG3`.

---

## 2026-09-01 — FL 하류 표기 정정 (차종마스터 목록 → 정제칸 → 판매 4탭 → ERP)


오더: 라이브 차종마스터 「세부모델목록」을 이름 정본으로, 정제칸·상품시트·ERP 세부모델을 맞춘다. ERP/상품시트만 덮지 않음. 라이브 「차종마스터」 mf- 탭·F03 7열은 안 씀.

### 정본 판정 (원본을 직접 염)

| 원본 | FL 세부모델 | G80 DH | 올 뉴 카니발 YP |
|---|---|---|---|
| 라이브 「세부모델목록」 1763 | **0** | 있음 | 있음 |
| F03 「차종마스터」 | **0** | 없음(세부모델 `G80` / `G80 RG3` / `일렉트리파이드 G80 RG3`) | 없음(`더 뉴 카니발 YP` · `카니발 KA4` · `더 뉴 카니발 KA4`) |
| `public/data/vehicle-master.json` | 세부모델 0 · **별칭 7**(G80 RG3 FL 등 → 정본 G80 RG3) | 있음 | 있음 |
| `public/data/vehicle-trim-master.json` (`records`, 라이브 mf- 탭 스냅 2026-08-15) | **있음** (`G80 RG3 FL` · `GV70 JK1 FL` · `GV80 JX1 FL` · `모델 Y FL` · `모델 3 FL` 등) | — | — |

mf- 키 의미 변경·라이브 「차종마스터」 탭 쓰기는 금지라 trim-master FL 행은 안 고침. 이름은 목록/json 별칭이 이미 `G80 RG3` 쪽.

**엔카 원문 (카니발 3세대):** car.encar.com 검색 조건 `Model.올 뉴 카니발`, 매물 제목 `올 뉴 카니발 9인승 프레스티지` 등. **YP 글자 없음.** 4세대 슬러그는 `카니발.4세대`(세대명 있음). 333모5593 공급사 원문=`올 뉴 카니발 자가용 9인승 럭셔리 디젤 2 15MY`. 목록·시트·ERP는 `올 뉴 카니발 YP`. **엔카에 코드 없음 vs 우리 목록에 YP = 갈림 → 안 벗김**(임의 YP 추가/삭제는 사장님).

**G80 DH:** 24저4970 · 52부9200 정제칸·상품리스트·ERP 모두 `G80 DH` 유지. F03의 `G80`으로 안 합침.

### dry-run (변경 전)

공급사 정제칸 FL **25대** · 쓸 칸 **50**(세부모델+차명(정제)). 판매 4탭 FL **14대**(상품리스트 10 + 오플 4). ERP 당시 16대 중 판매 미수록분이 더 있었음.

판매 4탭 변경 전:
- `G80 RG3 FL` 161허1331 · 222호3260 · 125하1829 · 102우8575 · 102우8587
- `모델 Y FL` 33허0472/0475/0476/0477/0479/0480
- `모델 3 FL` 10호3819
- `GV70 JK1 FL` 102우8590 · `GV80 JX1 FL` 102우8511
- 유지 `G80 DH` 24저4970 · 52부9200 · `올 뉴 카니발 YP` 333모5593

판매 밖 정제칸도 같이 목록 이름으로: 이안카 33허0471/0473/0474/0478 · 10하8210, 아이카 109호3261/109호3770, 오플 215거1381/192거5102, 스타 142호8434, 퍼시픽 161하3805.

근거: 목록에 `G80 RG3` · `모델 Y` · `모델 3` · `GV70 JK1` · `GV80 JX1` 실재. 원문에 FL 없음(`뉴 G80` · `신형 GV70` · `모델 Y RWD Premium` 등).

### 반영

1. 정제칸 50칸 `--apply` (공급사 시트만).
2. 판매 4탭 재발행: 상품리스트 09.01 10:12 · **299대** / 손오공구독 71 / 픽업구독 144 / 오플구독 71 + 요금 블록.
3. `run-sheet-daily-sync-local --apply` — 원본 538 · 갱신 536 · 백업 `run_pj97n25dwg`. 빌린카(RP021)는 **임시번호 신원서명 불일치 2건**으로 상태·가격 보류. 차명은 판매시트 기준이며 222호3260 ERP는 `G80 RG3`로 확인.

### 변경 후 재검증

판매 4탭 FL **0**. 지정 차번:

| 차량번호 | 전 | 후 (정제=판매=ERP, 주) |
|---|---|---|
| 161허1331 | G80 RG3 FL | G80 RG3 |
| 102우8575 · 102우8587 | G80 RG3 FL | G80 RG3 |
| 222호3260 · 125하1829 | G80 RG3 FL | G80 RG3 |
| 10호3819 | 모델 3 FL | 모델 3 |
| 33허0472/0475–0480 | 모델 Y FL | 모델 Y |
| 102우8590 | GV70 JK1 FL | GV70 JK1 |
| 102우8511 | GV80 JX1 FL | GV80 JX1 |
| 24저4970 · 52부9200 | G80 DH | G80 DH |
| 333모5593 | 올 뉴 카니발 YP | 올 뉴 카니발 YP (엔카 원문은 YP 없음 · 목록 유지) |

ERP FL 잔여 **3대** — 이안카 `33허0471` · `33허0473` · `33허0478` `모델 Y FL`. 정제칸은 `모델 Y`로 고쳤으나 **판매 4탭에 없음**(출고불가 제외)이라 일일동기가 안 덮음. ERP만 고치지 않음.

### 게이트

- 변경 전 dry-run: 쓸 칸 50 · G80 DH/카니발 YP keep
- `npx tsc --noEmit` PASS
- `npx tsx scripts/verify-master-pass.mts` **PASS** (`G80 RG3 FL`→`G80 RG3` 등)
- `npm run check:sync` PASS (축 18 어긋남 0)
- `npm run audit:axes` PASS (세부모델 정제/판매/ERP 100%)
- `npm run check:fonts` PASS
- `npx tsx scripts/check-vehicle-master-lock.mts` **어긋남 11** — 이번 작업과 무관. 잠금기가 `vehicle-trim-master.json`을 `entries[]`(이름사전)로 읽는데 실제 스키마는 `records[]`. 라이브 mf- 탭 안 고침.

---

## 2026-08-29 — 라운드5 확정 원자 정제칸 (Cursor)

오더 `docs/crosscheck/오더-cursor-정제칸-확정원자-2026-08-29.md`. 이름 = F03 확정 원자 시트. `round4Pool` 버그 수정. 원문→확정 1628만 매칭. 자동후보 632 · 검수대기 521 · 정제칸 238칸. F01 상품리스트·손오공구독·오플구독·천이. 픽업구독은 284→152 shrink 가드로 미발행. 상세 `docs/crosscheck/정제-수확량-루프-2026-08-29.md` 라운드 5.

---

## 2026-08-29 — F03 커서 지식검토·엔카대조 1668행

사장님: 지식검토와 엔카검토를 다시. 시트 칸은 이미 `클로드/커서/코덱스 × 지식검토·엔카대조`. 이름 7열·클로드·코덱스는 안 씀.

- 커서 지식검토: 매뉴얼만. `맞음(규칙) 1620` · `못정함 25` · `맞음(예외·합의안) 13` · `틀림 10`(더 뉴 기아 레이 제조사 누출).
- 커서 엔카대조: 라이브 iNav 114모델. `맞음 1619` · `틀림 49`(K8에 GL3 붙임 · 더 뉴 카니발 YP · RAV4/포터/봉고/볼트 조회 0).
- 갈림 예: 레이는 지식 틀림·엔카 맞음원문. G80 RG3는 지식 못정함·엔카 `G80 (RG3)`→`G80 RG3`.

---

## 2026-08-29 — F03 차종마스터 행마다 검수·AI 의견칸

사장님: 차종마스터 한 줄에 검수·클로드·커서·코덱스/제미나이 의견값을 넣을 수 있게. 이름 7열은 그대로. 오른쪽 4칸만 추가.

- 칸: `검수` · `클로드` · `커서` · `코덱스/제미나이`. 드롭다운 힌트(맞음/틀림/못정함, 검수는 대기/합의/갈림/사람/확정) · 긴 글도 가능.
- `npx tsx scripts/ensure-encar-master-review-columns.mts --apply` · 의견만 `write-encar-master-review --who=커서 --value=…`
- 공급사 연동 수식은 이름 7열만(`A:G`). 기존 `A:AB` 사본은 재게시 전까지 검수칸이 보일 수 있음.

---

## 2026-08-29 — 정제 감사기 F03 7열+제원마스터 계약 (읽기 전용)

오더 `docs/crosscheck/오더-cursor-감사기-F03-2026-08-29.md`. 옛 `차종마스터!A:AB` 제원열 전제를 버리고, 이름 7열 + 「제원마스터」 구분·값으로 갈랐다. 산출 버킷 자동후보/검수대기/읽기실패. 시트 쓰기 없음.

- 전 행 1153 · 자동후보 724 · 검수대기 429 · 읽기실패 0. 출고불가 제외 580/363/217.
- 상세 `docs/crosscheck/정제-수확량-루프-2026-08-29.md` 라운드 2 구현.

---

## 2026-08-31 — 간단카드 CORE 뱃지 사진 우하 원복

사장님: 웹보기 출고가능·중고렌트·미입력은 원래 위치 = 썸네일 우측 하단.

`a466b0aa`가 본문 첫 줄 아이콘+글자로 내린 것을 되돌림. `ProductCard` + `CardThumb coreBadges`만. 가로카드(상세) `withCredit`은 그대로.

---

## 2026-08-29 — 웹 영업자 패널 머리띠 원복

웹 우측 칼럼 영업 정보·계약 조건은 원래 반전 남색. 회색으로 같이 바꾼 것을 `sideCol`일 때만 invert로 되돌림. 좁은 화면은 회색 유지.

---

## 2026-08-29 — 차량스펙·대여료조건 머리띠 남색 강조

사장님: 차량스펙·대여료스펙은 강조해야 함. 헤더 남색.

- 배포본: `ProductDetail` 차량스펙 `headTone="invert"` · `ProductPriceTable`(대여료조건) 같은 반전. 보험·계약·기타·영업자 패널은 회색 유지.
- feat: `DetailTable` `accent="main"` → 반전 남색 복구(이미 차량스펙·대여료조건만 main).

---

## 2026-08-29 — 상세 섹션 머리띠 회색 복구

사장님: 수정 전 섹션헤드 색이 아니었음.

- 배포본 `ProductAgentPanel`: 영업 정보·계약 조건 `headTone="invert"`(반전 남색)를 빼고 본문과 같은 회색(`plain`). 좁은 화면은 `tone="main"`(흰 카드). 우측 칼럼만 `tone="agent"`(좌측 네이비 바).
- feat `DetailTable`: `accent="main"`이 머리띠를 다시 남색 반전시키던 잔여를 제거(08-22 「머리띠는 원래 회색 하나」와 맞춤). 차량스펙·대여료조건도 회색.

---

## 2026-08-29 — 기본스펙 칸 검수·보완

공급사 정제칸(기본스펙) 전수. 원문+엔카와 다른 칸만 바로잡음. `--only=` 축 단위, `--skip-plate=` 로 보류.

- 반영: 세부모델 297(개발코드 떼기·엔카 글자) · 모델 1(아이오닉→아이오닉5) · 세부트림 41 · 연료 3 · 인승 3 · 배터리 1 · 배기량 3(원문 2.0인데 1.6) · 구동 FWD/2륜→2WD 등. 빈 칸 배기·구동도 다시 채움(미러가 비운 듯).
- 안 덮음 11: 리더스 125호1238 트렌디(제보) · 이안카 10하8161 Long Range · 아이언 K5 배기 1598 · 오플 C220d 2000 · 4WD/4륜→AWD 7대.
- 재-dry-run 바로잡음 11·빈칸 0. 판매시트 발행 안 함. 연카 정제칸 없음.
...
## 2026-08-21 — 운영 배포가 어제(08-20 14:13)에 멈춰 있던 것 발견 → 작업트리 일괄 배포 (사장님 「로컬이랑 다르다, 이거저거 묶어서 배포」, Claude Code)
- 실측: freepasserp.com 별칭이 08-20 14:13 배포(dpl_2rpRj…)에 걸려 있었고, 그 뒤 바뀐 파일 136개(app/erp5 신설·esign API·감사·FAQ·채팅 등 여러 창 작업분)가 미배포.
- tsc 0 → 로컬 `next build` EXIT 0(39페이지) → `npx vercel --prod --yes --archive=tgz` → **08-21 21:30 배포·Aliased** ✓ (`/` 307 → /login 정상).
- ※ 운영은 여전히 «작업트리 CLI 배포»라 어느 창이든 「배포까지」라고 말해야 나간다 — 배포를 빼먹으면 이번처럼 로컬↔운영이 갈린다. 커밋/메인 반영은 여전히 미결(340+ 파일).
- (추가 22:10) 사장님 「코덱스가 모바일 화면 얼룩무늬 해놓고 굳이 라인 없어도 되는 곳에 라인」 → 실측: ① 목록 지브라 `.fp-card.fp-card-row:nth-child(even)`(3ffb6fc1, 07-22) — 행 하단선과 겹쳐 이중 → **주석 처리**(하단선만, globals.css 되살리기 주석 포함) ② 모바일 상세 섹션 머리띠 밑줄(DT.band borderBottom + 11140376 의 ACCENT borderBottomColor) — 띠 배경이 이미 경계 → **제거**(표 구조선 colTh·split·행 tr·Disclosure 하단선은 유지) ③ 판매시트 보기(SheetView) 격자는 구글시트 모사라 의도된 것 — 유지. 배포 08-21 22:0x Aliased ✓.

## 2026-08-22 — 모바일 목록 여백 2배 버그 (사장님 「모바일에서 여백을 더 준 거 같은데」, Claude Code)
- 원인: b507cd06(perf)이 목록 행을 겉 div+속 Link 두 겹으로 만들며 **둘 다에 padding 10×12** — 행 여백이 상하 20·좌우 24로 두 배. 지브라 제거 뒤에도 「여백 커짐」으로 보인 실체.
- 고침: 여백·gap 은 속 Link 한 곳에만, 겉 div 는 하단선·position 만(ProductRowCard). 그리드 카드(ProductCard)는 이중 아님 확인. tsc 0 → 배포 Aliased ✓.
- (추가) 사장님 「검색창이랑 이런데 라인이 없어야 · 색깔도 · 원래 상태랑 봐봐, 기능만 효율화하라고 했더니」 → 886c782c(08-20, 디자인 손대기 전)와 전수 대조: 이번에 새로 생긴 시각 변화 = ① 목록 행 여백 2배(수정 완료) ② 상세 섹션 색 머리띠+밑줄(11140376) ③ 카드 사진 위 스펙 알약의 1px 세로선(사진 위 유리 알약 — 의도된 것, 유지) ④ 지브라(전날 제거). 검색창 테두리는 7월부터 있던 것 — 사장님 지시대로 **모바일은 테두리 없는 알약**(bg-sunken 바탕·포커스 때만 테두리·링, 웹은 종전대로), 상세 머리띠는 **원래 회색 하나로**(ACCENT 색 밴드·아이콘 색 되돌림). tsc 0 → 배포 Aliased ✓. 교훈: perf 오더에 시각 변경이 섞여 들어옴 — 코덱스 오더는 「기능만, 스타일 diff 0」을 명시할 것.
- (추가) 검색창 원상복구 최종: 알약형(내 과잉) 철회 → 원래의 각진 입력칸(흰 바탕·얇은 테두리). 파란 배경의 정체 = 305caf4f(08-19)가 넣은 `inputStyle={{ background: C.selected }}` — 제거. 모바일 퀵필터 4개는 넣었다가 사장님 「모바일은 퀵필터 넣지 말자」로 즉시 철회(웹 퀵필터는 그대로, FinderQuickFilters 의 mobile 프롭·is-mobile CSS 는 무해하게 남음). 모바일 상단 = 검색창+필터 버튼만. 배포 Aliased ✓.
- (추가) 사장님 모바일 검수 오더(상단바=하단 라인만 · 하단바=상단 라인만 · 햄버거·아이콘 배치 · 좌우 배열): ① 상단바에 라인+그림자(--shadow-sm) 이중 → 그림자 제거 ② 하단 탭바도 라인+그림자(SH.dock) 이중 → 그림자 제거 ③ 상단바 좌우 패딩 12/14 비대칭 → 14/14 대칭 ④ 검색창 바깥 회색 띠 → 흰색(.fp-finder-toolbar bg-card). 햄버거 = 모바일 우측 끝·웹 좌측(기존 설계 유지, 뒤로가기는 하단독) — 배치 자체는 규칙대로라 손 안 댐. 배포 Aliased ✓.
- (추가 13:1x) 사장님 「상세페이지 하단에 이전 + 공유·텍스트복사 맞게」 → /m/[code] 하단독 = 이전 + **링크 복사·텍스트 복사**(ProductAgentShareActions 재사용, dynamic import) + 영업자는 검수 요청 유지. (08-20 「검수 요청만」 결정을 사장님 새 지시로 뒤집음 — 모바일에선 패널이 본문 끝이라 손이 안 닿았음.) 배포 13:1x Aliased ✓. ※ 운영 확인은 이제 freepasserp.com 에서 — 수정마다 배포 필수.
- (추가 13:40) 사장님 「캐시를 무력화하면서 개선해야지 아직 그대론데」 → 실측: 서비스워커 없음·HTML no-store — 문제는 **살아 있는 탭**(폰·카톡 인앱·홈화면 앱)이 옛 번들로 계속 도는 것 + 우리 배포는 커밋 없이 작업트리째라 BUILD_NO/SHA 가 배포 간 동일(CLI 배포라 git 정보도 빈 값). → **배포 도장** `NEXT_PUBLIC_BUILD_STAMP`(빌드 시각, next.config env — 배포마다 필히 바뀜) + `/api/version`(no-store) + `VersionWatcher`(layout 상주: visible/focus/online + 90초 간격으로 서버 도장 대조 → 다르면 스스로 새로고침 · 입력 중이면 미룸 · 같은 도장 1회 가드) 배포 13:37 Aliased ✓, /api/version stamp 응답 확인. **이번 배포부터는 열려 있던 화면도 90초 안(또는 앱 복귀 즉시)에 자동 갱신** — 단 지금 떠 있는 옛 화면엔 감시기가 없으므로 이번 한 번만 수동 새로고침 필요.
- (추가 14:2x) 사장님 「좌우 여백·배열 안 맞고 · 검색창 감싼 연회색 배경 없애고 · 검색창 하단 라인 없다」 → 실측(로컬 412px Playwright 렌더+픽셀 측정): 회색 띠 = @media(max-width:760px) 블록이 `.fp-finder-toolbar` 배경을 --bg-page(#EAEDF2)로 «나중에» 다시 덮고 있었음(내 최상위 오버라이드가 짐). 고침: 모바일 미디어 블록에서 배경 --bg-card·border-bottom 0(검색 줄이 목록과 한 면) · 상단바 모바일 좌우 12px(검색창·목록 12px와 같은 세로선). 재캡처로 검증(띠 255,255,255) 후 배포 14:18 Aliased ✓. 교훈: CSS 오버라이드는 미디어쿼리 안 나중 규칙까지 확인 — 컴퓨티드 스타일로 검증할 것.
- (추가 14:5x) 사장님 「버튼·검색창·상품카드 정렬 잘 보고 여백 최소화(가독성 유지)」 → 실측(Playwright 412px + getBoundingClientRect): 상단바 56·검색줄 56·행 102·탭바 57. 조임: 모바일 --fp-bar-pad-y 10→6(바 3개 56→48, 컨트롤 36 유지) · 행 padding 10→8·안쪽 gap 5→4(행 102→95, 글자 크기 불변) · 전 요소 좌우 12px 한 선. 재측정 48/48/36/95 확인·캡처 검수 후 배포 Aliased ✓.
- (추가 15:1x) 사장님 「목록을 라인 말고 박스로, 웹이랑 규격 맞춰서 · 상단헤더 좌우배열도」 → MobileRow = WebRow 와 같은 박스(테두리 C.line·R_CARD·SH.cardRest), 피드 = grid gap 8 · 좌우 12(헤더·검색창과 같은 세로선) · 상단선 제거. 밀도 유지(행 안 8×12·gap4·바 48). QA 캡처 검수 후 배포 Aliased ✓.
