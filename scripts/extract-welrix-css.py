...
/* 옵션 줄의 «설명»과 «못 고르는 까닭» — 원본 `.option-row` 에는 없던 두 줄이다.
   ⚠ 왜 안 되는지 안 적으면 사람이 「고장났다」고 읽는다(사장님 2026-09-09 「명확하게 다 구현」). */
.wx-root .option-row .o-sub { font-size: 11px; color: var(--ink-4); margin-top: 2px; line-height: 1.4; }
.wx-root .option-row .o-why { font-size: 11px; color: var(--warn, #b45309); margin-top: 2px; line-height: 1.4; }
.wx-root .option-row.disabled .o-name { color: var(--ink-4); }
