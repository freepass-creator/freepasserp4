...
/**
 * ⚠⚠ **2026-09-07 — 렌트 여덟 줄의 기대값이 엑셀과 갈렸다. 엑셀이 틀렸다.**
 *
 * 이 회귀 차량은 **cc 2,500** 이다. 엑셀 원식은 영업용 자동차세를 `IF(cc<2500, 19, 24)` 로 써서
 * **정확히 2,500cc 인 차에 24원**(최고 구간)을 물렸다. 지방세법 제127조는 「**2,500cc 이하** 19원」이다.
 * ⇒ 엑셀이 그 차의 자동차세를 **과다 부과**하고 있었고, 우리 견적이 그만큼 비쌌다.
 *   (2026-09-07 코덱스 대조에서 잡혔다 · `data/cost-config.js` CHANNEL.rent.cartaxBrackets)
 *
 * ★그래서 여기 기대값을 **법대로** 바꾼다. 「엑셀 무손실 이관」은 엑셀의 «오류»까지 지키자는 규칙이 아니다.
 *   바뀐 폭은 월 1,000~2,000원(0.2%)이고, 구독 여덟 줄은 그대로다(그 차는 경계에 안 걸린다).
...
// ← 법정 경계(2,500cc 이하 19원) 반영 — 위 머리말
// ★★2026-09-17 **자차 자체충당이 «해마다 줄어든 시세»에 걸리게 바뀜다** — 사장님
//   「연간 1.5% 충당한다는 거는 **감가된 금액의 시세의 1.5%**야. 3천만 원짜리 차가 첫 해는 3천만 원,
//    두 번째는 2,500만 원, 세 번째는 2천만 원에 1.5% 이렇게 산출이 된다고」.
//   여태는 «첫해 값 × 연수»라 뒤로 갈수록 과대했다. 렌트 열여덟이 그만큼 낮아졌다(월 1.8~2.1만).
//   구독은 자차가 0 이라 그대로다 — 그것이 이 변경이 «자차만» 건드렸다는 증거다.
[[24, 909000], [36, 811000], [48, 762000], [60, 713000]].forEach(([t, w]) =>
...
// ← 같은 까닭
// ← 같은 자차 규칙 변경(2026-09-17)
[[24, 2075000], [36, 1448000], [48, 1134000], [60, 945000]].forEach(([t, w]) =>
