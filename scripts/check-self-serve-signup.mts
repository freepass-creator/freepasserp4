...
const activationRoute = readFileSync(new URL('../app/api/auth/self-activate/route.ts', import.meta.url), 'utf8');
...
check('로그인 부팅이 기존 자가가입 자동 활성화 API를 호출', /\/api\/auth\/self-activate/.test(authSource));
check('자동 활성화 API는 Firebase ID token을 검증', /verifyIdToken\(token\)/.test(activationRoute));
check('자동 활성화 API는 transaction 안에서 조건을 재검사', /profileRef\.transaction/.test(activationRoute));
...
const legacyPending = {
  uid: 'uid-self-serve', role: 'provider', status: 'pending', requested_type: '공급사',
  created_at: 1_780_000_000_000, user_code: 'U0042',
};
check('소속 없는 구 자가가입 pending은 자동 활성화 대상',
  selfServeActivationDecision(legacyPending, 'uid-self-serve').eligible);
check('다른 uid의 pending 계정은 자동 활성화 불가',
  !selfServeActivationDecision(legacyPending, 'different-uid').eligible);
check('소속이 이미 지정된 pending 계정은 자동 활성화 불가',
  !selfServeActivationDecision({ ...legacyPending, company_code: 'SP001' }, 'uid-self-serve').eligible);
check('관리자가 중지한 pending 계정은 자동 활성화 불가',
  !selfServeActivationDecision({ ...legacyPending, is_active: '아니오' }, 'uid-self-serve').eligible);
check('삭제·반려 계정은 자동 활성화 불가',
  !selfServeActivationDecision({ ...legacyPending, status: 'rejected' }, 'uid-self-serve').eligible);
