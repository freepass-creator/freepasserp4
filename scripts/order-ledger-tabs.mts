 ...
 *
 * ★일하는 순서대로 놓는다 — 접수 → 취소 → 분납실적 → 완료실적 → 청구 → 참고표.
...
const ORDER = ['이 시트는', '접수', '취소', '분납실적', '완료실적', '청구', '청구요약', '월별 요약', '수수료표'];
...
const GUIDE: string[][] = [
  ...
  ['', '**인도완료를 체크하면 그 줄은 바로 나간다** — 분납이면 「분납실적」, 일시납이면 「완료실적」으로.'],
  ...
  ['★청구서의 원천', '**분납실적 + 완료실적**이다. 접수 탭은 아직 청구가 못 나가는 것들이라 안 본다.'],
  ...
  ['', '만료 = 인도일 + 회차개월. 그날이 지나고 환수가 없으면 이행된 것이라 저절로 완료실적으로 간다.'],
  ['★분납이 부러지면', '「다음회차일」이 다음에 돈이 들어올 날이다. 그 날이 지났는데 소식이 없으면 —'],
  ['', '**「환수」에 체크하고 환수사유·환수일·환수금액을 적는다.** 분납이 부러진 것도 환수로 적는다.'],
  ['', '켜면 줄이 주황이 되고, 청구에서 그만큼 빠지며, 더 굴러갈 회차가 없어 **완료실적으로 옮겨진다.**'],
  ['', '환수사유 — 분납 미납 · 대여료 미납 · 중도해지 · 계약위반 · 사고 · 고객요청 · 기타. 목록에 없으면 그냥 적는다.'],
  ['', '⚠ 회차별 입금 자료가 우리에게 없다. 기계는 «언제 확인할지»만 알려 주고, 들어왔는지는 사람이 안다.'],
  ['④ 완료실적', '끝난 것. 일시납이거나 분납 만료가 지났다. 환수도 여기 남는다. 쌓기만 한다.'],
  ['⑤ 청구', '청구월로 쌓는 장부. 분납실적 + 완료실적에서 만든다. 2026-08부터 담는다 — 그 전은 이미 청구가 끝났다.'],
  ['', '⚠ 환수는 여기서 안 적는다 — **분납실적·완료실적**에서 「환수」에 체크하고 환수일·환수금액을 적으면 따라온다.'],
