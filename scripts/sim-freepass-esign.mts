...
assert.match(publicRoute, /birthDate\(payload\.customer_id\)/);
assert.match(publicRoute, /const ageReference = S\(templateFields\.contract_start\) \|\| S\(templateFields\.contract_date\)/);
assert.match(publicRoute, /ageOnBirthDate\(customerBirth \|\| '', ageReference\)/);
assert.match(publicRoute, /sales_proof_method/);
assert.match(publicRoute, /residentIdEncrypted: encryptRrn\(resident\.digits\)/);
assert.match(publicRoute, /sales_proof: parsed\.salesProof/);
assert.doesNotMatch(publicRoute, /customer_id: encryptRrn/);
...
assert.match(publicPage, /서명할 계약서 보기/);
...
// 조건값은 축약본을 새로 만들지 않는다. 카드 값 칸에 원문을 두고, article은 조문 참조로만 표시한다.
assert.match(publicPage, /function conditionValue\(value: string, article\?: string\)/);
assert.match(publicPage, /function ContractSection\(/);
...
assert.match(publicPage, /세금계산서 발행 정보를 모두 입력해 주세요/);
assert.match(publicPage, /tax_biz_name: '', tax_biz_no: ''/);
assert.match(publicPage, /개인사업자입니다/);
assert.match(publicPage, /fmtRrn\(event\.target\.value\)/);
assert.match(publicPage, /fmtPhone\(event\.target\.value\)/);
...
assert.match(publicPage, /면허증 촬영/);
assert.match(publicPage, /aria-label="추가 운전자 개인정보 제공·면허증 제출 동의"/);
assert.match(publicPage, /모바일 계약서 전체보기/);
assert.match(publicPage, /계약서 미리보기/);
assert.doesNotMatch(publicPage, /A4 계약서 미리보기/);
assert.match(publicPage, /관리자 미리보기입니다/);
...
assert.match(publicPage, /약관을 확인해 주세요/);
assert.match(publicPage, /계약서에 들어갈 정보를 알려 주세요/);
...
assert.match(publicDocumentRoute, /customerCopyExpiresAt/);
assert.match(publicDocumentRoute, /customer_document_accessed/);
