...
import { SALES_EXCLUDE, SALES_MAPPING } from '../lib/domain/sales-sheet-mapping';
...
const ROWS: Row[] = [
  ...
  ['', '', ''],
  ['', '정렬 — 왼쪽', '제조사 · 모델 · 세부모델 · 파워트레인 · 세부트림 · 옵션 (긴 글. 가운데면 줄마다 시작 위치가 달라 세로로 못 훑는다)'],
  ...
  ['7. 차종마스터', '정본', 'public/data/vehicle-master.json (리포 파일). RTDB vehicle_master는 v3 잔재 껍데기다 — 보지 마라.'],
  ['', '축', '제조사 → 모델 → 세부모델 → 파워트레인 → 세부트림. 시트도 같은 차례로 서 있다.'],
  ['', '표기 사전', 'public/data/master-aliases.json — 「매물이 쓴 말 → 마스터가 쓰는 말」을 누적한다.'],
  ['', '규칙 넓이', '제조사·모델·세부모델까지 묶어 좁게 건다. 「트렌디」가 K7에서는 「LPI 트렌디(렌터카)」지만 셀토스에서는 그냥 「트렌디」다.'],
  ['', '사람이 정한 것', 'reviewed 규칙은 기계가 못 덮는다.'],
  ['', '⚠ 차종 판단', 'ERP 값을 근거로 삼지 마라. 공급사 원문을 먼저 봐라 — ERP의 잘못 붙은 「하이브리드 3.0 4MATIC」을 근거로 E200을 CLE450이라 판단한 적이 있다.'],
  ['', '감사', 'npx tsx scripts/audit-master-gap.mts  (2026-08-13 기준 100%)'],
  ['', '', ''],
  ...
  ['', '28열', '차량번호 | 입고일자 | 상태 | 분류 | 제조사 | 차명(트림) | 외부색상 | 내부색상 | 연식 | 연료 | 옵션 | 주행거리 | 배기량 | 차량가격 | 단기보증 | 1개월 | 12개월 | 장기보증 | 24개월 | 36개월 | 48개월 | 60개월 | 기타기간①②③ | 정책코드 | 최초등록일 | 사진링크'],
  ...
  ['', '⚠ 남의 시트', '우리 제공시트만 고친다. 가르는 표식은 열 이름 「차명(트림)」. 소유자로 거르지 마라 — 우리 것도 회사 계정 여럿에 흩어져 있어 전부 막힌다.'],
  ...
  ['', '아이언', '홈페이지 수집이라 시트가 없다. ironrentcar.com이 정본.'],
