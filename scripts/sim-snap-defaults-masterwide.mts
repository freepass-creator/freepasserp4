...
const seatAxis = master.filter((e) => seatAxisMatters(e) && choicesOf(e.sub_model, master).seats.length >= 2);
...
for (const e of seatAxis) {
  ...
  // 이 검사는 «인승이 있는 조합에서 인승 칸만 비움»이다. 인승 자체가 없는 화물/파생 variant를
  // 고르면 빈칸이 정답이므로, default 인승 조합 → 첫 인승 조합 순으로 입력 파워트레인을 잡는다.
  const v0 = (e.variants || []).find((v) => v.default === true && v.seat != null)
    || (e.variants || []).find((v) => v.seat != null)
    || (e.variants || [])[0];
  ...
  if (got && ch.seats.includes(got) && (applied?._snap_defaults as any)?.seats === true) seatOk++;
  else {
    seatFail++;
...
for (const e of driveAxis) {
  ...
  if (got && ch.drives.includes(got) && (applied?._snap_defaults as any)?.drive_type === true) driveOk++;
  else {
    driveFail++;
    fails.push(`DRV ${e.sub_model}→${S(snap.sub_model)} got=${S(applied?.drive_type)} choices=${ch.drives.join('/')}`);
  }
}

console.log(`\n인승축 빈칸→마스터 조합  OK ${seatOk} · FAIL ${seatFail}`);
console.log(`구동축 빈칸→마스터 조합  OK ${driveOk} · FAIL ${driveFail}`);
if (fails.length) {
  console.log('\n실패 표본:');
