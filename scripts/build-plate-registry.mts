/**
 * ⛔ **[안 씀 — 2026-08-28]** 사장님이 차량대장을 걷어냈다
 *    (「차량대장은 없어도 됨 · 그냥 수기로 입력하게 하자 · 직접 입력하는거로 얼마 힘들지 않으니까」).
 *    `run-settlement-sheet` 의 차례에서 빠졌다. **파이프라인에 다시 넣지 말 것** —
 *    되살리려면 사람이 정하고, ①대장 → ②탭 정리 → ③대장 얹기 차례를 지켜야 한다.
 *    파일을 남겨 두는 이유는 되돌릴 곳이 필요해서다(지우면 다시 만들어야 한다).
 */
/**
