...
assert.equal(sa?.variant, '', '판매시트 직접 정본은 폐지한 파워트레인 축을 다시 만들지 않는다');
...
assert.equal(sourceContractProduct?.vehicle_status, '계약중');
...
const workbook = importSalesInventoryWorkbook({
  ...
  sonogong: {
    title: '손오공상품 08.13 · 1대', gid: '2', rows: [
  ...
  autoplusMain: {
    title: '오플구독 08.13 · 1대', gid: '3', rows: [
      ...
      ['1', '48나2076', '니로 EV', '디 올뉴니로EV 에어', '검정', 'EV', '2023-07-14', '8,582', '판매중', '770,000', '730,000', '690,000', '690,000', '기본-컴포트', '기아', '국산'],
    ],
  },
  partners,
  entries,
});
assert.equal(workbook.products.length, 4);
assert.equal(workbook.lines.length, 4, 'line은 탭 수가 아니라 공급사 수다');
assert.equal(workbook.lines.find((line) => line.code === 'RP012')?.imported, 1);
assert.equal(workbook.lines.find((line) => line.code === 'RP023')?.imported, 1);
...
assert.equal((workbook.products.find((product) => product.car_number === '48나2076')?.price as Record<string, { rent?: number }>)?.['12_3만']?.rent, 770_000);
