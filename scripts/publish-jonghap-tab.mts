 ...
 *
 * ★영업자가 보는 표는 **이것 하나뿐이다**(사장님 2026-08-12 — 「신버전 구버전 없이 하나로,
 *   과거 하던 형태로」). 예전에 신버전(조회바·요약블록)을 나란히 두었는데 두 벌을 찍다가
 *   한쪽만 갱신돼 영업자가 옛 값을 봤다 — 오플 유령 48개월이 그렇게 남아 있었다.
...
const COLUMNS = [
   ...
   *   「영업자들이 그냥 기존걸 원했어 · 이 스타일대로 · 변한게 없는거처럼 보여지게」.
   *   순서·이름을 임의로 바꾸지 마라. 손에 익은 자리가 곧 그 표의 값어치다.
   * ★원본에 없어 뺀 것: 연식 · 파워트레인 · 정책코드(세부모델·트림에 이미 들어 있다).
   * ★「입고일자」는 자리만 지키고 비운다 — 원본도 비어 있고, 영업자용에는 필요 없는 값이다.
   * ★「공급사」 하나만 더했다. 원본은 아이카 한 곳만 담아 필요 없었지만
   *   우리 표는 17곳을 합치므로 이게 없으면 어느 회사 차인지 알 수가 없다.
   */
...
function cells(p: EntityRecord): Record<string, string> {
  ...
  return {
    ...
    단기보증: dep, '1개월': rent(1), '6개월': rent(6), '12개월': rent(12),
    장기보증: dep, '24개월': rent(24), '36개월': rent(36), '48개월': rent(48), '60개월': rent(60),
    입고일자: '', 트림: S(r.trim_name) || S(r.variant), 옵션: S(r.options),
...
const RIGHT = new Set(['Km', '6개월', '단기보증', '1개월', '12개월', '장기보증', '24개월', '36개월', '48개월', '60개월', '소비자가격', '배기량']);
/** 월 대여료만 굵게. 보증금까지 굵히면 어느 게 월 요금인지 흐려진다. */
const RENT = new Set(['1개월', '6개월', '12개월', '24개월', '36개월', '48개월', '60개월']);
/**
...
const title = `상품리스트 ${kst.slice(5, 10).replace('-', '.')} ${kst.slice(11, 16)} · ${rows.length}대`;
