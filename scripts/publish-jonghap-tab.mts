...
const COLUMNS = [
  /**
   * ★입고일자·재고일수는 **영업자 표에 넣지 않는다**(사장님 2026-08-12 —
   *   「영업자용에는 입고일자가 필요없음 · 공급사만 내부관리용으로 필요한거야」).
   *   영업자는 손님에게 팔 것을 본다. 이 차가 며칠째 서 있는지는 우리가 공급사를 관리하는 값이다.
   *   두 칸은 공급사 시트에만 둔다.
   */
  '상태', '구분', '차량번호', '차종분류', '세부모델', '외장', '내장', '연식', '연료', 'Km',
