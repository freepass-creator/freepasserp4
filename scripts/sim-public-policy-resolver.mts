...
assert.equal(embedded.policy, null, '상품 내장 파생 정책을 정책 원장처럼 사용했습니다.');
assert.equal(embedded.resolution.status, 'hold-ambiguous-or-missing', '정책 없는 상품이 HOLD가 아닙니다.');
...
const guest = sanitizeProductForGuest('veh_test', { policy_code: 'POL_S01' }, linked.policy);
assert.equal((guest._policy as Rec)?.insurance_included, '보험료 포함', '해소된 보험값이 손님 응답에서 누락됐습니다.');
const sparseGuest = sanitizeProductForGuest('veh_sparse', {}, embedded.policy);
assert.equal(sparseGuest._policy, undefined, '정책 원장 없는 상품이 정책을 공개했습니다.');
const redacted = sanitizeProductForGuest('veh_redacted', {}, { insurance_included: '보험료 포함', fee_rate: 99, partner_memo: 'internal' });
assert.deepEqual(redacted._policy, { insurance_included: '보험료 포함' }, '내장 정책 fallback에서 내부 정책값이 공개됐습니다.');
console.log('PASS public policy resolver: code/doc-id/padding/auto/ambiguous/HOLD/public boundary');
