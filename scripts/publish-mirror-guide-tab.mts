...
const NOTES: Record<string, string[]> = {
  RP004: [
    ...
    '원본 「월렌트」 칸은 우리 「1개월」, 「소비자가격」은 「차량가격」, 「트림」(+차종분류)은 「차명(트림)」, 「최초등록 26-7-24」는 「2026-07-24」로 온다.',
  ...
  RP031: [
    ...
    '「차종분류 + 세부모델 + 트림」이 「차명(트림)」 한 칸으로 합쳐 온다(겹치는 말은 한 번만).',
...
const IRON_MAP: [string, string][] = [
  ...
  ['분류', '목록 condition — new→신차렌트 · used→중고렌트'], ['제조사', '상세 제목 첫 단어'], ['차명(트림)', '상세 제목에서 제조사를 뺀 나머지'],
...
async function buildRows(m: MirrorSource, header: string[], maps: TabMap[]): Promise<string[][]> {
  ...
  if (m.kind === 'iron') {
    ...
    for (const t of maps) {
      ...
      for (const c of t.cols) {
        ...
        const note = c.ours.some((o) => o === '차명(트림)') && /차종|분류|트림|모델/.test(c.name) ? '차명(트림)은 차종/차종분류·모델명/세부모델·트림을 겹치지 않게 이어 붙인 값' : (!c.ours.length && /대인|대물|자차|자손|무보험|연주행|분납|21세|23세|1만|정비|운전자범위|계좌|기타|비고/.test(c.name) ? '줄별 조건 → 「정책」 탭(정책코드로 조인)' : '');
  ...
  for (const name of header) {
    ...
    const note = name === '상태' ? '원문을 ERP 와 같은 규격값(즉시출고/출고가능/상품화중/출고협의/출고불가)으로 맞춘다. 원본에서 사라진 차는 출고불가'
      : name === '정책코드' ? (m.policies ? '원본 줄의 조건 칸을 정책 탭으로 접어 넣고 여기 코드가 붙는다(sync-mirror-policies). 사람이 다른 코드를 고르면 그대로 둔다' : '비면 「(프리패스 기본)」')
        : name === '차명(트림)' ? '원본 여러 칸(차종/차종분류·모델명·세부모델·트림)을 겹치지 않게 합친 원문. 정제된 이름은 뒤 정제칸(모델·세부모델·세부트림)'
  ...
  R('여기서', '정제칸(차종코드~차종분류) · 정책코드 · 정책 탭 · 차명(트림)/색/연식/옵션/차량가격 같은 «처음 한 번» 칸(옮겨 온 뒤로는 우리 기록 — 미러가 되돌리지 않는다)', '', '');
...
for (const m of targets) {
  ...
  const hi = grid.findIndex((r) => r.some((c) => c.replace(/\s+/g, '') === '차명(트림)'));
