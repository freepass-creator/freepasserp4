...
must(/blankTail[\s\S]*values:\s*\[cols,\s*\.\.\.values,\s*\.\.\.blankTail\]/.test(read('scripts/build-channel-supplier-sheet.mts')),
  'F86 인플레이스 발행이 아래 여유 행까지 같은 요청으로 덮지 않습니다 — 줄 수 감소 때 묵은 차량이 남습니다.', 'scripts/build-channel-supplier-sheet.mts · atomic tail cleanup');
...
const cols = ['구분', '36개월', '36개월 반납형', '분납', '최초등록', '배기량', '배차상태', '차량번호', '제조사', '연료'];
...
{
  // ★2026-09-22 사용자 재확인 — 상품구분은 값별 운영색이다. 신차렌트는 밝은 분홍 FF00FF.
  const 구분조건부 = { addConditionalFormatRule: { rule: { ranges: [{ sheetId: 0, startColumnIndex: 0, endColumnIndex: 1 }], booleanRule: { condition: { type: 'TEXT_EQ', values: [{ userEnteredValue: '신차렌트' }] }, format: { textFormat: { foregroundColor: { red: 1, green: 0, blue: 1 } } } } } } };
  ...
  must(r2.some((r) => r.addConditionalFormatRule === 구분조건부.addConditionalFormatRule),
    '상품구분 값별 운영색이 F86 레트로에서 사라집니다 — 신차렌트 밝은 분홍 등 분류 SSOT를 유지해야 합니다.', `${SKIN} · applyRetroSkin ① — ${MANUAL} 4`);
  must(!r2.some((r) => [배차상태조건부, 제조사조건부, 연료조건부].some((x) => r.addConditionalFormatRule === x.addConditionalFormatRule)),
    'F86에서 제거하기로 한 다른 값별 의미색이 남았습니다.', `${SKIN} · applyRetroSkin ① — ${MANUAL} 4`);
...
must(bodyInk(0) === '', `「구분」에 열 고정색이 남았습니다: ${bodyInk(0)} — 값별 상품구분 색을 덮으면 안 됩니다.`, `${SKIN} · BODY_INK — ${MANUAL} 4`);
...
must(/checkF86TabFreshness\(/.test(auditF86) && /compareF86Cells\(/.test(auditF86)
  ...
  'F86 감사기가 탭 문패와 상품리스트 갱신 시각으로 신선도를 검사하지 않습니다.', `lib/server/f86-audit-checks.ts · scripts/audit-f86-vs-atom.mts — ${MANUAL} 6`);
...
must(/export function f86TabTitle/.test(planSrc) && /F86_BASE_TABS/.test(planSrc)
  && /if \(retro\) return presentationTitle\(company, count, mark\)/.test(planSrc),
...
for (const phrase of ['«완전 커스텀 레트로» 규격 (2026-09-16 픽스)', 'npm run check:f86', '상품리스트 → 손오공상품 → 픽업구독 → 오플구독', '손오공상품 = 저신용 렌트 + 저신용 구독', 'MM.DD HH:mm 상품리스트 N대', 'retroHasLongFee', '맑은 고딕 9pt', '기간이 같은 옛 칸 색', '굳힌 양식', '빈 값 표기 = F01 과 같다', '지키는 장치', 'F86 엔 공지사항 탭이 없다', '신차렌트 `FF00FF`', '배차상태는 파랑 `0000FF`']) {
