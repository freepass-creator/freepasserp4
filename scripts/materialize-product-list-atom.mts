...
// ★공급사 «입력 정책» 자동연결(사장님 2026-09-08) — policy_code 가 «빈» 매물은 그 공급사 정책으로 «투영 시점에» 해소한다.
//   데이터 주도(공급사코드 매칭)라 코드를 갈아엎어도 산다. 원자에 쓰지 않고 읽을 때 붙여, 다음 materialize 가 곧 최신 정책을 반영.
const byProvider = groupPoliciesByProvider((await db.collection('policy').get()).docs.map((d) => ({ _key: d.id, ...(d.data() as Record<string, unknown>) })));
...
for (const { id, x } of prods) {
  const effCode = autoPolicyCode(x, byProvider);   // 적힌 값 우선 · 없으면 공급사 정책(1개→자동)
  if (!S(x.policy_code) && effCode) autoPol++;
  const row = buildProductListRow(effCode ? { ...x, policy_code: effCode } : x, pol, names, ovMap);
...
console.log(`\n■ product_list_atom ${n}개 materialize — listable ${listable} · 회사명 ${names.size} · 오버라이드 ${ovMap.size} · 정책 ${pol.size} · 공급사정책 자동연결 ${autoPol}`);
