...
import { salesTabMatches, SALES_PUBLISHED_TAB_PREFIXES } from '../lib/domain/sheet-contract-identity';
...
try {
  ...
  const credentialPath = process.env.GOOGLE_SHEETS_APPLICATION_CREDENTIALS;
  if (credentialPath !== 'tmp/firebase-auth/sheet-format.json') throw new Error('HOLD: existing workflow credential path unavailable');
