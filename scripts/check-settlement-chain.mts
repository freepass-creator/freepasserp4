 ...
 *
 * ⚠ **줄 수가 갈리는 것에는 다리를 놓지 않는다.** 2026-09-10 에 「0 원인 줄은 원자가 안 싣는다」고
 *   짐작해 다리를 놓을 뻔했는데, 세어 보니 0 원인 원장 줄 11개 중 8개가 원자에 «실려» 있었다.
 *   진짜 까닭은 원장에 청구월이 박혔는데 원자 billMonth 가 빈 것이었다 — 규칙이 아니라 고칠 어긋남이다.
 *   ⇒ 넘기지 말고 **어느 줄인지 찍는다**. 짐작으로 놓은 다리는 사고를 규칙으로 둔갑시킨다.
 *
...
/**
 * ★★**줄 수가 갈리면 «어느 줄»인지 찍는다** — 숫자만 보면 어디를 고쳐야 할지 모른다.
 *
 * ⚠ 2026-09-10 에 이것을 「돈이 0 이라 원자가 안 싣는 것」으로 짐작하고 다리를 놓을 뻔했다.
 *   실제로 세어 보니 «0 원인 원장 줄 11개 중 8개는 원자에 실려» 있었다 — 짐작이 틀린 것이다.
 *   진짜 까닭은 **원장에 청구월이 박혔는데 원자의 billMonth 가 빈 것**이었다(원자화가 그 뒤로 안 돌았다).
 *   ⇒ 다리를 놓지 않는다. 이건 규칙이 아니라 **고쳐야 할 어긋남**이다. 대신 줄을 이름으로 찍어
 *     「무엇을 고치면 되는지」를 화면이 말하게 한다.
 */
if (led.length !== atoms.length) {
  const 원자키 = new Map<string, number>();
  for (const a of atoms) { const k = `${S(a.plate)}|${S(a.customer)}`; 원자키.set(k, (원자키.get(k) || 0) + 1); }
  const 빠진: L[] = [];
  for (const r of led) {
    const k = `${r.plate}|${r.cust}`;
    const n = 원자키.get(k) || 0;
    if (n > 0) 원자키.set(k, n - 1); else 빠진.push(r);
  }
  bad.push(`줄 수가 갈립니다 — 원장 ${led.length}줄 · 원자 ${atoms.length}줄`);
  for (const r of 빠진) {
    bad.push(`   · ${pad(r.plate, 11)} ${pad(r.cust, 9)} ${pad(r.sup, 11)} 청구 ${won(r.claim)} — 원장엔 ${MONTH} 인데 원자 billMonth 가 다릅니다`);
  }
}
