/**
 * **공급사 시트마다 「상품시트」 탭 — 발행된 판매시트(상품리스트·손오공상품·오플구독)에서 그 공급사 줄을 그대로 옮겨 놓는다.** 기본 dry-run, 반영은 `--apply`.
 *
 ...
 *   · 손오공은 렌트(상품리스트 줄)+구독(손오공상품 줄)이 한 탭에 이어 실린다(구독 줄은 요금 블록 머리글이 달라 두 표로 나눠 놓는다). 오토플러스는 오플구독 줄.
