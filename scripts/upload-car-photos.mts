 ...
 *
 * ★폴더 규격은 이미 있는 것을 따른다(SA 폴더 실측 2026-08-11) — 「109호1739 GV70」 처럼
 *   «차량번호 + 차명» 한 폴더에 그 차 사진을 모은다. 규격을 새로 만들지 않는다.
...
const carName = [S(car.sub_model) || S(car.model), S(car.trim_name)].filter(Boolean).join(' ');
console.log(`■ ${PLATE} — ${pname}(${code}) · ${S(car.maker)} ${carName}\n`);
...
const files = readdirSync(dir).filter((f) => /\.(jpe?g|png|webp|heic)$/i.test(f) && (!prefix || f.startsWith(prefix))).sort();
...
const folderName = `${S(car.car_number)} ${carName}`.trim();
