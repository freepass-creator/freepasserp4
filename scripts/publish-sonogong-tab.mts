/**
 * **별도 탭(오공구독 · 오플구독)에 «원본 요금 블록»을 덧붙인다.** 기본 dry-run, 반영은 `--apply`.
 *
 * ★사장님 2026-08-19 — 「복구하자, 판매시트 탭 3개로 회귀: 상품리스트 · 오공구독(반납형이랑 인수형 붙여서) · 오플구독(정제된 거로)」
 *   · 줄은 `publish-origin-tab --only=RP012:구독 --tab=오공구독`(또는 `--only=RP023 --tab=오플구독`)이 **상품리스트와 같은 발행기·같은 정본 차명·같은 열**로 먼저 찍는다.
 ...
 *       오공구독: 보증금 반납형(글자 「연수×대여료」 = 보증금) · 12~60개월 반납형 · 보증금 인수형 · 36/48/60개월 인수형 (제공시트 「구독재고」) → 반납형+인수형 한 탭
 ...
 *   · 두 번 돌려도 블록이 두 벌 안 된다(있으면 걷어 내고 다시 붙임). 탭 이름(「오공구독 MM.DD HH:MM · N대」)은 origin-tab 이 준 그대로 둔다.
 * ★상품리스트에는 이 두 갈래가 @제외로 빠져 있다(SALES_EXCLUDE) — 같은 차가 두 탭에 서면 사고.
 * ★옛 「손오공인수형구독」 탭(2026-08-18 하루짜리)은 지운다.
 *
 *   npx tsx scripts/publish-sonogong-tab.mts [--apply]                 # 오공구독(기본)
...
import { NATIVE_MONEY_BLOCK, STANDARD_MONEY_COLUMNS, nativeMoneyLabel, canonicalSalesTabName, salesTabMatches, type NativeLeadColumn } from '../lib/domain/sales-published-tabs';
...
const tabCandidates = ((meta0.sheets || []) as Rec[]).map((s) => S(s.properties?.title)).filter((t) => salesTabMatches(t, TAB));
if (tabCandidates.length > 1) throw new Error(`판매 탭 중복: ${TAB}`);
const tabTitle = tabCandidates[0];
...
// 갈래 탭(오공구독·픽업구독·오플구독)에선 그 공급사가 안 쓰는 빈 칸을 뺀다 — 상품리스트엔 남긴다(사장님 2026-08-27 「6개월 어정쩡한 거 날려줘」).
const 갈래빈칸 = ['6개월'];
