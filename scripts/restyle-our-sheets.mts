/**
 * **우리가 만든 공급사 시트에 값은 건드리지 않고 서식만 다시 입힌다.** 기본 dry-run, 반영은 `--apply`.
 *
 * ⚠ 이름이 비슷한 `restyle-supplier-sheets` 와 **다른 도구**다. 그쪽은 정리표에 적힌
 *   «우리가 뜬 사본»을 손보고, 이쪽은 드라이브의 「프리패스 재고 · …」 원본을 손본다.
 *
...
for (const f of ((found.files || []) as Rec[])) {
  ...
  if (!stocks.length) { console.log(`  △ ${label.padEnd(12)}재고 탭이 없다`); continue; }
  for (const sh of stocks) {
   ...
   */
  /**
   * ★렌트·구독 두 규격에서 모두 되찾는다. 구독 탭 열은 「12개월 인수형」 꼴이라
   *   렌트 정의만 뒤지면 한 칸도 못 찾아 헤더가 밋밋해진다.
   */
  const periodsOf = (re: RegExp) => hdr.filter((h) => re.test(h)).map((h) => (h.match(/^(\d+)개월/) || [])[1]).filter(Boolean) as string[];
  const known = new Map([
    ...buildColumns(periodsOf(/^\d+개월$/)).map((c) => [c.name, c] as const),
    ...buildSubscriptionColumns(periodsOf(/^\d+개월\s/)).map((c) => [c.name, c] as const),
  ]);
  ...
  console.log(`  ${`${label}/${TAB}`.padEnd(18)}열 ${cols.length} · 옛 서식 ${had} → 새 서식 ${reqs.filter((r) => r.addConditionalFormatRule).length}`);
  if (!APPLY) continue;
