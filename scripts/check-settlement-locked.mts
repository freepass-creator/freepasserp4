/**
 * ⚠⚠ **2026-09-06 — 이 검사를 «되살려» main 에 올렸다. 지금은 «빨간불»이다.**
 *
 * CLAUDE.md 는 「정산 기계 검사 = `npm run check:settlement`」라고 못 박아 두었는데,
 * main 에는 **그 스크립트도 매뉴얼도 없었다.** 둘 다 `feat/spring-atom-monitor` 가지에만 있었고
 * 그 가지는 main 대비 **193 커밋이 미머지**다(2026-09-06 실측).
 *
 * 되살려 돌려 보니 **일곱 군데가 어긋난다** — 넉 칸(확인·정정·정정금액·메모)·예정분·「누락」 줄 읽기 등.
 * 즉 **검사가 틀린 게 아니라 main 이 그 작업들을 아직 안 받은 것**이다.
 * 그래서 규칙 그대로 — **정본(검사·매뉴얼)을 먼저 세우고, 코드가 따라오게 둔다.**
 *
 * ★그래서 **CI 에는 아직 안 걸었다.** 정산 파이프라인 작업이 main 에 들어오면 그때 건다.
 *   ⚠ 통과시키려고 이 검사를 «약하게» 고치지 말 것 — 그건 규격을 지우는 것이다.
 */
/**
