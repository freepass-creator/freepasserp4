 ...
 * ★「영업사」는 2026-09-08 에 **「영업담당자」로 이름이 바뀌었다** — 접수 탭이 쓰는 이름에 맞춘 것이고,
 *   같은 자리에 「영업채널」이 나란히 섰다(사장님 「영업채널 하허호 담당자 이태헌 이런 식으로」).
 *   ⇒ 지키는 것은 «이름»이 아니라 «그 뜻의 칸이 있느냐»다. 둘 다 요구한다.
 */
for (const col of ['차량 가격(신차)', '영업채널', '영업담당자', '보증금', '납입 방식']) {
  must(tabs.includes(`'${col}'`),
    `달 탭 머리글에서 「${col}」 칸이 빠졌습니다. 하허호가 시트 메모로 요청해 붙인 칸입니다.`,
    `lib/server/channel-sheet-tabs.ts · SETTLE_COLUMNS — ${manual} §칸 규격`);
}
/**
 * ★★**빗장 — 「영업채널·영업담당자」는 공급사 시트에 서면 안 된다.**
 *   사장님 「절대 영업자 지급 수수료가 얼만지 공급사시트에는 반영되면 안 돼」의 거울.
 *   `hide: '공급사'` 가 그 일을 하는 유일한 장치라, 그게 붙어 있는지를 여기서 지킨다.
 */
for (const col of ['영업채널', '영업담당자']) {
  must(new RegExp(`'${col}'[^}]*hide: '공급사'`).test(tabs),
    `「${col}」 칸에 «hide: '공급사'» 빗장이 없습니다 — 공급사 시트에 채널 쪽 사람이 드러납니다.`,
    `lib/server/channel-sheet-tabs.ts · SETTLE_COLUMNS — ${manual} §칸 규격`);
}
