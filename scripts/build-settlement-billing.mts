 ...
 *
 * ★**청구 줄의 짝은 분납실적이나 완료실적에 있다.** 접수·취소에는 있으면 안 된다 — 검산한다.
 * ★**2026-08부터만 쌓는다.** 그 전은 이미 청구가 끝났다(사장님 「기존거는 다 청구했고 이거는
 *   따로 맞출게 나는 이번달거랑 앞으로 어떻게 할지만 볼거야」). 과거는 완료실적에 그대로 있다.
...
const TAB = '청구';
const PAY = '분납실적', DONE = '완료실적', CUR = '접수', CANCEL = '취소';
...
const ABOUT = '청구 — 청구월로 쌓는 장부입니다. **분납실적 + 완료실적**에서 만듭니다. 2026-08부터 담습니다(그 전은 이미 청구가 끝났습니다). '
  + '한 계약이 한 줄이고, 환수가 있으면 그 줄이 주황이 되며 순액 = 우리 몫 − 환수금액입니다. '
  + '⚠ **환수는 여기서 적지 않습니다** — 분납실적·완료실적에서 「환수」에 체크하고 환수일·환수금액을 적으면 여기에 따라옵니다. '
...
const OUT = [
  ...
  '접수일', '접수년', '접수월', '차량번호', '공급사', '고객명', '영업채널', '영업담당자', '상품구분',
  ...
  '환수사유', '비고'] as const;
/**
 * ★**여기서는 환수를 적지 않는다.** 환수 체크·환수일·환수금액은 **실적 탭(분납실적·완료실적)**에서
...
const COPY: Partial<Record<(typeof OUT)[number], string>> = {
  차량번호: '차량번호', 접수일: '접수일', 접수년: '접수년', 접수월: '접수월', 고객명: '고객명', 공급사: '공급사',
...
/**
 * ★**청구서의 원천은 「분납실적 + 완료실적」이다**(사장님 「분납과 완료실적을 기반으로 청구서를 만드는거지」).
...
if (!rows.length) { console.log('⛔ 분납실적·완료실적이 비었다 — build-settlement-tabs 를 먼저 돌려라.'); process.exit(1); }
...
for (const x of rows) {
   ...
   *   과거 환수도 이미 정산이 끝난 것으로 본다 — 남아 있으면 완료실적에서 여전히 보인다.
...
console.log(`   과거(${FROM} 앞) ${skipped}줄은 안 담았다 — 이미 청구가 끝났고 완료실적에 그대로 있다.`);
...
for (const [mk, outM] of [...byMonth].sort()) {
  ...
  const plain = (['접수년', '접수월', '청구년', '청구월', '계약기간'] as const).map(col).filter((i) => i >= 0);
