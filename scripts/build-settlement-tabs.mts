/**
 * **정산원장 탭을 넷으로 세운다 — 접수 · 취소 · 분납실적 · 완료실적.** 기본 dry-run, 반영은 `--apply`.
 *
 * ★사장님 2026-08-25
 ...
 * 분납실적   청구월이 있고 분납 만료가 안 지났다. 지나고 환수가 없으면 저절로 완료실적으로 간다
 * 완료실적   청구월이 있고 (일시납이거나 만료가 지났다). 환수도 여기 남는다
 * ```
 *
 ...
 *
 * ★**환수는 취소가 아니다.** 계약은 완료됐고 나중에 조건이 터진 것 → 완료실적 + 마이너스 청구.
...
const SOURCES = ['실적', '작업', '접수', '취소', '분납실적', '완료실적', '당월실적'];
/**
...
const CUR = '접수', CANCEL = '취소', PAY = '분납실적', DONE = '완료실적';
 ...
 *     환수를 켜면 그 줄은 분납실적에서 빠져 완료실적으로 간다.
...
const HINT: Record<string, string> = {
  ...
  환수: '계약이 끝난 뒤 조건이 터져 돈을 되돌릴 때 체크 — **분납이 부러진 것도 여기에 체크합니다.** 켜면 그 줄은 분납실적에서 빠져 완료실적으로 갑니다. 환수는 여기서만 체크하고 청구 탭은 읽기만 합니다.',
...
Object.assign(ABOUT, {
  [CUR]: '접수 — 미처리 이월건 + 당월 접수건. 접수일 빠른 것이 위이고, 아래 빈 줄에 이어 적습니다. '
    ...
    + '달이 바뀌면 분납이면 「분납실적」, 일시납이면 「완료실적」으로 옮겨집니다.',
  ...
  [PAY]: '분납실적 — 인도됐고 보증금 분납이 아직 안 끝난 것. **1회차는 인도 때 냈고**, 「다음회차일」이 다음에 들어올 날입니다 — '
    ...
    + '켜면 줄이 주황이 되고 청구에서 그만큼 빠지며, 더 굴러갈 회차가 없어 「완료실적」으로 옮겨집니다. 만료 = 인도일 + 회차개월. '
    + '만료가 지나고 환수가 없으면 제대로 이행된 것이라 저절로 완료실적으로 넘어갑니다. 만료 가까운 순으로 놓입니다.',
  [DONE]: '완료실적 — 끝난 것. 일시납이면 인도완료를 체크하는 순간 바로 여기로 오고, 분납은 만료가 지나면 옵니다. '
    + '환수도 여기 남습니다. 청구서는 «분납실적 + 완료실적»에서 만듭니다. 쌓기만 하고 손대지 않습니다.',
});
 ...
 * ```
 * 행선지 — 인도완료되면 분납이면 「분납실적」, 일시납이면 「완료실적」으로 간다
 * 시점  — 다만 **당월 접수건은 이달이 마무리될 때까지 접수에 머문다**
 *         (사장님 「완료실적으로 넘기는거는 이달 마무리 되면」 · 「접수에 당월거는 아직 안옮긴다고」)
 * ```
...
for (const row of seen.values()) {
  ...
  if (ON(row[head.indexOf(CANCEL_BOX)])) cancel.push(row);
    ...
    const due = dueOf(row);
    // ★환수를 켜면 분납은 끝난다 — 완료실적으로 보낸다. 되돌릴 돈은 환수금액이 말한다.
...
/**
 * ★**네 탭이 같은 차례로 선다**(사장님 2026-08-25 「접수탭이 그대로 넘어가고 분납실적 / 완료실적에
 *   접수탭 뒤로 세부조건들을 쭉 달아야 하는거잖아」).
 *   한 계약이 접수 → 분납실적 → 완료실적으로 옮겨가는데 탭마다 열 차례가 다르면
 ...
 *   원장을 편집 권한으로 열어 줘도 팀장이 완료실적·청구를 건드릴 수 없다.
...
for (const [DTAB, DBODY] of [[CUR, curBody], [CANCEL, cancel], [PAY, pay], [DONE, done]] as const) {
   ...
   *   접수탭에는 있을필요는 없어 · 분납실적 완료실적에만 존재하지」).
