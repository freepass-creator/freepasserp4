...
/**
 * ★**한 곳 실패가 «전체»를 멈추지 않는다** — ①′ 아래와 같은 이유(2026-09-15).
 *
 *   `sync-mirror-all.mts` 는 네 소스를 순서대로 돌며 실패한 소스만 건너뛰고 나머지는 이미 써 둔다
 *   (한 곳 실패해도 루프는 계속 돈다) — 그리고 **하나라도 실패하면 exit 1** 로 끝난다.
 *   여기서 그 exit 1 을 그대로 `stop()` 하면, 아이언(홈페이지 스크래핑 — 상세 한 건만 형식이
 *   달라져도 fail-closed 로 던진다)이 실패했다는 이유로 **아이카·오토플러스·이안카까지** 그 시간의
 *   판매시트 발행(⑥)·ERP 반영(⑦)을 전부 못 받는다 — 「아이언만 빠졌다」가 아니라
 *   실은 전체가 그 시각에 멈춘 것이다.
 *
 *   실패한 소스의 정제시트만 «갱신 안 됨»(어제 값 유지)으로 남기고, 나머지는 그대로 발행까지
 *   보낸다 — 매뉴얼 §1-③ 그대로 「멈추면 «어제 표»가 남는다. 덜 갱신된 표가 낫다」.
 * ⚠ **성공으로 적지 않는다** — ①′ 와 동일하게 `ok:false`·경고·메일은 그대로 남긴다.
 */
if (!s1.ok) {
  warnings.push('① 정제시트 갱신 일부 실패 — 발행은 계속한다(실패한 소스만 어제 값 유지)');
  console.log('   ▲ ① 일부 실패 — 그래도 ⑥ 발행까지 간다(멈추면 다른 공급사까지 «어제 표»가 된다). 상태로그는 ok:false 다.');
}
line.push(s1.ok ? `정제시트 ${s1.picked.find((l) => /새 차/.test(l))?.replace(/\s+/g, ' ').slice(0, 60) || 'ok'}` : '정제시트 ✗(경고 · 일부만 발행)');
