 ...
 *   ⑥ 판매시트 «4탭» 발행(상품리스트 · 손오공상품 · 픽업구독 · 오플구독) + 요금블록
...
if (existsSync(손오공계정)) {
   ...
   *   ⓪ 을 건너뛴 채 ⑥ 에서 손오공상품·픽업구독을 다시 발행하면 «낡은 재고»가 새 발행처럼 찍힌다.
  ...
  warnings.push('손오공 없이 진행 — 손오공상품·픽업구독 발행과 ⑩ 천이를 건너뛴다');
 ...
 * ⚠ 실측 2026-09-02: 손오공상품 72대 중 원산지가 24%밖에 안 차 있었다 — 91칸이 비어 있었고
...
if (손오공탭발행) {
  const p2 = run('⑥ 스테이징 손오공상품', ['scripts/publish-origin-tab.mts', ...STAGE, '--only=RP012:구독', '--tab=손오공상품', '--at=1', ...A], /반영 완료|중단|Error/);
  if (!p2.ok) stop('손오공상품 발행 실패');
...
if (APPLY) {
   ...
   *   ⚠ 2026-09-08 — 두 칸이 다른 값을 들어 한쪽은 「계약중」, 다른 쪽은 「출고불가」였다(실측 149대).
   *   시트는 `vehicle_status`, ERP 일부는 `status` 를 읽으니 **판 차가 목록에 다시 설 수 있었다.**
   *   잠금(출고불가·계약중)이 이긴다 — 팔 수 있는 차가 하루 숨는 편이 낫다. best-effort.
   */
  const stHeal = run('⑬½¼ 상태 아물기', ['--require', './scripts/lib/server-only-shim.cjs', 'scripts/heal-atom-status.mts', '--apply'], /한 벌로 아물렀다|이미 한 벌|▲/);
  if (!stHeal.ok) warnings.push('⑬½¼ 상태 아물기 실패(발행엔 영향 없음)');

  /**
   * ⑬½½ **원천 주소 검사** — 값을 재기 «전»에 «어디서 읽는지»를 잰다.
   *   ⚠ 2026-09-08 — 웰릭스가 **2026-08-15 에 폐기된 옛 시트**를 24일 동안 읽고 있었다. 그 시트의
   *   차명 열 이름이 달라 차명이 비었고, 남은 제조사 한 마디(「기아」)를 보고 매칭기가 K8 을 「모닝」으로 붙였다.
   *   시트는 «읽혔고» 행도 «있었으니» 값 검사로는 안 잡힌다 — 주소를 봐야 잡힌다. 실측 11개 공급사가 같은 상태였다.
   *   ★알림만 한다 — 읽는 주소가 죽었으면 그 공급사 수집이 먼저 실패하므로 여기서 또 멈출 일은 아니다.
   */
  const srcChk = run('⑬½½ 원천 주소 검사', ['--require', './scripts/lib/server-only-shim.cjs', 'scripts/audit-supplier-source.mts'], /주소는 전부 살아|죽은 원천|▲|⛔/);
  if (!srcChk.ok) warnings.push('원천 주소가 어긋났다 — 폐기 시트를 읽고 있을 수 있다(문패를 고쳐라)');
