 ...
 *   ⑥ 판매시트 «4탭» 발행(상품리스트 · 오공구독 · 픽업구독 · 오플구독) + 요금블록
...
if (existsSync(손오공계정)) {
   ...
   *   ⓪ 을 건너뛴 채 ⑥ 에서 오공구독·픽업구독을 다시 발행하면 «낡은 재고»가 새 발행처럼 찍힌다.
  ...
  warnings.push('손오공 없이 진행 — 오공구독·픽업구독 발행과 ⑩ 천이를 건너뛴다');
 ...
 * ⚠ 실측 2026-09-02: 오공구독 72대 중 원산지가 24%밖에 안 차 있었다 — 91칸이 비어 있었고
...
if (손오공탭발행) {
  const p2 = run('⑥ 스테이징 오공구독', ['scripts/publish-origin-tab.mts', ...STAGE, '--only=RP012:구독', '--tab=오공구독', '--at=1', ...A], /반영 완료|중단|Error/);
  if (!p2.ok) stop('오공구독 발행 실패');
