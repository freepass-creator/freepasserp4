...
const 참고: string[] = [];
for (const m of 달) {
  const L2 = led.filter((r) => r.month === m);
  ...
  console.log(`   ${pad(m, 9)} ${String(L2.length).padStart(4)} ${String(A2.length).padStart(4)}   ${W(LC).padStart(13)} ${W(AC).padStart(13)}   ${W(LP).padStart(13)} ${W(AP).padStart(13)}${표}`);
