/**
 * **공급사 시트 재고 탭에 「차대번호」 열을 보장한다** — 차량번호 바로 뒤. 없을 때만 넣는다(있으면 아무것도 안 함). 기본 dry-run, --apply 로 반영. --who=손오공 한 곳만 · --skip=아이언,손오공 건너뜀.
 *   사장님 2026-08-19 공통 항목: 차량번호 입고일자 점검사항 상태 분류 제조사 **차대번호** 차명(세부모델+트림) 옵션 외부색상 내부색상 연식 주행거리 연료 배기량 차량가격
 *   · 차대번호 = 검색되는 모델 이름(제조사 말·연료 꼬리 뗀 것, 차종마스터가 알면 그 이름) — 값 채움은 별도(fill-from-saeop / sync-mirror-sheet / fill-model-names).
...
for (const b of books) {
  ...
  for (const p of tabs) {
    ...
    const hdr = rows[hi]; const mi = hdr.findIndex((h) => norm(h) === '차대번호'); const mk = hdr.findIndex((h) => norm(h) === '차량번호');
    if (mi >= 0) { already++; console.log(`   ${b.label.padEnd(8)} 「${title}」 차대번호 열 있음(${colA1(mi)})`); continue; }
    if (mk < 0) { console.log(`   ! ${b.label} 「${title}」 차량번호 열 없음 — 건너뜀`); continue; }
    console.log(`■ ${b.label.padEnd(8)} 「${title}」 차대번호 열 없음 → 차량번호(${colA1(mk)}) 뒤 ${colA1(mk + 1)} 에 ${APPLY ? '추가' : '추가 예정'}`);
    added++;
    if (!APPLY) continue;
    ...
    await call(`${SH}/${b.id}/values/${encodeURIComponent(`'${title.replace(/'/g, "''")}'!${colA1(mk + 1)}${hi + 1}`)}?valueInputOption=RAW`, { method: 'PUT', body: JSON.stringify({ values: [['차대번호']] }) });
    await sleep(300);
  }
}
console.log(`■ 차대번호 열 ${APPLY ? '추가' : '추가 예정'} ${added} · 이미 있음 ${already}${APPLY ? '' : ' (dry-run)'}`);
