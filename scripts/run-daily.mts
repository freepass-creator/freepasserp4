 ...
 *   ④ 상품시트 발행(상품리스트 → 오공구독(+인수형 블록) → 오플구독(+오플 요금 블록) — 탭 3개, 2026-08-19)
...
// ★탭 4개(2026-08-27 픽업구독 추가): 상품리스트 · 오공구독 · 픽업구독 · 오플구독. 같은 발행기로 갈래 탭을 찍고 원본 요금 블록을 덧붙인다.
const p2 = run('④ 스테이징 오공구독', ['scripts/publish-origin-tab.mts', ...STAGE, '--only=RP012:구독', '--tab=오공구독', '--at=1', ...A, ...(FORCE ? ['--force-shrink'] : [])], /우리 시트 |반영 완료|Error|중단/);
report.push(`④ 오공구독 ${p2.ok ? '✓' : '✗'} ${p2.picked.find((l) => /반영 완료|우리 시트 /.test(l)) || ''}`); if (!p2.ok) stop('오공구독 탭 발행 실패');
const p2b = run('④ 스테이징 손오공 인수형', ['scripts/publish-sonogong-tab.mts', ...STAGE, ...A], /실을 차|사진링크|반영 완료|Error/);
report.push(`④ 인수형 블록 ${p2b.ok ? '✓' : '✗'} ${p2b.picked.find((l) => /반영 완료|실을 차/.test(l)) || ''}`); if (!p2b.ok) stop('오공구독 인수형 블록 실패');
