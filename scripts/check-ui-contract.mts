  ...
  /* 견적(`/estimate`)의 화면 정본은 **웰릭스 테이블**이다(사장님 2026-09-07 「야 일단 웰릭스 테이블을
     **그대로 복사**해와봐」 · 「거기서 **신차에서 중고차로만 변환**하고 **원가구조만 다르게** 쓰면 되는 거잖아」).
     ...
     raw 를 세면 이렇게 된다 —
       ...
       input 16  = 차량정보(시세·연식·주행·배기량·매입할인·신차 차량가) + 잔가 5 +
                   손님·담당자·연락처·수수료 + 조건(보증금·선납) … 원본 `.cs-field`/`.qc-field` 짜임
       select 2  = 취득 경로 · 「손님 발송용」 기간 드롭다운(원본 `.term-card__term-dd`)
     ⚠ 숫자가 달라지면 화면이 «원본에서» 벗어났다는 뜻이다. 고치기 전에 **원본과 대조**할 것
       (`C:\dev\welrixtable/index.html` · `src/components/*.vue`). */
  ['app/estimate/page.tsx', {
    counts: { button: 7, input: 16, select: 2 },
    reason: '견적 = 웰릭스 테이블 그대로 — 상품 카드·차종·차량정보·잔가·손님/담당자·조건·발송용 견적',
  }],
