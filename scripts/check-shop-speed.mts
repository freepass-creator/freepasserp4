 ...
 *
 * ⚠ 운영 실측(2026-09-17 웹 1440) — 카드 칸은 297px 인데 `33vw`(475px)라 적혀 있었다.
 *   웹 목록은 본문이 1280 에서 멈추고 조건칸(260)을 빼고 셋으로 나누므로 **화면이 아무리 넓어도
 *   320 을 안 넘는다** — 여기는 «화면 비율»이 아니라 «상한»이다.
 * ★버는 곳은 **망막 화면(DPR 2)** 이다 — 1080 → 640(픽셀 -45%). 보통 화면(DPR 1)은 전과 같다:
 *   `sizes` 에 vw 가 있으면 Next 가 640 밑 후보를 다 버려서(`getWidths` smallestRatio) 384 가
 *   애초에 없다. 자세한 셈은 `PHOTO_SIZES.card` 머리말.
